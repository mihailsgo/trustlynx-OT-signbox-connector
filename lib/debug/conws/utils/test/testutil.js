csui.define([
  ...
  // methods set_footprint and log_footprint just for debugging purposes
  ...
  function log_footprint(obj,message) {
    // to enable log output for debugging promise chain
    // set footprint = [] in test case and pass message asa last method to run method.
  ...
  function do_run(done,action,message) {
    ...
    function subdone() {
      // trigger next step. call done method if there is no next step.
    ...
    subdone.fail = function (message) {
      // propagate to next step. call done.fail if there is no next step..
  ...
  function do_justWait(done,message,timeout) {
    ...
    if (!_.isFunction(done)) {
      // assume that no "done" method is given and reassign/shift parameters
  ...
  function do_waitFor(done,check,message,timeout) {
    ...
    // create error object here to have better stack trace on timeout
    timeout===undefined && (timeout = 1000);
  ...
  var TestUtil = {
    ...
    // for test and logging purposes copy and reduce object to some depth only and fake functions.
    ...
    // set polyfill here, if we need the event type
    // also in browsers, which do not support it.
    ...
    /**
     * use this method to check consistency of mocked responses according common url parameters.
     * use this mockjax calls for business workspaces REST calls to be sure to provide reasonable
     * data to the ui, as it makes no sense to provide data, that never really occur.
     *
     * @param {string} url a URL containing 'fields=properties{...}'
     * @param {json} response a response, that is checked to contain the same fields.
     * @param {string} where a text displayed with the error message to identify the use/test case
     */
