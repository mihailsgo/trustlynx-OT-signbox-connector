...
csui.define([
  ...
  'csui/utils/log'
], function ($, _, mockjax, PageContext, TestUtil, HeaderView, IconView, log) {
  ...
  TestManager.iconView = undefined;
  ...
  TestManager.init = function (id, options) {
    ...
    if (!TestManager.view) {
      ...
      TestManager.iconView = new IconView(viewOptions);
    }
  };
  ...
  TestManager.prepareWkspId = function (nodeId, wkspId) {
    var url = TestManager.perspectiveUrl.replace('{0}', wkspId);
    $.mockjax({
      url: url,
      responseTime: 10,
          ...
          [wkspId]: {
            "data": {
              "addcategory": {
                ...
                "content_type": "",
                "form_href": "",
                "href": "/api/v2/nodes/"+wkspId+"/categories",
                "method": "POST",
                "name": "Add Category"
              },
              "comment": {
  ...
  TestManager.prepare = function (id) {
    ...
    //console.log("prepare, id",id,"TestManager.id",TestManager.id);
    $.mockjax({
      url: TestManager.url,
      responseTime: 10,
  ...
  TestManager.prepareError = function (id) {
    ...
    $.mockjax({
      ...
      responseTime: 100,
      status: 400,
      responseText: {"error":"Invalid datatype specified for argument \"bw_id\"."}
    });
  };

  TestManager.fetch = function (done,view,id) {

    //console.log("fetch, id",id,"TestManager.id",TestManager.id);

    // prepare test manager
    this.prepare(id);

    // fetch data and wait
    //console.log("fetching, id",id,"TestManager.id",TestManager.id);
  ...
  // test data - default workspace representation
  ...
  // test data - no workspace type image
  ...
  // test data - workspace instance image
  TestManager.testWSImage = function () {
    var ret = this.testBase("WSImage");

    // remove type icon content and set instance icon
  ...
  // test data - no workspace edit permissions
  ...
  // test data - some workspace edit permissions
  ...
  // test data - multi-value attributes
