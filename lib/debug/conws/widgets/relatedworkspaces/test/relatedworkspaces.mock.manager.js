csui.define(['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/jquery.mockjax',
  'csui/utils/log', 'conws/utils/test/testutil'
], function (_, $, mockjax, log, TestUtil) {

  var doLogConsole = false, doLogResponse = false;
  var logConsole = function (logobj) {
    doLogConsole && console.log(logobj);
  };

  var logResponse = function (response) {
    if (doLogResponse) {
      console.log("adding response to mockjax:");
      var items = response.results;
      for (var ii = 0; ii < items.length; ii++) {
        console.log(items[ii].data.properties.name);
      }
    }
  };
  ...
  DataManager.test = function (totalCount, title, workspaceTypeId, addCustomColumns) {
    ...
    mockjax({
      ...
      //url: '//server/otcs/cs/api/v2/businessworkspaces/120?metadata&fields=categories&include_icon=true&expand=properties%7Bcreate_user_id%2Cmodify_user_id%2Cowner_group_id%2Cowner_user_id%2Creserved_user_id%7D',
      responseText: {
        "links": {
          "data": {
            "self": {
  ...
  DataManager._provideMockjax = function (totalCount, title, workspaceTypeId, addCustomColumns,
    ...
    if (workspaceTypeId) {
      urlsRel = [
        ...
        '&where_relationtype=child&global_metadata',
        baseUrl + '?where_workspace_type_id=' + workspaceTypeId +
        '&where_relationtype=parent&global_metadata'
      ];
    } else {
      urlsRel = [
        baseUrl + '?limit=' + pageSize + '&page=1&sort=asc_name&global_metadata'
      ];
    }
    ...
    urlsRel.forEach(function (url) {
      ...
      mockjax({
        ...
        response: _.partial(function(options,responseText) {
          TestUtil.assertWkspMockConsistency(options.url,responseText,'related workspaces 1');
          this.status = 200;
    ...
    sorting.forEach(function (sort) {
      ...
      for (var i = 0; i <= pages; i++) {
        ...
        var fieldsParams = [
          '?fields=' +
          encodeURIComponent('properties{id,container,name,type,type_name,custom_123_1,custom_123_2,wnd_modifiedby,modify_date}') +
          '&action=properties-properties',
          '?fields=' +
          encodeURIComponent('properties{id,container,name,type,type_name,custom_123_1,custom_123_2,custom_124_1}') +
          '&action=properties-properties',
          '?fields=' +
          encodeURIComponent('properties{id,container,name,type,type_name}') +
          '&action=properties-properties',
          '?fields=' +
          encodeURIComponent('properties{custom_124_1,custom_123_2,custom_123_1,type,name,size,modify_date,favorite,id,type_name,container}') +
          '&action=properties-properties',
          '?fields=' +
        ...
        relation.forEach(function (relType) {
          ...
          fieldsParams.forEach(function (fieldParams) {
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      ...
                      '&sort=' + sort + '&expand_users=true&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&where_relationtype=' + relType + '&sort=' + sort +
                      '&expand_users=true&limit=' + limit +
                      '&page=' + page + '&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&where_relationtype=' + relType + '&expand_users=true&limit=' + limit +
                      '&page=' + page + '&sort=' + sort + '&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&limit=' + limit + '&page=' + page + '&expand_users=true&global_metadata' +
                      '&where_relationtype=' + relType);
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&limit=' + limit + '&page=' + page + '&sort=' + sort +
                      '&expand_users=true&global_metadata' +
                      '&where_relationtype=' + relType);
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
          ...
          urls.forEach(function (url) {
            ...
            mockjax({
              ...
              responseTime: 0,
              response: _.partial(function(options,responseText) {
                TestUtil.assertWkspMockConsistency(options.url,responseText,'related workspaces 2');
                this.status = 200;
                this.responseText = responseText;
              },_,responseText)
            });
          ...
          fieldsParams.forEach(function (fieldParams) {
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&where_relationtype=' + relType + '&limit=' + limit + '&page=' + page +
                      '&sort=' + sort + '&expand_users=true' + filterPart + '&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&where_relationtype=' + relType + '&sort=' + sort +
                      '&expand_users=true&limit=' + limit +
                      '&page=' + page + filterPart + '&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&where_relationtype=' + relType + '&expand_users=true&limit=' + limit +
                      '&page=' + page + '&sort=' + sort + filterPart + '&global_metadata');
            urls.push(baseUrl + fieldParams + '&where_workspace_type_id=' + workspaceTypeId +
                      '&limit=' + limit + '&page=' + page + '&sort=' + sort +
                      '&expand_users=true&global_metadata' +
          ...
          urls.forEach(function (url) {
            ...
            mockjax({
              ...
              responseTime: 0,
              response: _.partial(function(options,responseText) {
                TestUtil.assertWkspMockConsistency(options.url,responseText,'related workspaces 3');
                this.status = 200;
                this.responseText = responseText;
              },_,responseText)
            });
