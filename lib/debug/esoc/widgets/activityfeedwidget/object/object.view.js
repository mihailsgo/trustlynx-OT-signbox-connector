...
], function (_, $, Backbone, Marionette, Log, DefaultActionBehavior, ConnectorFactory, CommonUtil,
    base, ActivityFeedItemView, EmptyActivityFeedView,
    PerfectScrollingBehavior, BlockingView, InfiniteScrollingBehavior, Util, NodeFactory,
    ErrorView, objectTemplate, lang, Adaptivepoll) {
  var ObjectView = Marionette.CompositeView.extend({
    ...
    initialize: function (options) {
      ...
      this.setError = function (request) {
        var error = new base.Error(request),
          errorRegion = new Marionette.Region({
            el: this.el
          });
        this.unblockActions();
        this.$el.find(".esoc-empty-activityfeed-wrapper").show();
        this.el.classList.add('esoc-afw-error');
      ...
      this.listenTo(this.collection, 'error', _.bind(function (collection, request, options) {
        if (!self.$el.parents().hasClass("tile-content")) {
          var headeractivityfeed = setInterval(function () {
            if (self.$el.parents().hasClass("conws-header-widget")) {
              self.setError(request);
              clearInterval(headeractivityfeed);
            }
          }, 100);
        }
        else {
    ...
    getNewSinceId: function (e) {
      ...
      $.ajax(conn.extendAjaxOptions({
        url: conn.getConnectionUrl().getApiBase('v2') + this.commonUtil.REST_URLS.csGetAFSinceId + self.feedtype +
             queryString,
        type: 'GET',
        success: function (data, status, jXHR) {
          var responsefromServer = data.results.lastestIDReturned !== undefined;
