  ...
  var ActivityFeedItemView = Marionette.ItemView.extend({
    ...
    templateHelpers: function () {
      ...
      var mimeTypeClass           = "csui-icon mime_document",
          ...
          objModel                = this.model, subType, defaultActionUrl, isSGMShortCut, isBwsNode,
      ...
      try {
        if (this.model.attributes.extended_info.display_icon_id === "") {
          ...
          this.bwsNodeId = this.options.context.viewStateModel.attributes.session_state &&
            (!!this.options.context.viewStateModel.attributes.session_state.id) ?
            this.options.context.viewStateModel.attributes.session_state.id :
            (!!this.options.context.node ? this.options.context.node.get('id') : undefined);
          if (!( this.bwsNodeId === this.sgmModel.get('id') && this.sgmModel.get('type') === 848)) {
        ...
        if (this.model.attributes.extended_info.attachment_subtype !== "") {
          ...
          if (this.bwsNodeId === this.attachmentModel.get('id') && this.attachmentModel.get('type') === 848) {
            isBwsNode = true;
          }
    ...
    events: {
      "click .esoc-socialactions-getcomments.esoc-social-comment-container button": "getComments",
    ...
    constructor: function ActivityFeedItemView(options) {
      ...
      var commentCount = this.model.get('wnd_comments') || this.model.get('comments') || 0;
      this.iconName = commentCount > 0 ? 'esoc_comment32' : 'esoc_no_comment32';
    ...
    onShow: function (e) {
      ...
      if (this.model.attributes.feed_event_type === 9) {
        ...
        this.isBwsNode = this.model.attributes.data_id === this.bwsNodeId && this.model.attributes.type === 848;
        if (this.model.attributes.extended_info.subtype === 1) {
          if (this.isBwsNode) {
            sgmText = sgmText.replace('[MimeType]',
              "<span class='csui-icon-group esoc-social-no-link-mime-icon'>" +
          ...
          sgmText = sgmText.replace('[MimeType]',
            "<span class='" + this.templateHelpers().messages.mimeTypeClass +
            " esoc-social-no-link-mime-icon'></span>");
          }
          else {
            sgmText = sgmText.replace('[MimeType]',
            "<span class='" + this.templateHelpers().messages.mimeTypeClass +
        ...
        if (this.isBwsNode) {
          this.$el.find(".esoc-object-name").addClass('esoc-object-no-link-name');
          this.$el.find(".esoc-object-name").removeClass('esoc-object-name');
          this.$el.find(".esoc-object-no-link-name").addClass(this.templateHelpers().messages.inactiveClass);
        }
        else {
