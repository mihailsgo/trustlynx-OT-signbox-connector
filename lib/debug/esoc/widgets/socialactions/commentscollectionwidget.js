  ...
  function CommentsCollectionWidget(options) {
    ...
    if (isvisible) {
      ...
      this.show = function () {
        ...
        $("[id^='esoc-social-comment-widget-mask_']").on('keydown click close',
      ...
      this.initializeWidget = function () {
        ...
        commentsCollectionModel.fetch({
          success: function() {
            ...
            options.blockingParentView && options.blockingParentView.unblockActions();
          },
        ...
        // ESOC-10239- to handle closing the popover when switching to doc overview,
        // when an attachment is clicked.
        commentsCollectionView.listenToOnce(options.context, 'change:perspective retain:perspective', function () {
          var widgetMaskElem = $("[id^='esoc-social-comment-widget-mask_']");
          widgetMaskElem.length && widgetMaskElem.trigger('close');
        });
        if (options.blockingParentView) {
          Marionette.triggerMethodOn(options.blockingParentView, 'popover:open', this);
        }
      };
