      ...
      var Utils = {
        ...
        onDeleteSuccess: function (args, data, status, jXHR) {
          ...
          case 'CommentDelete':
            ...
            // update the comments count in the node model - this triggers re-render of views
            // like toolbar comment command icon or table cell view
            commentListItem.options.commentConfigOptions.currentNodeModel.set('wnd_comments',
                totalComments,{'silent':true});
            commentListItem.options.commentConfigOptions.currentNodeModel.trigger('comment:icon:view');
        ...
        closeCommentDialog: function (event, callbackFun) {
          ...
          if (_e.type === 'popstate' || _e.type === 'hashchange' || _e.type === 'close') {
        ...
        setCommentDialogPointer: function () {
          ...
          if (this.widgetBaseElement.length === 0) {
            return;
          }

          if (this.widgetBaseElement.length > 1) {
            // if identified more than one base element, filter them based on visibility.
            var visibleIndex = 0;
            this.widgetBaseElement.each(function (ind) {
              var isVisible = $(this).is(':visible');
              visibleIndex = isVisible && visibleIndex === 0 ? ind : visibleIndex;
            });
            this.widgetBaseElement = visibleIndex > 0 ? $(this.widgetBaseElement[visibleIndex]) :
                                     this.widgetBaseElement;
          }
