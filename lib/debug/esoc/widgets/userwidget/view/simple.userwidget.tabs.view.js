csui.define([
  ...
  var SimpleUserWidgetViewTabs = Marionette.ItemView.extend({
    ...
    leaveOrCancelAlert: function(e, nextActiveTab){   
      var deferred    = $.Deferred();
      var self = this;
      require(['csui/dialogs/modal.alert/modal.alert'], function (alertDialog) {
        ...
        Lang.txtLeaveContentMgt,
            {
              ...
              if(self.settingTab){
                var tabElm =  self.settingTab.parentElement;
                if(tabElm){
                  $(tabElm).removeClass('binf-active')
                }
              }
              self.showTab(e);
        }, function () {
            // Cancel
            if(self.settingTab){
              self.settingTab.focus();
              var tabElm =  self.settingTab.parentElement;
              if(tabElm){
                if(nextActiveTab){
                  $(nextActiveTab.parentElement).removeClass('binf-active');
                }
                $(tabElm).addClass('binf-active');
              }
            }
            deferred.resolve();      
        });
    ...
    showTab: function (e) {
       // keeping track of setting tabs once only
       var hashTag, target, nextActiveTab = document.activeElement;
       if(!this.settingTab){
         if(e.target.hash){
           hashTag =  e.target.hash;
           target = e.target;
         }else if(e.target.parentElement && e.target.parentElement.hash){
           hashTag = e.target.parentElement.hash;
           target = e.target.parentElement;
         }
         if(hashTag === '#usersettings'){
           this.settingTab = target;
         }
       }
      if(this.options.context.cannotMoveOutOfSettingPage){
        e.preventDefault();
        this.leaveOrCancelAlert(e, nextActiveTab);
        return;
      }
