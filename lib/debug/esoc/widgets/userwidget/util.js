csui.define(['require',
    ...
    function (require, module, $, Marionette, _, ConnectorFactory, Lang, CommonUtil, ChatView,
        PresenceView,
        SkypeAttributeModel, DialogView, SidePanelView, NonEmptyingRegion, MultipleDialogsView, slick, Url,
        ChatFactory, PulseSettingsFactory) {
      var Utils = {
        lang: Lang,
        ...
        showUserProfileDialog: function (userwidgetview, UserProfileView, SimpleUserWidgetView) {
          if (userwidgetview.templateHelpers().messages.showUserProfileLink) {
            ...
            if (!userwidgetview.options.enableSimpleSettingsModel) {
              ...
              var loggedInUser = userwidgetview.options.context.getModel("user"),
                otherUser = userwidgetview.options.userid !== loggedInUser.id;
              if ((!!this.userProfileSidePanel && otherUser) || otherUser) {
                if (!!this.userDialog) {
                  ...
                  this.userProfileSidePanel && this.userProfileSidePanel.trigger('destroy');
                ...
                if (this.simpleUserWidgetView === undefined) {
                  // destroy any existing simple user profile when side panel is to be rendered
                  this.userDialog && this.userDialog.trigger('destroy:all');

                  userwidgetview.options.sidePanel = true;
                  this.simpleUserWidgetView = new SimpleUserWidgetView(userwidgetview.options);
                  var self = this;
                  this.simpleUserWidgetView.layoutState = {}
                  this.userProfileSidePanel = new SidePanelView({
                    slides: [{
                      content: this.simpleUserWidgetView
                    }],
                    sidePanelClassName: 'esoc-simple-user-widget-dialog',
                    layout: { header: false, footer: true, mask: true, resize: false },
                    thresholdWidth: 95,
                    scrollbleContent: '',
                    contentView: this.simpleUserWidgetView,
                    context: userwidgetview.options.context,
                    keyboard: false,
                    footerButtonLabel: Lang.sidePanelCloseButton,
                  });

                  this.userProfileSidePanel.show();
                  this.simpleUserWidgetView.listenToOnce(this.simpleUserWidgetView, 'before:destroy',
                    function () {
                      // destroy side panel when clicked on close icon on header.
                      self.userProfileSidePanel && self.userProfileSidePanel.trigger('destroy');
                      self.userProfileSidePanel = undefined;
                      self.simpleUserWidgetView = undefined;
                    });
                  this.userProfileSidePanel.footerView.$el.on('keydown', _.bind(function (event) {
                    self.simpleUserWidgetView.trigger("sidepanel:keydown", event);
                  }));

                  self.userProfileSidePanel.listenToOnce(self.userProfileSidePanel, 'before:destroy',
                    function () {
                      self.userProfileSidePanel = undefined;
                      // destroy simple user profile whe clicked on close button on side panel footer
                      self.simpleUserWidgetView && self.simpleUserWidgetView.trigger('destroy');
                      self.simpleUserWidgetView = undefined
                    });

                  self.userProfileSidePanel.listenTo(self.userProfileSidePanel, "after:show", function () {
                    // initial focus must be on 'X' icon
                    self.simpleUserWidgetView.ui.userProfileClose.focus();
                  });
                }
              }
            }
            ...
            userwidgetview.on('userwidgetview.show.user.dialog', function (event) {
              ...
              require(["esoc/controls/userwidget/userwidget.view"], function(UserWidgetView){
                ...
                var widgetOptions  = _.extend({}, self.options);
        ...
        displayUserWidget: function (context, el) {
          el.find(".esoc-widget-user-display-name").each(function () {
            ...
            var UserWidget = require("esoc/widgets/userwidget/userwidget");
