...
define([
  ...
  'css!esoc/widgets/userwidget/impl/userwidget.css'
     ...
     Util, UserWidgetTemplate, lang, ChatFactory) {
  var self           = null,
      userDialog     = null,
      UserWidgetView = Marionette.ItemView.extend({
        ...
        templateHelpers: function () {
          return {
            messages: {
              ...
              uniqueId: _.uniqueId(),
              userImgSrc: this.options.imgSrc ? this.options.imgSrc : "data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=",
        ...
        initialize: function (options) {
          ...
          this.options.model.attributes.otherUser = this.compareValues(loggedUserId, this.options.userid);
          self = this;
		...
		compareValues: function (loggedUserId, userid) {
          if (typeof loggedUserId === 'string' && typeof userid === 'string') {
            return loggedUserId !== userid;
          } else {
        	return parseInt(loggedUserId, 10) !== parseInt(userid, 10);  
          }
        },
        onRender: function (e) {
        ...
        showMiniProfilePopup: function (e, extraParameters) {
          if (this.model.attributes.type === 0) {
            ...
            var self = this;
            require(['esoc/widgets/userwidget/view/userprofile.view',
              'esoc/widgets/userwidget/view/miniprofile.view'],
                function(UserProfileView, MiniProfileView) {
              self.options.userprofileView = UserProfileView;
              var miniProfileView = new MiniProfileView(self.options);
        ...
        showUserProfileDialog: function (e) {
          if (this.model.attributes.type === 0) {
            if (e) {
              var typeaheadContainer = $(e.currentTarget).find('a');
              this.options.targetEle = typeaheadContainer.is(':visible') ? typeaheadContainer : e.currentTarget;
            }
            ...
            require(['esoc/widgets/userwidget/view/userprofile.view',
                ...
                function(UserProfileView, SimpleUserWidgetView) {
                  Util.showUserProfileDialog(self, UserProfileView, SimpleUserWidgetView);
                  var alreadyPushed = ((Util.userStack.length > 0 &&
                      Util.userStack[Util.userStack.length - 1].model.id ===
                      self.model.id) ? true : false);
                  if (!alreadyPushed) {
                    Util.userStack.push(self);
                  }
                });
          }
