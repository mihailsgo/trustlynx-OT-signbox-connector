        ...
        updateCommentCount: function (args) {
          ...
          if (commentCount !== undefined) {
            ...
            if (commentCount > 0) {
              ...
              commentSpan.find('.cs-icon-comment').addClass('csui-non-empty-comment-icon');
            } else {
              commentNumberSpan.html("");
              commentSpan.find('.cs-icon-comment').removeClass('csui-non-empty-comment-icon');
            }
          }
        },
                ...
                if (!userFactoriesId || !userFactoriesId.length) {
                  ...
                  userFactories.options[userid].push(function (photo) {
                    if (photo) {
                      ...
                      dfdPhoto.reject(photoElement, defaultPhotElement);
                    }
                  });
        ...
        updateSelection: function (event, view, radioBtnGroupName) {
          if (event.target.type === "checkbox") {
            ...
            var currentTargetId = event.target.id,
                radioSectionSelector = $(event.target).attr('name');
            view.$el.find('input[type="radio"][name='+radioSectionSelector+']').each(function() {
              var id = $(this).attr('id');
