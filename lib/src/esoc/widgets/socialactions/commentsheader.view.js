...
define([
  ...
  var CommentsHeaderView = Marionette.ItemView.extend({
    initialize: function (options) {
      ...
      this.wrapperElement = $(this.currentTarget).parent();
    },
    options: {},
    _elements: {
    ...
    onCommentFormBlur: function (e) {
      if (!this.$el.find(".esoc-social-comment-form-mask").is(':visible')) {
        ...
        if ((commentString === undefined || commentString === '') &&
          ...
          $("#esoc-social-comment-header").removeClass("emoji-wysiwyg-editor-focus").trigger(
              'blur');
          if (this.util.commonUtil.isIE() && this.$el.find(".esoc-header-emoji").length > 0 &&
              this.$el.find(".esoc-header-emoji")[0].textContent.length === 0) {
    ...
    onCommentSubmitSuccess: function (model, response/*, options*/) {
      ...
      this.model && this.model.currentNodeModel && this.model.currentNodeModel.set({'wnd_comments' : totalComments}, {'silent' : true});
      this.model && this.model.currentNodeModel && this.model.currentNodeModel.trigger('comment:icon:view');
    ...
    findFocusElement: function (currentTarget) {
      var focusElement = $(currentTarget).find('.csui-acc-focusable');
      return focusElement.length ? focusElement : currentTarget;
    },

    getCurrentObject: function() {
      var currentTarget = $(this.findFocusElement(this.currentTarget));
      if (!currentTarget.is(':visible')) {
        this.currentTarget = this.wrapperElement.find('.csui-acc-focusable');
    ...
    onCloseButton: function (e) {
      ...
      var element = this.getCurrentObject();
      if (element) {
