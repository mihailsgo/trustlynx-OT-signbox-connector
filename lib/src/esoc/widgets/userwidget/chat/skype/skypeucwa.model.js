...
define([
...
], function (_require, $, Backbone, Base, ConnectorFactory, ChatUtil, Cache, Transport, Batch, Events, SkypeModel,
  ...
  var SkypeUCWAModel = SkypeModel.extend({
    ...
    showUserPresenceForTeams: function (options) {
      var connector = options.context.getObject(ConnectorFactory);
      var url = connector.getConnectionUrl().getApiBase('v2') + '/pulse/statuses/getuserpresence/' + options.username;
      $.ajax(connector.extendAjaxOptions({
        ...
        success: function (response) {
          var presence = { 'status': response.results.userPresence.availability }
          ChatUtil.updatePresenceModel(options, presence);
        },
        error: function (xhr) {
