      ...
      return {
        ...
        cancelAllAsync: function () {
          if (pendingTimers.length > 0) {
            console.error(
                "Total " + pendingTimers.length + " was running. Tests needs to be fixed.");
          }
        ...
        waitFor: function (check, message, timeout) {
          ...
          if (check()) {
            ...
            var state = {
              ...
              error: new Error(_.str.sformat("Timeout {0} reached while waiting for {1}.", timeout,
                  message))
            };
          }
          return deferred.promise();
        ...
        findHidingElement: function (element, root) {
          ...
          while (parentElement) {
            ...
            var overflowX = parentStyle["overflow-x"] !== "clip" &&
                            parentStyle["overflow-x"] !== "hidden";
            var overflowY = parentStyle["overflow-y"] !== "clip" &&
                            parentStyle["overflow-y"] !== "hidden";
            var visibleX = overflowX || parentRect.width && parentRect.left <= middleX &&
                                        middleX <= parentRect.right;
            var visibleY = overflowY || parentRect.height && parentRect.top <= middleY &&
                                        middleY <= parentRect.bottom;
