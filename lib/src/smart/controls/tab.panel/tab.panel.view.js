...
define(['nuc/lib/underscore', 'nuc/lib/backbone', 'nuc/lib/marionette',
  ...
  'css!smart/controls/tab.panel/impl/tab.panel',
  'smart/lib/binf/js/binf'
], function (_, Backbone, Marionette, TabLinkCollectionView,
    TabContentCollectionView, ViewEventsPropagationMixin,
    TabableRegionBehavior, TabPanelKeyboardBehavior, NonEmptyingRegion) {
  'use strict';
    ...
    _scrollToActiveTab: function (tabContent, tabPane, tabLink) {
      ...
      if (this.options.mode) {
        ...
        if (hrefElems.length > 0) {
          ...
          this.trigger('tab:contents:header:view:change:tab:title', tabLink);
          var tablinkInfo ={
            tablink : tabLink
          };
          this.trigger("tablink:click",tablinkInfo);
        }
      }
    },
