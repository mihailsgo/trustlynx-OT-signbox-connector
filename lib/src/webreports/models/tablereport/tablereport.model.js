...
define(['module', 'csui/lib/underscore', 'csui/lib/backbone', 'webreports/utils/url.webreports', 'webreports/utils/table.validate',
    ...
    var TableReportCollection = Backbone.Collection.extend({
        ...
        _reservedParams: function(){
            ...
            if (options && options.data && options.data.columnsWithSearch){
                reservedParms = _.union(reservedParms, _.map(options.data.columnsWithSearch, function(columnWithSearch){
                    return "where_" + columnWithSearch.toLowerCase();
                }));
            }
            return reservedParms;
        },
        ...
        url: function () {
            var query = '',
                ...
                reservedParams = this._reservedParams();

            query = UrlWebReports.combineQueryString(
                query,
                this.getBrowsableUrlQuery()
            );
            parameters = _.reject(parameters, function(parameter){
                return _.contains(reservedParams, parameter.name.toLowerCase());
            });
        ...
        parse: function (response) {
            ...
            this.columns.set(this._processColumns(response));
        ...
        _processColumns: function (response) {
            ...
            if (options && options.data && options.data.columnsWithSearch){
                columnsWithSearch = _.map(options.data.columnsWithSearch, function(columnWithSearch){
                    return columnWithSearch.toLowerCase();
                });
            }
            if (this.orderBy === "") {
            ...
            return _.map(response.columns, _.bind(function (column) {
                ...
                if (_.contains(columnsWithSearch, column.column_key.toLowerCase())){
                    column.widthFactor = 2;
                }
