...
define(['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/marionette',
  ...
  describe('NodePicker -', function () {
    ...
    xdescribe('shortcut selectable and browsable', function () {
      ...
      it('browse the folder checkbox should be selected', function() {
        $el.find('.binf-list-group li:first-child  span.csui-browsable').trigger('click');
        expect($el.find('.binf-list-group li:first-child span.icon-checkbox-selected').length).toBe(1);
      });
    ...
    xdescribe('Multiselect functionality in target browse', function () {
      var nodePicker, collection, context, container, connector, node, $el;
      ...
      it('Removing a selected node from select list dropdown', function () {
        ...
        listItemToBeRemoved.find('.csui-deselected-icon').trigger('click');
        expect($el.find(
