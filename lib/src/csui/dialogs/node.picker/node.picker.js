  ...
  _.extend(NodePicker.prototype, {
    ...
    _createHeaderControl: function (initialLocation) {
      ...
      var headerView = new HeaderView({
        ...
        disableSearchFromHere: options.disableSearchFromHere
      });

      headerView.startLocationView && headerView.startLocationView.on('change:location', this.onChangeLocation, this);
    ...
    onClickSelectButton: function () {
      if (this && this._view && (this._view.getSelection().length > 0)) {
        ...
        if (this._headerControl.ui && this._headerControl.ui.saveFilter &&
          ...
          multiLingualData = this._headerControl.model && this._headerControl.model.get('name_multilingual');
        } else {
          return;
        }
        this._result = {
    ...
    onDrillUp: function (nodeId) {
      ...
      var searchLocationModel = this._locations.get('csui/dialogs/node.picker/start.locations/search.location');
      if(searchLocationModel) {
        searchLocationModel.get('factory').options.container = this.options.container;
      }
    },
