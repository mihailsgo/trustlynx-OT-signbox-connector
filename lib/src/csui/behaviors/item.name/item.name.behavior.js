  ...
  var ItemNameBehavior = Marionette.Behavior.extend({
    ...
    constructor: function ItemNameBehavior(options, view) {
      ...
      this._nameSchema = _.extend({
        required: true,
        readonly: false
      }, this.options.nameSchema, view.options.nameSchema);
      this.view = view;
      this.view.ItemNameBehavior = this;
      this.view.readonly = this._nameSchema.readonly; // TODO: use this._nameSchema.readonly only
    ...
    onKeyInView: function (e) {
      ...
      if (event.keyCode === 9 && this.getInputBoxValue() === "" && this.autoSave) {
        ...
        return;
      } else if (event.keyCode===9 && this.view.editing) {
        this.view.tabableElements && this.view._moveTab(event);
        return;
      }
    ...
    showInlineError: function (errorText) {
      this.view.ui.titleError.text(errorText);
      ...
      this.view.ui.nameInput[0].setAttribute("aria-invalid", true);
      this.view.ui.saveButtonInEditMode.prop('disabled', true);
    },
    ...
    clearInlineError: function () {
      ...
      this.view.ui.nameInput[0].setAttribute("aria-invalid", false);
    },
    ...
    _toggleEditMode: function (edit, setFocus) {
      ...
      var name_multilingual = this.view.model.get("name_multilingual");
      edit && name_multilingual && (this.view.prevMultilingualVal = _.clone(name_multilingual));
      if (edit) {
        if (this._isEditingEnabled()) {
          ...
          if (!this.autoSave || !!this.view.$el.find('.csui-multilingual-icon').length) {
            this.view.refreshTabableElements && this.view.refreshTabableElements();
          }
          ...
          this.view && _.isFunction(this.view._openMLFlyoutInEditMode) && this.view._openMLFlyoutInEditMode();
        }
        ...
        if (this.view.$el.is(":visible")) {
          ...
          this.view.nameEditDivIgnoreBlurOnce = true;
    ...
    isSaveEnabled: function (newName) {
      if (!newName.length || (newName.trim() === this.view.model.get('name'))) {
        return false;
      } else {
        return true;
      }
    ...
    onKeyReleased: function (event) {
      ...
      if (!(!!this.view.options && this.view.options.mode === 'create')) {
        ...
        if (event.keyCode === 27) {
          this.cancelNameEdit(event);
        } else if (event.keyCode === 13 && this.isSaveEnabled(event.target.value)) {
    ...
    saveInput: function (event) {
      ...
      if (this.view.nameEditDivIgnoreBlurOnce === true && this.autoSave) {
        this.view.nameEditDivIgnoreBlurOnce = false;
        return;
      }
    ...
    saveInputOnFocusOut: function (event) {
      if (this.view.readonly || !this.autoSave) {
        return;
      }
    ...
    cancelNameEdit: function (event) {
      ...
      this.clearInlineError();
      this.view.enableAddButton && this.view.ui.saveButtonInEditMode.prop('disabled', true);
      ...
      this.view.prevMultilingualVal &&  this.view.model.set("name_multilingual", this.view.prevMultilingualVal);
    },
