...
Feature('Perspective Manage Mock Test cases');

BeforeSuite((I) => {
    I.loadHTML("lib/src/csui/perspective.manage/test/index.html");
    I.waitMaxTimeForElement(".pman-container");
    I.seeElement(".pman-container");
});
...
Scenario('Open and verify standard widget modules accordion', async (PM) => {
    expect(await PM.showAccordionListOfModules('"CSUI"')).to.be.true;
});

Scenario('Drag and drop a widget from standard widgets into Perspective view', (PM) => {
...
Scenario.skip('Execute Performance tests - Perspective Manage', async (I) => {
    I.loadHTML("lib/src/csui/perspective.manage/test/index.html");
