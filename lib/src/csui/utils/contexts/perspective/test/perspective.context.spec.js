...
define([
  ...
  './perspective.context.mock.js'
    ...
    NextNodeModelFactory, Factory, PerspectiveContextPlugin, ContextPlugin,
    CsuiContextPlugin, mock) {
  'use strict';

  describe('PerspectiveContext', function () {
    var EmptyContextPlugin, TestObjectFactory, factories;
    var perspectiveContext, fetchSpy, applicationScope, nextNode;

    beforeAll(function () {
      EmptyContextPlugin = ContextPlugin.extend({});

      TestObjectFactory = Factory.extend({
        propertyPrefix: 'test',
    ...
    it('processes the csui plugin before the others', function () {
      var viewStateModel;
      var TestPerspectiveContextPlugin = PerspectiveContextPlugin.extend({
        constructor: function TestPerspectiveContextPlugin(options) {
          PerspectiveContextPlugin.call(this, options);
          viewStateModel = this.context.viewStateModel;
        }
      });
      viewStateModel = null;
      perspectiveContext = new PerspectiveContext({
        plugins: [CsuiContextPlugin, TestPerspectiveContextPlugin]
      });
      expect(viewStateModel).toBeTruthy();
      viewStateModel = null;
      perspectiveContext = new PerspectiveContext({
        plugins: [TestPerspectiveContextPlugin, CsuiContextPlugin]
      });
      expect(viewStateModel).toBeTruthy();
    });
    ...
    describe('with a perspective context plugin', function () {
      ...
      var TestPerspectiveContextPlugin, testPlugin;

      beforeAll(function () {
        TestPerspectiveContextPlugin = PerspectiveContextPlugin.extend({
      ...
      it('survives an empty context plugin as the previous plugin in chain', function (done) {
        var Empty2ContextPlugin = ContextPlugin.extend({});
        perspectiveContext = new PerspectiveContext({
          plugins: [TestPerspectiveContextPlugin, EmptyContextPlugin],
          factories: factories
        });
        spyOnContext();
        perspectiveContext.once('change:perspective', function () {
      ...
      function loadAndChangePerspective(done) {
        var perspectiveRequested, perspectiveSynced;
        perspectiveContext
          .once('request:perspective', function () {
            perspectiveRequested = true;
      ...
      function forceChangePerspective () {
        var onApply = testPlugin.onApply;
        testPlugin.onApply = function () {
          onApply.apply(this, arguments);
          return $.Deferred().resolve({ forceChange: true }).promise();
        };
      }

      it('can enforce perspective change from onApply', loadAndChangePerspective);
