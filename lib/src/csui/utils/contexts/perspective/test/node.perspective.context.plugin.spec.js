...
define([
  ...
  'csui/utils/contexts/factories/global.error','csui/utils/base',
  './perspective.context.mock.js'
], function (_, PerspectiveContext, ApplicationScopeModelFactory,
    NextNodeModelFactory, PreviousNodeModelFactory, NodeModelFactory,
    NodeChildrenCollectionFactory, GlobalErrorModelFactory, base, mock) {
  'use strict';
  ...
  describe('NodePerspectiveContextPlugin', function () {
    ...
    it('delays node-change events to the children update', function (done) {
      perspectiveContext.once('change:perspective', function () {
        var triggered = 0;
        perspectiveContext
        ...
        perspectiveContext.getModel(NodeChildrenCollectionFactory);
        nextNode.set('id', 60);
      });
      nextNode.set('id', 50);
    });
    ...
    it('failing to fetch the contextual node leads to a rejection of the perspective navigation with the right parameters', function (done) {
      var globalError = perspectiveContext.getModel(GlobalErrorModelFactory);
      globalError.once('change', function () {
        var changed = globalError.changedAttributes();
        expect(typeof changed.message).toBe('string');
        expect(typeof changed.details).toBe('string');
        done();
      });
      perspectiveContext.once('error:perspective', function (plugin, error) {
        var wrappedError = new base.Error(error);
        expect(typeof wrappedError.message).toBe('string');
        expect(typeof wrappedError.errorDetails).toBe('string');
      });
      nextNode.set('id', 100);
    });
  });
