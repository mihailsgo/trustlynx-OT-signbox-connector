  ...
  init: function (editor) {
    editor.addCommand('cslink', {
      exec: function (editor) {
        var toolBar = new CKEDITOR.dom.element( document.getElementsByClassName( 'csui-rich-text-editor-toolbar' )[0]);
        toolBar.addClass('binf-disabled');
        ...
        editor.config.csLink.nodePicker().show().done(
          ...
          toolBar.removeClass('binf-disabled');
        });
      }
    });
