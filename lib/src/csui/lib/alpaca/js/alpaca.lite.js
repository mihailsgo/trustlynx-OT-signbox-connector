...
//
// * Use csui module names in the UMD wrapper
// * Improve performance of AbstractTemplateEngine

// [OT] Use csui module names in the UMD wrapper
define([
    'csui/lib/jquery', 'csui/lib/handlebars', 'csui/lib/moment',
    'i18n!csui/lib/alpaca/nls/lang', 'csui/lib/binf/js/binf'
], function ($, Handlebars, moment, lang) {
...
this["HandlebarsPrecompiled"]["web-edit"]["control-radio"] = Handlebars.template({"1":function(depth0,helpers,partials,data) {
  ...
  return "<div class=\"binf-radio\"><label><input type=\"radio\" "
            ...
            if (field.options.showMessages)
            {
                if (!field.initializing)
                {
                    ...
                    for (var messageId in field.validation)
                    {
                        ...
                        {
                            // we don't markup invalidation state for readonly fields.
                            // but at least we log it to the console to help the developer.
                            if (!field.options.readonly)
                            {
                                messages.push({
                                    "id": messageId,
                                ...
                                Alpaca.logWarn("Validation for field (id=" + field.getId() + ", title=" + field.getTitle() + ", path=" + field.path + "): "+field.validation[messageId]["message"]);
    ...
    Alpaca.AbstractTemplateEngine = Base.extend(
    {
        constructor: function(id)
        {
            ...
            this.cleanup = function(html)
            {
                if (html)
                {
                    // [OT] Replace multiple creations of a jQuery object to check
                    // and strip the template off the wrapper script using pure
                    // JavaScript. Reusing the same jQuery object cuts the execution
                    // time to 50%, pure JavaScript to 30% of the original time.
                    var wrapper = document.createElement("div");
                    wrapper.innerHTML = html;
                    // if it starts with a script tag, then we strip that out
     ...
     */
    {
        ...
        postRender: function(callback)
        {
            ...
            if (this.view.type !== 'view') {
              ...
              var doDisableField = function (setDisableProp) {
                ...
                $(":radio", self.field).off().on('click', function (e) {
                  e.preventDefault();
                  e.stopImmediatePropagation();
                  return false;
                });
                $(".binf-radio label", self.field).off().on('click', function (e) {
                  e.preventDefault();
                  e.stopImmediatePropagation();
                  return false;
                });
                ...
                $(":input", self.field).off().on('click', function (e) {
                  e.preventDefault();
                  e.stopImmediatePropagation();
                  return false;
                });
            ...
            {
                this.field.addClass("binf-hidden");
            }
            ...
            {
                $(this.field[0].querySelectorAll(".alpaca-readonly")).addClass("binf-hidden");
            }
      ...
      refreshContainer: function(defaultData, callback)
      {
        ...
        show: function()
        {
            ...
            {
                // [OT]: for sets, show the complete alpaca container item along with current field.
                var elementToBeShow = $(this.field);
                if (!elementToBeShow.hasClass('alpaca-container-item')) {
                  var closestWrapper = elementToBeShow.closest('.alpaca-container-item');
                  if (closestWrapper.length && closestWrapper[0].classList.contains("binf-hidden")) {
                    // closestWrapper.css({
                    //   'display': ''
                    // });
                    closestWrapper[0].classList.remove("binf-hidden");
                  }
                }
                // show the field
                // elementToBeShow.css({
                //   'display': ''
                // });
                elementToBeShow[0] && elementToBeShow[0].classList.remove("binf-hidden");

                if (this.standaloneHelper) {
                    // this.standaloneHelper.css({
                    //     "display": ""
                    // });
                    this.standaloneHelper[0].classList && this.standaloneHelper[0].classList.remove("binf-hidden");
                }
        ...
        hide: function()
        {
            ...
            //     'display': 'none'
            // });
            elementToBeShow[0] && elementToBeShow[0].classList.add("binf-hidden");

            // [OT]: for sets, hide the complete alpaca container item along with current field.
            if (!elementToBeShow.hasClass('alpaca-container-item')) {
              var closestWrapper = elementToBeShow.closest('.alpaca-container-item');
              if (closestWrapper.length) {
                closestWrapper[0].classList && closestWrapper[0].classList.add("binf-hidden");
                // closestWrapper.css({
                //   'display': 'none'
                // });
              }
            ...
            if (this.standaloneHelper) {
                ...
                //     "display": "none"
                // });
                this.standaloneHelper[0] && this.standaloneHelper[0].classList.add("binf-hidden");
            }
        ...
        isHidden: function() {
            return ("none" === $(this.field).css("display") || (this.field[0] && this.field[0].classList.contains("binf-hidden")));
        },
        ...
        initEvents: function()
        {
            ...
            if (this.field)
            {
                this.field.on('mouseover', function(e) {
                    _this.onMouseOver.call(_this, e);
                    _this.trigger("mouseover", e);
                });

                this.field.on('mouseout', function(e) {
...
(function($) {
     ...
     */
    {
        ...
        renderFieldElements: function(callback) {
            ...
            this.control = $(this.field[0].querySelectorAll("." + Alpaca.MARKER_CLASS_CONTROL_FIELD));
...
(function($) {
     ...
     */
    {
        ...
        renderFieldElements: function(callback) {
            ...
            // find our insertion point
            // this is marked by the handlebars helper
            this.container = $(this.field[0].querySelectorAll("." + Alpaca.MARKER_CLASS_CONTAINER_FIELD));
        ...
        applyCreatedItems: function(model, callback)
        {
            ...
            for (var i = 0; i < model.items.length; i++)
            {
                ...
                var innerSelector = "." + Alpaca.MARKER_CLASS_CONTAINER_FIELD_ITEM + "[" + Alpaca.MARKER_DATA_CONTAINER_FIELD_ITEM_KEY + "='" + item.name + "']";
                var insertionPoint = $(findElementFromCollection(self.container, innerSelector));
               if (!layoutBindings)
                {
                ...
                item.containerItemEl[0].classList.add("alpaca-container-item");

                if (i === 0)
                {
                    item.containerItemEl[0].classList.add("alpaca-container-item-first")
                }

                if (i + 1 === model.items.length)
                {
                    item.containerItemEl[0].classList.add("alpaca-container-item-last")
                }

                item.containerItemEl[0].setAttribute("data-alpaca-container-item-index", i);
                item.containerItemEl[0].setAttribute("data-alpaca-container-item-name", item.name);
                item.containerItemEl[0].setAttribute("data-alpaca-container-item-parent-field-id", self.getId());
        ...
        updateChildDOMElements: function()
        {
            ...
            if (!layoutBindings)
            {
                ...
                {
                    var child = self.children[i], childPrevName = child.name;
                  ...
                  $(child.containerItemEl).attr("data-alpaca-container-item-parent-field-id",
                      self.getId());
                  // Child field names are not updated when a SET row is added/deleted.LPAD-89818.    
                  $(child.containerItemEl).find(
                      '[data-alpaca-container-item-name ^="' + childPrevName + '"]').each(
                      function () {
                        var itemName = $(this).attr('data-alpaca-container-item-name').replace(
                            childPrevName, child.name);
                        $(this).attr('data-alpaca-container-item-name', itemName);
                      });
                  $(child.containerItemEl).find('[data-name ^="' + childPrevName + '"]').each(
                      function () {
                        var dataName = $(this).attr('data-name').replace(childPrevName, child.name);
                        $(this).attr('data-name', dataName);
                      });
                  var scrollableName = 'csui-scrollable-' + childPrevName + ' csui-scrollable-' +
                                       child.name;
                  $(child.containerItemEl).find('.csui-scrollable-' + childPrevName).toggleClass(
                      scrollableName);
                }
            }
        },
        ...
        focus: function(onFocusCallback)
        {
            ...
            if (index > -1)
            {
                this.children[index].trigger('focus');
...
(function($) {
     ...
     */
    {
        ...
        processRender: function(parentEl, callback)
        {
            ...
            // find our insertion point
            // this is marked by the handlebars helper
            this.formFieldsContainer = $(this.form[0].querySelectorAll("." + Alpaca.MARKER_CLASS_FORM_ITEMS_FIELD));
            ...
            this.buttons = {};
            $(parentEl[0].querySelectorAll(".alpaca-form-button")).each(function() {
                thisElem = $(this);
                thisElem.on('click', function(e) {
                    thisElem.attr("button-pushed", true);
                });

                // custom click handler?
                var key = thisElem.attr("data-key");
                if (key)
                {
                    ...
                    if (buttonConfig)
                    {
                        ...
                        {
                            thisElem.on('click', function(form, handler) {
                                return function(e) {
                                    e.preventDefault();
                                    handler.call(form, e);
                                }
        ...
        initEvents: function()
        {
            ...
            var formTag = $(this.domEl[0].querySelectorAll("form"));

            var v = this.getValue();
            $(formTag).on('submit', v, function(e) {
            ...
            {
                $(_this.topControl.getFieldEl()).on("fieldupdate", function() {
        ...
        {
            return $(this.domEl[0].querySelectorAll(".alpaca-form-button-" + buttonId));
        },
        ...
        focus: function(onFocusCallback)
        {
            this.topControl.on('focus', function(controlWithFocus) {
     ...
     */
    {
        ...
        _validatePattern: function()
        {
            if (this.schema.pattern) {
              ...
              if (this.fieldView && this.fieldView.alpacaField.type === 'url') {
                var ptrn = /^((http||https):\/\/)/;
     ...
     */
    {
        ...
        postRender: function(callback) {
            ...
            this.base(function() {
                ...
                //
                if(self.getFieldEl()[0]) {
                    $(self.getFieldEl()[0].querySelectorAll("input")).on('change', function(evt) {
                        self.triggerWithPropagation("change");
                    });
    
                    $(self.getFieldEl()[0].querySelectorAll("checkbox")).on('change', function(evt) {
                        self.triggerWithPropagation("change");
                    });
                }
     ...
     */
    {
            ...
            if (self._validateEqualMaxItems())
            {
                ...
                formEl.alpaca({
                    ...
                    "postRender": function(control)
                    {
                        ...
                        // find the insertion point
                        var insertionPointEl = $(findElementFromCollection(containerItemEl, "." + Alpaca.MARKER_CLASS_CONTAINER_FIELD_ITEM_FIELD));
        ...
        */
        ...
        updateToolbars: function()
        {
            ...
            {
                // hide toolbar
                $(toolbarEl).addClass("binf-hidden");
            }
            else
            {
                // show toolbar
                $(toolbarEl).removeClass("binf-hidden");

                // CLICK: array toolbar buttons
                $(findElementFromCollection(toolbarEl, "[data-alpaca-array-toolbar-action]")).each(function () {
                    var thisElem = $(this);

                    var actionKey = thisElem.attr("data-alpaca-array-toolbar-action");
                    var action = self.findAction(self.toolbar.actions, actionKey);
                    if (action) {
                        thisElem.off().on('click', function (e) {
                            e.preventDefault();
            ...
            if (typeof(this.options.toolbarSticky) === "undefined")
            {
                ...
                $(items).each(function(itemIndex) {
                    ...
                    {
                        $(this).hover(function() {
                            $(actionbarEl).removeClass("binf-hidden");
                        }, function() {
                            $(actionbarEl).addClass("binf-hidden");
                        });
            ...
            else if (this.options.toolbarSticky)
            {
                ...
                $(self.getFieldEl()).find(".alpaca-array-actionbar[data-alpaca-array-actionbar-parent-field-id='" + self.getId() +  "']").removeClass("binf-hidden");
            }
            else if (!this.options.toolbarSticky)
            {
                // always hide the actionbars
                $(self.getFieldEl()).find(".alpaca-array-actionbar[data-alpaca-array-actionbar-parent-field-id='" + self.getId() +  "']").addClass("binf-hidden");
            }
            ...
            $(actionbarEls).each(function() {
                ...
                $(this.querySelectorAll("[data-alpaca-array-actionbar-action]")).each(function () {
                    var thisElem = $(this);
                    var actionKey = thisElem.attr("data-alpaca-array-actionbar-action");
                ...
                if (self._validateEqualMaxItems())
                {
                    $(this.querySelectorAll("[data-alpaca-array-actionbar-action='add']")).each(function(index) {
                ...
                {
                    $(this.querySelectorAll("[data-alpaca-array-actionbar-action='remove']")).each(function(index) {
                        $(this).removeClass('alpaca-button-disabled');
                        self.fireCallback("enableButton", this);
                    });
                }
     ...
     */
    {
            ...
            formEl.alpaca({
                ...
                "postRender": function(control) {
                  ...
                  var helperControl     = $(containerItemEl).find('.alpaca-field > .binf-help-block'),
                      helperControlHtml = "";
                  if (helperControl.length) {
                    //create helpertext id for individual field
                    var id = helperControl.attr("id");
                    if (!id) {
                      id = Alpaca.generateId();
                      helperControl.attr("id", id);
                      if (control.options) {
                        control.options.descriptionId = id;
                      }
                      if (helperControl.hasClass('binf-help-block-standalone')) {
                        ...
                        control.standaloneHelper = $(helperControlHtml);
                        containerItemEl.append(control.standaloneHelper);
                      }
                    }
                  }
                  control.containerItemEl = containerItemEl;
        ...
        bindDependencyFieldUpdateEvent: function(propertyId)
        {
            ...
            var bindEvent = function(propertyId, dependencyPropertyId)
            {
                ...
                if (dependentField)
                {
                    dependentField.getFieldEl().on("fieldupdate", (function(propertyField, dependencyField, propertyId, dependencyPropertyId) {
...
(function($) {
    ...
    callbacks["required"] = function()
    {
        ...
        var label = $(fieldEl[0].querySelectorAll("label.alpaca-control-label"));
    ...
    callbacks["control"] = function()
    {
        ...
        $(fieldEl[0].querySelectorAll("input")).addClass("binf-form-control");
        $(fieldEl[0].querySelectorAll("textarea")).addClass("binf-form-control");
        $(fieldEl[0].querySelectorAll("select")).addClass("binf-form-control");
        // except for the following
        $(fieldEl[0].querySelectorAll("input[type=checkbox]")).removeClass("binf-form-control");
        $(fieldEl[0].querySelectorAll("input[type=file]")).removeClass("binf-form-control");
        $(fieldEl[0].querySelectorAll("input[type=radio]")).removeClass("binf-form-control");
        ...
        $(fieldEl[0].querySelectorAll("input[type=checkbox]")).parent().parent().addClass("binf-checkbox");
        // any radio inputs get the "radio" class on their radio
        $(fieldEl[0].querySelectorAll("input[type=radio]")).parent().parent().addClass("binf-radio");
        ...
        $(fieldEl[0].querySelectorAll("label.alpaca-control-label")).addClass("binf-control-label");

        // if in horizontal mode, add a wrapper div (binf-col-sm-9) and label gets (binf-col-sm-3)
        if (this.view.horizontal)
        {
            $(fieldEl[0].querySelectorAll("label.alpaca-control-label")).addClass("binf-col-sm-3");
            ...
            if($(fieldEl).find('.binf-help-block').length) {
                $(fieldEl[0].querySelectorAll('.binf-help-block')).addClass('binf-help-block-standalone');
            }
        }
    };
    callbacks["container"] = function()
    ...
    callbacks["collapsible"] = function()
    {
        ...
        if ($(anchorEl).length > 0)
        {
            ...
            $(anchorEl).on('mouseover', function(e) {
...
// Helper function to find element using plain javacript when outer selector is a collection of elements
function findElementFromCollection(outerElements, innerSelector) {
    var elementsCollection = [];
    if (outerElements && outerElements.length > 0) {
        for (var i = 0; i < outerElements.length; i++) {
            if (outerElements[i] instanceof HTMLElement) {
                var elements_in_outer = outerElements[i].querySelectorAll(innerSelector);
                ...
                elementsCollection = elementsCollection.concat(elements_in_outer);
            }
        }
    }
