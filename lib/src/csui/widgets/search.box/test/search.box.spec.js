...
define(['csui/lib/marionette', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/widgets/search.box/search.box.view',
  ...
  describe('SearchBoxView', function () {
    ...
    describe('given empty configuration', function () {
      ...
      xit('should focus on downcaret', function () {
      ...
      it('should clear on clearerClicked', function () {
        ...
        expect(searchBoxView.ui.searchIcon.removeClass(
            searchBoxView.options.data.customSearchIconCloseClass).addClass(
      ...
      xit('should show downCaret', function () {
      ...
      it('should hide options dropdown data on pressing escape', function () {
        searchBoxView.ui.searchIcon.trigger({type: 'keydown', keyCode: 13});
        searchBoxView.ui.input.trigger({type: 'keydown', keyCode: 27});
    ...
    describe('given configuration', function() {
      ...
      xit('should focusout on slice popover', function() {
      ...
      xit('show/hide slice popover', function() {    
    ...
    describe('slices dropdown functionality', function () {
      beforeAll(function (done) {
        searchBoxView = new SearchBoxView({
          context: pageContext,
          data: {
        ...
        searchBoxView.$el.addClass('csui-search-expanded');
        pageContext.fetch().done(done);
      });
      ...
      it('verify search options dropdown on clicking on input field', function () {
        searchBoxView.ui.input.trigger('click');
        expect(searchBoxView.ui.searchIcon.css('display') === 'block' ||
          !searchBoxView.ui.searchIcon.hasClass('binf-hidden') ||
          searchBoxView.ui.searchIcon.hasClass('binf-show')).toBeTruthy();
        expect(searchBoxView.$el.find('.csui-search-options-dropdown').length).toEqual(1);
      ...
      it('verify slice selection', function (done) {
        var slice = $(searchBoxView.searchSlicesListView.$el.find('.csui-search-popover-row-body')[0]);
        slice.trigger('click');
        TestUtils.asyncElement(slice,
          ".icon-listview-checkmark").done(
            function ($el) {
      ...
      it('verify slice unselection', function (done) {
        var slice = $(searchBoxView.searchSlicesListView.$el.find('.csui-search-popover-row-body')[0]);
        slice.trigger('click');
        TestUtils.asyncElement(slice,
          ".csui-search-popover-checked").done(
            function ($el) {
              expect($el.hasClass('icon-listview-checkmark')).toBeFalsy();
              done();
            });
      });

      it('verify show more button functionality', function () {
        var collectionLength = searchBoxView.searchSlicesListView.$el.find('.csui-search-popover-row').length,
            showMore = $(searchBoxView.$el.find('.csui-slices-more'));
        showMore.trigger('click');
        expect(collectionLength < searchBoxView.$el.find('.csui-search-popover-row').length).toBeTruthy();
      });
    });
