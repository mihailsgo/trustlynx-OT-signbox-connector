...
define([
  ...
  "../../../utils/testutils/async.test.utils.js"
    ...
    MetadataMock, AuditMock, MetadataPropertiesView, TestUtils) {
  'use strict';

  xdescribe("MetaDataView Widget", function () {
    ...
    describe("tab links", function () {
      ...
      it('tab links present', function (done) {
        expect(v1.$el.find('.cs-tab-links').length).toEqual(1);
        done();
      });

      it('alphabetical order of categories', function (done) {
      ...
      it('highlighting tab link on clicking', function (done) {
      ...
      it('shows loading icons and add-properties menu', function (done) {
        ...
        TestUtils.asyncElement(v1.$el, '[id^="addPropertiesButton"]:visible').done(
            function (el) {
              ...
              TestUtils.asyncElement($.fn.binf_modal.getDefaultContainer(),
                  '.binf-modal-dialog .binf-modal-content').done(function (el) {
                expect(el.length).toEqual(1);
                expect(el.find('.csui-title').text()).toBe("Select a Category to add");
      ...
      it('check system attributes', function(done) {
        var systemAttributes = v1.$el.find('.systemattributes_section')[0];
        expect(systemAttributes.children.length).toEqual(2);
        done();
      });
    ...
    describe('Name Bhevaiours', function () {
      ...
      it('Cancel Name Input Box', function () {
        var cancelName = metadataItemNameView.$el.find('.csui-undo.csui-edit-cancel');
    ...
    describe('AuditView', function () {
      it('Click on audit tab', function (done) {
        ...
        clickMenuItem(dropdownEle, 'audit');
      ...
      it('Close the user search box', function (done) {
        $('.csui-audit .csui-audit-table .csui-nodetable th[data-csui-attribute = user_id] .csui-icon-v2__csui_action_close32')
            .trigger('click');
      ...
      it('Click on action search hide button', function (done) {
        var hideSearchIcon = $('.csui-audit .csui-audit-table .csui-nodetable th' +
                               ' .csui-table-search-icon .csui-icon-v2__csui_action_close32');
        expect(hideSearchIcon.length).toBe(1);
        hideSearchIcon.trigger('click');
    ...
    describe('VersionsTable', function () {
      ...
      it('Click on versions tab', function (done) {
        var dropdownEle = $('.cs-tab-links.binf-dropdown:first');
        dropdownEle.find('button').trigger('click');
        clickMenuItem(dropdownEle, 'versions');
        TestUtils.asyncElement('body',
      ...
      it('Select a version and click on promote to major version', function(done) {
        ...
        TestUtils.asyncElement('body',
            ...
            function(el) {
                el.trigger('click');
                TestUtils.asyncElement('body',
                    '.csui-metadata-versions tbody td.csui-table-cell-version_number_name > span[title="3.0"]').done(
                    function() {
                        done();
                    });
            });
  ...
  function clickMenuItem(dropdown, name) {
    var menuItem = dropdown.find('li[data-csui-tab=' + name + '] > a');
    expect(menuItem.length).toBe(1);
    clickElement(menuItem[0]);
  }
