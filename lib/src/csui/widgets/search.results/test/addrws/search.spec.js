...
define([
  ...
  function switchResultView(done, what) {
    var children = "";
    var searchResultsElement = searchResultsView.$el.find(
        ".csui-search-results-body .csui-search-results-content .csui-result-list");
    ...
    var deferred;
    if (standardElement.length === 1 && what === "standard" || tableElement.length === 1 && what ===
        "table") {
      done();
  ...
  function EventTracker() {
    ...
    this.start = function start(startFunction, timeoutMillis) {
      var eventNames = _.reduce(trackings, function (names, tracking) {
        names.push(tracking.name);
        return names;
      }, []);
      timeoutMillis = timeoutMillis || 500;
      mytimeout.error = new Error(
          _.str.sformat("Timeout {0} reached while waiting for events [{1}].", timeoutMillis,
              eventNames.join(',')));
  ...
  describe('AddRwsTest', function () {
    ...
    afterAll(function (done) {
      ...
      commands.filter({signature: "AddRelationTest"}).forEach(
          function (el) { commands.remove(el); });
    ...
    describe('BrowserCheckTest', function () {
      if (navigator.userAgent.indexOf('Macintosh') > 0) {
        return it(
            'Skip the check for the browser window size. The properties are zero in the headless Chrome on Macintosh.');
      }
    ...
    describe('EmtpyViewTest', function () {
      ...
      it('header title is initially visible and contains correct title', function (done) {
        ...
        expect(headerTitleElement.text().trim()).toEqual("Test add relationships",
            "header title text correct");
        expect(headerTitleElement.length && headerTitleElement[0].tagName.toLowerCase()).toEqual(
            "h2");
        done();
      });

      it('search icon is  not visible', function (done) {
        var searchIcon = searchResultsView.headerView.$el.find(
          ".csui-search-filter-parent");
        var filterIcon = searchResultsView.headerView.$el.find(
          ".csui-search-filter");
        expect(searchIcon.hasClass("binf-hidden")).toEqual(true, "search icon visibility");
        expect(isVisible(filterIcon[0])).toEqual(false, "filter icon visibility");
        expect(searchResultsView.$el.find(".csui-search-left-panel")
          .hasClass("csui-is-visible")).toEqual(true, "side panel visibility");
        done();
      });

      it('search fields are displayed', function (done) {
        var searchFieldsElements = searchResultsView.customSearchView.$el.find(
            ".csui-custom-search-formitems .binf-control-label");
        expect(searchFieldsElements.text().trim()).toEqual(
            "NameNumberObject KeyStreetCityCountryPostal CodeIndustryStatusStage");
        done();
      });
      ...
      it('search button is visible', function (done) {
        ...
        if (!isVisible(searchButtonElement[0])) {
          setTimeout(function () {
            ...
            expect(isVisible(searchButtonElement[0])).toEqual(true,
                "search button is inside visible area");
            done();
          }, 1000);
        } else {
          done();
      ...
      it('search results are initially empty', function (done) {
        expect($(".conws-searchview .csui-search-results-content .csui-nodetable").length).toEqual(
            0);
        done();
      });
    ...
    describe('StandardViewTest', function () {
      ...
      xit('paging bar is displayed correctly', function (done) {
        var itemCountElement = searchResultsView.$el.find(
            ".csui-search-results-body .csui-pager .csui-total-container-items");
        expect(itemCountElement.text().trim()).toEqual("About 99 items");
        var pageBrowseElements = searchResultsView.$el.find(
            ".csui-search-results-body .csui-pager .csui-pages [data-pageid]");
      ...
      it('standard view displays page 1', function (done) {
        var nameElements = searchResultsView.$el.find(
            ".csui-search-results-body .csui-search-results-content .csui-result-list .csui-search-item-name");
      ...
      it('standard view sorting drop down', function (done) {
        var sortSelectionElement = searchResultsView.$el.find(
            ".csui-search-results-body .csui-search-sorting .csui-search-sort-options > button");
        expect(sortSelectionElement.text().trim()).toEqual("Name", "sorting field");
        var sortDirectionElement = searchResultsView.$el.find(
            ".csui-search-results-body .csui-search-sorting .cs-sort-links .search-sort-btn");
        expect(sortDirectionElement.hasClass("icon-sortArrowUp")).toEqual(true,
            "sort direction is ascending");
        var sortOptionElements = searchResultsView.$el.find(
            ".csui-search-results-body .csui-search-sorting .csui-search-sort-options .csui-sort-option");
        expect(sortOptionElements.text().trim()).toEqual(
            "NumberObject KeyStreetCityCountryPostal CodeIndustryStatusStageName");
        done();
      });
      ...
      describe('ItemSelect', function () {
        ...
        beforeAll(function (done) {
          expect(searchResultsView.targetView.standardHeaderView._selectAllView &&
                 searchResultsView.targetView.standardHeaderView._selectAllView.$el).toBeTruthy();
          ...
          expect(searchResultsView.targetView.standardHeaderView.selectedCounterView &&
                 searchResultsView.targetView.standardHeaderView.selectedCounterView.$el).toBeTruthy();
          expect(searchResultsView.targetView.standardHeaderView.toolbarView &&
                 searchResultsView.targetView.standardHeaderView.toolbarView.$el).toBeTruthy();
          done();
        });
        ...
        it('before select', function (done) {
          ...
          checkboxElements = searchResultsView.targetView.standardHeaderView._selectAllView.$el.find(
              ".csui-checkbox");
          expect($(checkboxElements[0]).attr("aria-checked")).toEqual("false", "header check box");

          checkboxElements = searchResultsView.targetView.$el.find(
              ".csui-search-item-check .csui-checkbox");
          ...
          selectedCounterRegion = searchResultsView.targetView.standardHeaderView.selectedCounterView.$el.find(
              ".csui-selected-counter-region>button");
          expect(isVisible(selectedCounterRegion[0])).toEqual(false,
              "selection counter visibility");

          testCommandElement = searchResultsView.targetView.standardHeaderView.toolbarView.$el.find(
              '[data-csui-command="addrelationtest"]');
        ...
        it('select item', function (done) {
          ...
          searchResultsView.targetView.children.findByIndex(0)._checkboxView.model.set("checked",
              "true");
          ...
          expect($(selectedCounterRegion[0]).text().trim()).toEqual("Selected1",
              "selection counter text");

          testCommandElement = searchResultsView.targetView.standardHeaderView.toolbarView.$el.find(
              '[data-csui-command="addrelationtest"]');
          expect(isVisible(testCommandElement[0])).toEqual(true, "test command visibility");
          expect(testCommandElement.text().trim()).toEqual(lang.ToolbarItemAddRelationSubmit,
              "test command text");
      ...
      describe('PageSelect', function () {
        ...
        it('select page', function (done) {
          ...
          selectedCounterRegion = searchResultsView.targetView.standardHeaderView.selectedCounterView.$el.find(
              ".csui-selected-counter-region>button");
          expect(isVisible(selectedCounterRegion[0])).toEqual(true, "selection counter visibility");
          expect($(selectedCounterRegion[0]).text().trim()).toEqual("Selected10",
              "selection counter text");
    ...
    describe('TableViewTest', function () {
      ...
      it('table view sorting drop down', function (done) {
        var sortSelectionElement = searchResultsView.headerView.$el.find(
            ".csui-search-sorting .csui-search-sort-options > button");
        expect(sortSelectionElement.text().trim()).toEqual("Name", "sorting field");
        var sortDirectionElement = searchResultsView.headerView.$el.find(
            ".csui-search-sorting .cs-sort-links .search-sort-btn");
        expect(sortDirectionElement.hasClass("icon-sortArrowUp")).toEqual(true,
            "sort direction is ascending");
        var sortOptionElements = searchResultsView.headerView.$el.find(
            ".csui-search-sorting .csui-search-sort-options .csui-sort-option");
        expect(sortOptionElements.text().trim()).toEqual(
            "NumberObject KeyStreetCityCountryPostal CodeIndustryStatusStageName");
        done();
      });
      ...
      describe('ItemSelect', function () {
        ...
        beforeAll(function (done) {
          expect(searchResultsView.targetView && searchResultsView.targetView.$el).toBeTruthy();
          expect(searchResultsView.headerView && searchResultsView.headerView.$el).toBeTruthy();
          expect(searchResultsView.targetView._tableRowSelectionToolbarView &&
                 searchResultsView.targetView._tableRowSelectionToolbarView.$el).toBeTruthy();
          done();
        });
        ...
        it('check before select item', function (done) {
          ...
          checkboxElements = searchResultsView.targetView.$el.find(
              "thead .csui-table-cell-_select .csui-checkbox");
          expect($(checkboxElements[0]).attr("aria-checked")).toEqual("false", "header check box");

          checkboxElements = searchResultsView.targetView.$el.find(
              "tbody .csui-table-cell-_select .csui-checkbox");
          expect($(checkboxElements[0]).attr("aria-checked")).toEqual("false", "item check box");
          ...
          selectedCounterRegion = searchResultsView.targetView._tableRowSelectionToolbarView.$el.find(
              ".csui-selected-counter-region>button");
          expect(isVisible(selectedCounterRegion[0])).toEqual(false,
              "selection counter visibility");

          testCommandElement = searchResultsView.targetView._tableRowSelectionToolbarView.$el.find(
              '[data-csui-command="addrelationtest"]');
          expect(isVisible(testCommandElement[0])).toEqual(false, "test command visibility");
        ...
        it('select item', function (done) {
          ...
          TestUtil.waitFor(function () {
            var headerHeight = searchResultsView.headerView.$el.find(
                ".csui-search-header").height();
            var toolbarHeight = searchResultsView.headerView.$el.find(
                ".csui-table-rowselection-toolbar").height();
            return headerHeight === 0 && toolbarHeight >= 48;
          }, "header to disappear, toolbar to appear").then(done, done.fail);
        });
        ...
        it('show header', function (done) {
          var toggleHeaderElement = searchResultsView.targetView._tableRowSelectionToolbarView.$el.find(
              ".csui-condensed-header-toggle");
          ...
          }, "header to reappear, toolbar to stay visible").then(done, done.fail);
        });
        ...
        it('deselect item', function (done) {
          ...
          }, "header to stay visible, toolbar to disappear").then(done, done.fail);
        });
      ...
      describe('PageSelect', function () {
        ...
        it('select page', function (done) {
          ...
          }, "header to stay hidden, toolbar to stay visible").then(done.fail,
              function () {done();});
        });
        ...
        it('deselect page', function (done) {
          ...
          TestUtil.waitFor(function () {
            ...
            var toolbarHeight = searchResultsView.headerView.$el.find(
                ".csui-table-rowselection-toolbar").height();
            return headerHeight >= 48 && toolbarHeight === 0;
          }, "header to reappear, toolbar to disappear").then(done, done.fail);
        });
      ...
      describe('Side panel', function () {
        ...
        it('search icon is visible', function (done) {
          searchIcon = searchResultsView.headerView.$el.find(
            ".csui-search-form-filter");
          filterIcon = searchResultsView.headerView.$el.find(
            ".csui-search-filter");
          expect(isVisible(searchIcon[0])).toEqual(true, "search icon visibility");
          expect(isVisible(filterIcon[0])).toEqual(false, "filter icon visibility");
          done();
        });

        it('verify search results form is open in side panel', function (done) {
          if (searchResultsView.$el.find(
            ".csui-search-results-custom").hasClass("binf-hidden")) {
            $(searchIcon[0]).trigger("click");
          }
          expect(searchResultsView.$el.find(".csui-search-left-panel").hasClass("csui-is-visible")).toEqual(true, "side panel visibility");
          expect(searchResultsView.$el.find(
            ".csui-search-results-custom").hasClass("binf-hidden")).toEqual(false, "search form visibility");
          done();
        });

        it('Close side panel', function (done) {
          $(searchIcon[0]).trigger("click");
          expect(searchResultsView.$el.find(".csui-search-left-panel").hasClass("csui-is-visible")).toEqual(false, "side panel visibility");
          done();
        });
      });
      ...
      describe('ShowDescription', function () {
        ...
        var showDesciptionHeaderButton, descriptionText;

        it('toogle description button in header available', function (done) {
          showDesciptionHeaderButton = searchResultsView.headerView.$el.find(
              ".csui-search-header-action.csui-description-toggle");
          expect(showDesciptionHeaderButton.length).toBe(1);
          expect(showDesciptionHeaderButton.attr("title")).toEqual("Show description");
          done();
        });

        it('check item description is shown', function (done) {
          descriptionText = searchResultsView.targetView.$el.find(
              "tbody .csui-details-row .csui-description-colunm .description");
          expect(descriptionText.length).toEqual(4);
          expect($(descriptionText[0]).text().trim()).toEqual(
              "1 \nThe best result we found for your search is Agnes J Imes age 60s in Statesville, NC. Agnes is related to Alana Imes and Anthony Wayne Imes as well as 5 additional people. Select this result to view Agnes J Imes's phone number, address, and more. \n2 \nTh");
          done();
        });
    ...
    describe('ViewSwitchTest', function () {
      ...
      it('table view select first item', function (done) {
        var checkboxElements = searchResultsView.targetView.$el.find(
            "tbody .csui-table-cell-_select .csui-checkbox");
        $(checkboxElements[0]).trigger('click');
        expect($(checkboxElements[0]).attr("aria-checked")).toEqual("true", "item check box");
        done();
      ...
      it('search setup page 5', function (done) {
        ...
        page5request.start(function () {
          var pageBrowseElement5 = searchResultsView.$el.find(
              ".csui-search-results-body .csui-pager .csui-pages [data-pageid=4]");
      ...
      it('standard view select page 5', function (done) {
        var checkboxElements = searchResultsView.targetView.standardHeaderView._selectAllView.$el.find(
            ".csui-checkbox");
        $(checkboxElements[0]).trigger('click');
        var selectedCounterRegion = searchResultsView.targetView.standardHeaderView &&
                                    searchResultsView.targetView.standardHeaderView.selectedCounterView.$el.find(
                                        ".csui-selected-counter-region>button");
        expect($(selectedCounterRegion[0]).text().trim()).toEqual("Selected11",
            "selection counter text");
        done();
      });
      it('standard view command execution', function (done) {
        ...
        var testCommandLink = searchResultsView.targetView.standardHeaderView &&
                              searchResultsView.targetView.standardHeaderView.toolbarView.$el.find(
                                  '[data-csui-command="addrelationtest"]>a');
    ...
    describe('SearchSortTest', function () {
      ...
      it("prepare search Country DE", function (done) {
        searchResultsView.customSearchView.once("render:form", done);
        searchResultsView.customSearchView.customFormView.formView.model.get(
            "data").search.wnf_att_yw39_7 = "DE";
      ...
      it('sort City setup', function (done) {
        ...
        sortCITYrequest.start(function () {
          var sortOptionButton = searchResultsView.headerView.$el.find(
              ".csui-search-sorting .csui-search-sort-options button");
          $(sortOptionButton[0]).trigger('click');
          var sortOptionElement = searchResultsView.headerView.$el.find(
              ".csui-search-sorting .csui-search-sort-options a[data-csui-sortoption-id=wnf_att_yw39_6]");
      ...
      it('toggle Sort setup', function (done) {
        ...
        toggleSORTrequest.start(function () {
          var sortDirectionElement = searchResultsView.$el.find(
              ".csui-search-results-body .csui-search-sorting .cs-sort-links a.search-sort-btn");
    ...
    describe('InitialSortOrderTest', function () {
      ...
      it('table view sorting drop down', function (done) {
        ...
        var sortDirectionElement = searchResultsView.headerView.$el.find(
            ".csui-search-sorting .cs-sort-links .search-sort-btn");
        expect(sortDirectionElement.hasClass("icon-sortArrowDown")).toEqual(true,
