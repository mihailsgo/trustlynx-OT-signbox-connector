...
define([
  ...
  './search.results.mock.js','smart/controls/pagination/impl/nls/root/localized.strings', 'csui/lib/jquery.mousehover'
], function (Marionette, $, _, SearchResultsView, PageContext, SearchResultsCollectionFactory,
    SearchQueryModelFactory, SearchMetadataFactory, SearchBoxView, TestUtils, mock, paginationLang) {
  'use strict';
  describe('SearchResultsView', function () {
    ...
    describe('given empty configuration', function () {
      ...
      it('move to page 2, should get table-actions and counter icon', function (done) {
        var str = _.str.sformat(paginationLang.ShowPageNAria, 2, searchResultsView.paginationView.numPages);
        var page2 = searchResultsView.$el.find(
            ".csui-paging-navbar > ul > li:not(.csui-overflow) > a[aria-label='"+str+"']");
        page2.trigger('click');
        TestUtils.asyncElement(searchResultsView.$el,
            ".csui-control.csui-checkbox[title='Select all results on current page.'][aria-checked='false']").done(
            function (el) {
              expect(searchResultsView.$el.find(
                  '.csui-selected-counter-region .csui-selected-counter-value:visible'
              ).length).toEqual(
          ...
          function () {
            var collectionLength = searchResultsView.collection.selectedItems.length,
                unselectIcon     = searchResultsView.$el.find(
                    '.csui-selected-item .csui-deselected-icon').first(),
      ...
      it('click on clearAll icon, shows confirmation dialog, click on yes should reset all selection',
         ...
         it('verify whether checkboxes have enabled', function (done) {
            TestUtils.asyncElement(searchResultsView.$el, '.cs-list-group .csui-checkbox').done(
              function(el) {
                expect(el.is(':visible')).toBeTruthy();
                done();
            });
          ...
          it("verify facet bar is displayed on applying a filter", function (done) {
            TestUtils.asyncElement($('body'), ".csui-filter-name").done(
                function (el) {
                  expect(el).toBeTruthy();
                  el[0].click();
                  TestUtils.asyncElement(searchResultsView.$el, ".csui-facet-bar-hidden", true).done(
                      function () {
                        expect(searchResultsView.$el.find(
                            '.csui-facet-bar-hidden').length).toEqual(0);
                        done();
                  });
              });
          });
          ...
          it("clear filters on clicking clear-all icon", function (done) {
            var clearAllButton = searchResultsView.$el.find('.csui-clear-all');
            clearAllButton.trigger('click');
            TestUtils.asyncElement(searchResultsView.$el, ".csui-table-empty", true).done(
                function () {
                  expect(searchResultsView.$el.find('.csui-facet-panel').length).toEqual(1);
                  done();
            });
          });
         ...
         it('verify facet panel is closed on clicking hide filter panel', function(done) {
           TestUtils.asyncElement(searchResultsView.$el,'.csui-search-filter').done(
             function (el) {
              var collapsed = searchResultsView.$el.find(
                  '.csui-icon-v2-on.csui-icon-v2__csui_action_filter32');
              expect(collapsed.length).toBe(1);
              searchResultsView.listenTo(searchResultsView.targetView, 'facet:opened',
                  function (open) {
    ...
    describe('Tabular Search', function () {
      ...
      it('verify table toolbar is displayed', function () {
        var tableToolbar = searchResultsView.$el.find(
            '.csui-table-rowselection-toolbar.csui-table-rowselection-toolbar-visible');
      ...
      it("checking for the presence of standard search view toggle button",
          function () {
            expect($(".csui-tabular-view" +
                     " .csui-icon-v2.csui-icon-v2__csui_action_table_standard32").is(
                ":visible")).toBeTruthy();
      });
      ...
      xdescribe('Rename an item from Search results', function () {
        var searchItem;
    ...
    describe('Search Settings', function () {
      ...
      beforeAll(function () {
        ...
        detectCardChangeAndDone = function (done) {
          var el = searchResultsView.headerView.settingsView.$el
            .find('.transition-hide, .transition-show');
          if (el.length) {
      ...
      it("Check for the presence of tabular view icon", function (done) {
        TestUtils.asyncElement(searchResultsView.$el, ".csui-tabular-view").done(
            function () {
              var tabularViewIcon = $(
                  ".csui-tabular-view .csui-icon-v2.csui-icon-v2__csui_action_table_tabular32").is(
                  ":visible");
              if (searchResultsView.collection.length > 0) {
                expect(tabularViewIcon).toBeTruthy();
              } else {
      ...
      it('select summary/description',function(done){
        var summaryDescriptionList = searchResultsView.$el.find(
            '.csui-summary-description-list .columns-list .column-item .column-item-container');
      ...
      it('Click on delete button to remove item', function (done) {
        var selectedItems = searchResultsView.$el.find(
            '.selected-columns-container .csui-selected-column .column-item'),
            selectedCount = selectedItems.length;
        selectedItems.eq(2).trigger('click');
        var deleteButton = selectedItems.eq(2).find('.remove-button');
    ...
    describe('Search Settings for ESC press', function () {
      ...
      it("Check for the presence of tabular view icon", function (done) {
        TestUtils.asyncElement(searchResultsView.$el, ".csui-tabular-view").done(
            function () {
              var tabularViewIcon = $(".csui-tabular-view" +
                                      " .csui-icon-v2__csui_action_table_tabular32").is(":visible");
              if (searchResultsView.collection.length > 0) {
                expect(tabularViewIcon).toBeTruthy();
              } else {
                expect(tabularViewIcon).toBeFalsy();
      ...
      describe('For summary/description settings', function () {

        describe('ESC on child view', function () {
          ...
          it('save summary/description settings',function(done){
            var summaryDescriptionList = searchResultsView.$el.find(
                '.csui-summary-description-list .columns-list .column-item .column-item-container');
            summaryDescriptionList.eq(0).trigger({type: 'keydown', keyCode: 27 });
            TestUtils.asyncElement(searchResultsView.headerView.$el,
                ".csui-settings-dropdown").done(
              function (el) {
                var searchDropdown = el.find('.settings-dropdown-container');
                expect(searchDropdown.is(":visible")).toBeFalsy();
                done();
              });
          });

          it('description icon should not be displayed',function(){
            var descriptionIcon = searchResultsView.$el.find(
                '.csui-search-header-action.icon-description-toggle');
            expect(descriptionIcon.is(":visible")).toBeFalsy();
          });

          it('description and summary are not displayed',function(){
            var descriptionColumn = searchResultsView.$el.find(
                '.csui-details-row .csui-description-colunm');
            var summaryColumn = searchResultsView.$el.find(
                '.csui-details-row .csui-summary-colunm');
            expect(descriptionColumn.length).toEqual(0);
            expect(summaryColumn.length).toEqual(0);
          });
          ...
          it('Should display checkmark for None', function (done){
            var label = $('.icon.selected + span');
            expect(label.text()).toEqual('None');
            done();
          });
        ...
        describe('ESC on back button ', function () {

          it('keydown for select summary/description',function(done){
            var summaryDescriptionList = searchResultsView.$el.find(
                '.csui-summary-description-list .columns-list .column-item .column-item-container');
            summaryDescriptionList.eq(1).trigger('focus');
            summaryDescriptionList.eq(1).trigger({type: 'keydown', keyCode: 32});
            var label = $('.icon.selected + span');
            expect(label.text()).toEqual('Summaries and descriptions');
            done();
          });
          ...
          it('focus should move to back button', function(done){
            var summaryDescriptionList = searchResultsView.$el.find(
              ...
              var backButton = searchResultsView.$el.find(
                '.csui-summary-description-list .column-header .arrow_back');
              summaryDescriptionList.eq(1).trigger({type: 'keydown', keyCode: 9});
              TestUtils.asyncElement(document.body,
                ".summary-description-container.csui-settings-show").done(
                function (el) {
                  var backButton = el.find(
          ...
          it('save summary/description settings',function(done){
            ...
            var summaryDescriptionList = searchResultsView.$el.find(
                '.csui-summary-description-list .columns-list .column-item .column-item-container');
            backButton.trigger({type: 'keydown', keyCode: 27 });
            TestUtils.asyncElement(searchResultsView.headerView.$el,
                ".csui-settings-dropdown").done(
              function (el) {
                var searchDropdown = el.find('.settings-dropdown-container');
          ...
          it('description icon should be displayed',function(){
            var descriptionIcon = searchResultsView.$el.find(
                '.csui-search-header-action .csui-icon-v2.csui-icon-v2__csui_action_reveal_description32');
            expect(descriptionIcon.is(":visible")).toBeTruthy();
          });

          it('description and summary should be displayed',function(){
            var descriptionColumn = searchResultsView.$el.find(
                '.csui-details-row .csui-description-colunm');
            var summaryColumn = searchResultsView.$el.find(
                '.csui-details-row .csui-summary-colunm');
            expect(descriptionColumn.length).toBeGreaterThan(0);
            expect(summaryColumn.length).toBeGreaterThan(0);
          });
          ...
          it('Should display checkmark for Summaries and descriptions', function (done){
            var label = $('.icon.selected + span');
            expect(label.text()).toEqual('Summaries and descriptions');
            done();
          });
    ...
    describe('Without Search Settings', function () {
      ...
      it('verify absence of search settings', function (done) {
        TestUtils.asyncElement(searchResultsView.$el, ".csui-search-header").done(
          function () {
              var searchSettings = searchResultsView.$el.find(
                  '.csui-search-settings .icon-toolbar-settings');
