...
define([
  ...
  'csui/models/favorite2columns', 'csui/utils/contexts/factories/favorites2',
  'csui/models/favorites2', 'csui/widgets/favorites2.table/toolbaritems',
  'csui/widgets/favorites2.table/toolbaritems.masks',
  'csui/controls/progressblocker/blocker',
  'csui/widgets/favorites2.table/impl/favorites2.groups.view',
  "csui/controls/table/table.view", 'csui/utils/commands',
  ...
  'csui/utils/accessibility', 'csui/utils/contexts/perspective/plugins/node/node.extra.data',
  'csui/lib/perfect-scrollbar', 'css!csui/widgets/favorites2.table/impl/favorites2.table.view'
    ...
    Favorite2CollectionFactory, Favorite2Collection, toolbaritems, ToolbarItemsMasks,
    BlockingView, FavoriteGroupsView, TableView, commands, TableActionBarView,
    ToolbarCommandController, LayoutViewEventsPropagationMixin, DefaultActionBehavior,
    InfiniteScrollingBehavior, PerfectScrollingBehavior,
    TabableRegionBehavior, GlobalMessage, favorites2TableColumns,
    PerspectiveHeaderView, FieldsV2Mixin, MultiNodeFetchMixin,
    NodeSelectionRestoreMixin, template, lang, TableRowSelectionToolbarBehavior,
    Accessibility, nodeExtraData) {
  'use strict';
  ...
  var Favorites2TableView = Marionette.LayoutView.extend({
    ...
    initialize: function () {
      ...
      var defaultActionCommands = this.defaultActionController.actionItems.getAllCommandSignatures(commands);
      this.collection.setCommands(commands.getAllSignatures());
      this.collection.setDefaultActionCommands(defaultActionCommands);
      this.collection.setEnabledDelayRestCommands(true);
      this.groups.favorites.setResourceScope(Favorite2CollectionFactory.getDefaultsOnlyResourceScope());
      this.groups.favorites.setDefaultActionCommands(defaultActionCommands);
      this.groups.favorites.setEnabledDelayRestCommands(false);
      this.groups.favorites.setFields(nodeExtraData.getModelFields());
      this.groups.favorites.setExpand(nodeExtraData.getModelExpand());
      ...
      this.setGroupsView();
      this.initSelectionMixin(this.options, this.collection, "favorites");
      this.setTableView();
    ...
    _addUrlParametersSupport: function (context) {
      ...
      this.enableViewState = viewStateModel &&
                             viewStateModel.addUrlParameters(this.getDefaultUrlParameters(), context);
    },

    _setSelectedGroup: function (group) {
      var setViewState = false;
      ...
      setViewState && this.clearAllSelectedNodes();
      setViewState && this.setViewStateGroupName(group.get('name'));
    },
    ...
    _getGroupNameFromViewState: function () {
      ...
      if (viewStateModel) {
        return viewStateModel.getViewState('group', true);
      }
    },
    ...
    _trySelectGroup: function () {
      ...
      if (this.selectedGroup !== undefined) {
        ...
        var groupName  = this._getGroupNameFromViewState();
        groupToSelect = groupName && this._getGroupByName(groupName);
        if (!groupToSelect) {
    ...
    setTableView: function () {
      this.tableView = new InfiniteScrollingTableView({
        ...
        tableAria: lang.favoritesTableAria,
        inlineBar: {
    ...
    getViewStateGroupName: function () {
      return this.context.viewStateModel.getViewState('group', true);
    },

    setViewStateGroupName: function (groupName, options) {
      if (this.getViewStateGroupName() !== groupName) {
        var viewStateModel = this.context.viewStateModel;
        options || (options = {});
        options.encode = true;
        if (options && options.default) {
          viewStateModel.setDefaultViewState('group', groupName, options);
        }
        return viewStateModel.setViewState('group', groupName, options);
      }
    },
    ...
    isRoutingToOtherRouter: function () {
      ...
      if (viewStateModel.get(viewStateModel.CONSTANTS.CURRENT_ROUTER) !== 'Root') {
        return true;
      }
    },
    ...
    onViewStateChanged: function () {
      ...
      var viewStateModel = this.context.viewStateModel,
          groupName      = viewStateModel.getViewState('group', true),
      ...
      selectedGroupChanged && this._trySelectGroup();
    }
  });

  _.extend(Favorites2TableView.prototype, LayoutViewEventsPropagationMixin);
  _.extend(Favorites2TableView.prototype, MultiNodeFetchMixin);
  _.extend(Favorites2TableView.prototype, NodeSelectionRestoreMixin);
