...
define(["module",
  ...
  var FavoriteGroupsView = Marionette.LayoutView.extend({
    ...
    _tryReSetFocus: function () {
      ...
      if (!model) {
        var index2Use = 0;
        if (this._focusedModelIndex !== undefined) {
          index2Use = this._focusedModelIndex;
        }
        model = this.collection.at(index2Use);
      }
      if (model) {
    ...
    selectGroup: function (group) {
      ...
      this._tryReSetFocus(newlySelectedGroupModel);
    },
