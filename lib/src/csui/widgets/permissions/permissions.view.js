...
define(['module', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/marionette',
  ...
  'css!csui/widgets/permissions/impl/permissions'
    ...
    TaskQueue, PermissionPreCheck, NoPermssionsTemplate, Url, ServerInfoFactory,
    NodeActionCollection, lang) {
  ...
  var PermissionsView = Marionette.ItemView.extend({
    ...
    constructor: function PermissionsView(options) {
      ...
      if (options.model.fetching) {
          this.listenToOnce(options.model, "sync", this.init.bind(this, options));
      } else {
        this.init(options);
      }
    },
    ...
    init: function(options) {
      ...
      if (this.hasPermissionAction) {
        ...
        var serverInfo = this.context.getObject(ServerInfoFactory, {
          permanent: true,
          detached: true
        });

        serverInfo.ensureFetched().done(
            _.bind(function () {
              var advVersioningEnabled = serverInfo.get('server') &&
                                         serverInfo.get('server').advanced_versioning;
              this.model.set('advancedVersioningEnabled', advVersioningEnabled, {
                'silent': true
              });
              if (!this.model.actions.get('permissions')) {
                this.model.fetch(
                    {error: _.bind(self.onPermissionsRequestFailed, self, self.options)}).done(
                    _.bind(function () {
                ...
                if (self.collection.fetching) {
                  self.listenToOnce(self.collection, "sync", _.bind(self.onPermissionsRequestSuccess, self, self.options));
                } else {
                  self.collection.fetch({
                    success: _.bind(self.onPermissionsRequestSuccess, self, self.options),
                    error: _.bind(self.onPermissionsRequestFailed, self, self.options)
        ...
        this.listenTo(this, "permission:changed", function (status) {
          ...
          openables.fetch().then(_.bind(function (response) {
            var userHasEditPermissions = _.map(response.results, function (value, key) {
              return value.data.editpermissions;
            });
            this.permissionsContentView.options.hasEditPermissionAction = userHasEditPermissions &&
                                                                          !!userHasEditPermissions[0];
            ...
            if (!this.permissionsContentView.options.hasEditPermissionAction &&
    ...
    onFocusInUserPicker: function (event) {
      ...
      !!element && element.length && element.hasClass('binf-open') &&
      element.removeClass('binf-open');
    ...
    renderContent: function () {
      ...
      if (this.permissionsContentView) {
        ...
        this.viewStateModel && this.viewStateModel.setSessionViewState('permissions', true);
      }
    },
    ...
    onBeforeDestroy: function () {
      if (this.permissionsContentView) {
        this.cancelEventsToViewsPropagation(this.permissionsContentView);
        this.permissionsContentView.destroy();        
        this.viewStateModel && this.viewStateModel.setSessionViewState('permissions', false);
      }
    },
