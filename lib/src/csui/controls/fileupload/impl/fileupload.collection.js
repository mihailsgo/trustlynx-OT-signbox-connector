...
define(['require', 'module', 'csui/lib/jquery', 'csui/lib/underscore',
  ...
  'csui/controls/fileupload/impl/upload.controller', "csui/utils/url",
...
], function (require, module, $, _, Backbone, log, lang, UploadController, URL,
  ...
  var FileUploadModel = Backbone.Model.extend({
    ...
    constructor: function FileUploadModel(attributes, options) {
      ...
      var uploadCollectionClass = attributes.UploadFileCollection || UploadFileCollection;

      Backbone.Model.prototype.constructor.call(this, attributes, options);

      var container = this.container = this.get('container'),
        parentCollection = this.get('parentCollection');
      this.uploadFiles = new uploadCollectionClass();
    ...
    resolveNamingConflicts: function (options) {
      ...
      if (options.skipResolveNamingConflicts) {
    ...
    addToUpload: function (files) {
      ...
      if (files && files.length > 0) {
        ...
        var originatingView = this.get('originatingView');
        var largeFileSettings = originatingView && originatingView.largeFileSettingsModel && originatingView.largeFileSettingsModel.get('largeFileSettings');
        var minFileSize = largeFileSettings && largeFileSettings.min_size,
          isContainerCompoundDocument = originatingView && originatingView.container && originatingView.container.get('type') === 136,
          fileOrder,order_next;
        if (isContainerCompoundDocument) {
          order_next = originatingView.container.get('order_next');
          fileOrder = !!originatingView.container.get('fileOrder') ? originatingView.container.get('fileOrder') : 0;
        }
        promises = _.map(files.models, function (file) {
          ...
          if (!file.get('id') || file.get('newName') || file.get('newVersion')) {
            ...
            if (isContainerCompoundDocument && !!order_next && !file.get('newVersion')) {
              file.set("order_new", order_next + fileOrder++);
            }
            if (size > minFileSize) {
        ...
        if(isContainerCompoundDocument){
          originatingView.container.set('fileOrder',fileOrder); //setting the latest fileOrder value to prevent duplicate order values in multiple upload scenario
        }
      }
      else {
