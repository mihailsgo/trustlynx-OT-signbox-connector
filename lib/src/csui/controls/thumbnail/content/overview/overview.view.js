...
define(['csui/lib/jquery',
  ...
  'css!csui/controls/thumbnail/content/overview/impl/overview'
    ...
    ModalKeyboardNavigationMixin, base, iconRegistry,
    i18n, lang, template) {
  'use strict';
  ...
  var OverviewView = Marionette.LayoutView.extend({
    ...
    _showPopover: function (event) {
      ...
      this.$popoverEl.on('shown.binf.popover', _.bind(function (event) {
        ...
        this.propertiesOverview && this.propertiesOverview.nodeStateRegion.$el.find('button, a').attr("tabindex", "0");
    ...
    onShowOverviewFlyout: function (args) {
      ...
      if (base.isIE11()) {
        ...
        flyOutTopPosition = flyOutTarget.offset().top - (navBarHeight + parseInt(popoverArrowEl.css('top'))) + 9;
      }
      ...
      if ((popoverMetadata.height() > window.innerHeight) && !base.isIE11()) {
        popoverMetadataHeight = window.innerHeight - (popoverActions.height() + navBarHeight + $('.csui-table-paginationview').height() + $('#breadcrumb-wrap').innerHeight());
        popoverMetadata.css({ 'height': popoverMetadataHeight });
        popoverArrowTop = flyOutTarget.offset().top - (navBarHeight + flyOutTopPosition) + 11;
        if (popoverArrowTop > popoverMetadataHeight) {
          flyOutTopPosition = flyOutTopPosition + (popoverArrowTop - popoverMetadataHeight - popoverActions.height());
          popoverArrowTop = popoverArrowTop - (popoverArrowTop - popoverMetadataHeight - popoverActions.height());
        }
        if (flyOutTopPosition < 0) {
          flyOutTopPosition = 0;
          popoverArrowTop = flyOutTarget.offset().top - (navBarHeight + flyOutTopPosition) + 11;
        }
      ...
      else if (flyOutTopPosition < 0 && !base.isIE11()) {
      ...
      else if ((popoverMetadata.height() > window.innerHeight) && base.isIE11()) {
        popoverMetadataHeight = window.innerHeight - (popoverActions.height() + navBarHeight + $('.csui-table-paginationview').height() + $('#breadcrumb-wrap').innerHeight());
        popoverMetadata.css({ 'height': popoverMetadataHeight });
        popoverArrowTop = (flyOutTarget.offset().top - flyOutTopPosition) + perspectivePanelClientTop - navBarHeight + 12;
      ...
      if (isRtl && isWidthDiff) {
        var widthDiff = ($('.csui-thumbnail-container').width() - $(window).width()),
          facetView = this.options.originatingView && this.options.originatingView.$el.find('.csui-table-facetview');
        ...
        if (!facetView.hasClass("csui-facetview-hidden")) {
    ...
    _closePopover: function () {
      ...
      $('body').removeClass('csui-apply-zero-zindex');
      this.propertiesOverview && this.propertiesOverview.nodeStateRegion.$el.find('button, a').attr("tabindex", "-1");
