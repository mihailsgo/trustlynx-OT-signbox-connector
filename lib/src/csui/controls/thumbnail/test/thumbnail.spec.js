...
define(['csui/lib/jquery', 'csui/lib/underscore', 'csui/lib/marionette', 'csui/lib/backbone',
  ...
  describe("Thumbnail View suite", function () {
    ...
    describe("Thumbnail View", function () {
      ...
      xdescribe('Thumbnail add action', function () {
        ...
        it('Click the Selectall checkbox in thumbnail view', function (done) {
          var selectAll = $(".csui-checkbox-selectAll .csui-checkbox");
      ...
      xdescribe('Thumbnail add action', function () {
        ...
        it('Adding a folder', function (done) {
          TestUtils.asyncElement('body', ".csui-addToolbar:visible").done(
              function () {
                ...
                var addFolder = $("li[data-csui-command='add'] > a:contains('Folder')");
                addFolder.trigger('click');
                TestUtils.asyncElement(nodesTableView.$el,
                    ".csui-inline-editform:visible").done(
                    function () {
        ...
        it('Adding a Web Url', function (done) {
          TestUtils.asyncElement('body', ".csui-addToolbar:visible").done(
              function () {
                var addToolbar = $(".binf-dropdown-toggle").eq(0);
                addToolbar.trigger('click');
                var addFolder = $("a[aria-label='Add Web address']");
                addFolder.trigger('click');
                TestUtils.asyncElement(nodesTableView.$el,
                    ".csui-inline-editform:visible").done(
                    function () {
        ...
        it('Adding a Collection', function (done) {
          TestUtils.asyncElement('body', ".csui-addToolbar:visible").done(
              function () {
                var addToolbar = $(".binf-dropdown-toggle").eq(0);
                addToolbar.trigger('click');
                var addFolder = $("a[aria-label='Add Collection']");
                addFolder.trigger('click');
                TestUtils.asyncElement(nodesTableView.$el,
                    ".csui-inline-editform:visible").done(
                    function () {
        ...
        it('after checking for the inline form view click on cancel', function (done) {
          ...
          TestUtils.asyncElement(nodesTableView.$el,
              ".csui-inline-editform", true).done(
              function () {
        ...
        it('Adding a Shortcut', function (done) {
          TestUtils.asyncElement('body', ".csui-addToolbar:visible").done(
              function () {
                var addToolbar = $(".binf-dropdown-toggle").eq(0);
                addToolbar.trigger('click');
                var addFolder = $("li[data-csui-command='add'] > a:contains('Shortcut')");
                addFolder.trigger('click');
      ...
      describe('Thumbnail overview popover', function () {
        ...
        it('verify inline form is displayed',  function (done) {
          TestUtils.asyncElement(nodesTableView.$el, '.csui-inline-editform').done(
              function ($el) {
                expect($el.is(':visible')).toBeTruthy();
                done();
              });
        ...
        it('To verify thumbnail overview popover', function (done) {
          var overViewButton = $(".csui-thumbnail-overview-container .csui-thumbnail-overview-icon").eq(1);
          ...
          TestUtils.asyncElement('body', ".binf-popover-content:visible").done(
              function () {
                ...
                var object = $(".csui-thumbnail-item");
                var inlineForm = object.eq(0).find('.csui-inline-editform');
        ...
        it('To verify thumbnail properties actions in popover', function (done) {
          TestUtils.asyncElement(".binf-popover-content", ".csui-btn-metadata").done(
              function ($el) {
                expect($el.length).toEqual(1);
                expect($el.is(':visible')).toBeTruthy();
                $el.trigger('click');
        ...
        it('To verify thumbnail icons in metadata navigation panel', function (done) {
          TestUtils.asyncElement(".metadata-sidebar", ".cs-list-group").done(
              function ($el) {
                TestUtils.asyncElement('.cs-list-group', '.csui-icon-group:visible').done(
                    function ($el) {
                      expect($el.length).toBeGreaterThan(0);
                      expect($el.is(':visible')).toBeTruthy();
                      done();
      ...
      describe('Thumbnail rename action', function () {
        describe('verify webaddress view', function () {
          ...
          xit('navigate to permission page', function (done) {
            TestUtils.asyncElement(".csui-table-actionbar-bubble", ".binf-dropdown.binf-open").done(
                function ($el) {
                  expect($el.is(':visible')).toBeTruthy();
                  var permissionCommand = $el.find("li[data-csui-command='permissions'] > a");
                  permissionCommand.trigger('click');
          ...
          xit('click on the back arrow button to navigate back', function (done) {
        ...
        xdescribe('verify on renaming an item, font color of name is not changed', function () {
          ...
          it('verify inline form is displayed',  function (done) {
            TestUtils.asyncElement('body', '.csui-inline-editform').done(
                function ($el) {
                  expect($el.is(':visible')).toBeTruthy();
                  done();
                });
          ...
          it('verify inline form is displayed for second item', function (done) {
            TestUtils.asyncElement(nodesTableView.$el, '.csui-inline-editform').done(
                function ($el) {
                  var object = $(".csui-thumbnail-item");
                  var inlineForm = object.eq(1).find('.csui-inline-editform');
                  expect(inlineForm.is(':visible')).toBeTruthy();
                  expect(object.eq(1).find('.csui-thumbnail-no-default-action').length).toBe(0);
                  expect($el.is(':visible')).toBeTruthy();
                  done();
                });
          });
        });
        xdescribe('To verify inline form is not displayed on navigating back from permissions page', function () {
          it('click on inline action bar of first item', function (done) {
            var object = $(".csui-thumbnail-item");
            object.eq(0).trigger(
                {type: "pointerenter", originalEvent: {pointerType: "mouse"}});
        ...
        describe('To validate rename error conditions for items', function () {
          ...
          xit('select rename action for first item', function (done) {
            TestUtils.asyncElement(".csui-table-actionbar-bubble", ".binf-dropdown.binf-open").done(
                function ($el) {
                  expect($el.is(':visible')).toBeTruthy();
                  var renameCommand = $("li[data-csui-command='inlineedit'] > a");
          ...
          xit('verify inline form is displayed',  function (done) {
            TestUtils.asyncElement(nodesTableView.$el, '.csui-inline-editform').done(
                function ($el) {
                  expect($el.is(':visible')).toBeTruthy();
                  done();
          ...
          xit("Validating error scenario with existing name", function (done) {
            var nameInput = nodesTableView.$el.find('.csui-inlineform-type-name');
            nameInput.val('Child 2');
            nodesTableView.$el.find('.csui-btn-save').prop('disabled', false).trigger('click');
          ...
          xit("Check whether previous name is displayed on cancelling", function (done) {
            var cancelButton = nodesTableView.$el.find('.csui-btn-cancel');
            cancelButton.trigger('click');
