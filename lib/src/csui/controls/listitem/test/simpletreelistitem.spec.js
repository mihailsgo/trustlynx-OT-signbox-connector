...
define(["csui/lib/jquery",
  ...
  describe("The SimpleTreeListItem Control", function () {
    ...
    it("with the model and children collection", function () {
      ...
      expect(w.$('.cs-content .binf-list-group-item .csui-icon-v2').length).toEqual(3);
      expect($(w.$('.cs-content .list-item-title')[0]).html()).toEqual(itemName1);
      expect(w.$('.cs-content .binf-list-group-item .csui-icon-v2__csui_mime_pdf')
          .length).toEqual(1);
      expect($(w.$('.cs-content .list-item-title')[1]).html()).toEqual(itemName2);
      expect(w.$('.cs-content .binf-list-group-item .csui-icon-v2__csui_mime_word')
          .length).toEqual(1);
        ...
        function (done) {
          TestUtils.asyncElement(view.$el,
            ...
            TestUtils.asyncElement($($treelistitems[0]),
              ...
              expect($($treelistitems[0]).find('.binf-open .binf-dropdown-menu li:first a').is(
                  ':focus')).toBeTruthy();
              done();
            });
          });
