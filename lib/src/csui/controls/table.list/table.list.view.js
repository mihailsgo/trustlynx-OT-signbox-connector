...
define([
  ...
  var TableRowView = Marionette.LayoutView.extend({
    ...
    className: function () {
      var clsName = 'csui-table-row';
      if (this.model.get('isAddNew')) {
        clsName += ' active-row';
      }
      return clsName;
    },
    ...
    onFocusInTableCell: function (event) {
      if (this.options.showInlineToolbar && !this.lockedForOtherContols && this.$el.find('.csui-inlinetoolbar').hasClass('binf-hidden')) {
        this._showInlineRowActions();
      }
    },
    ...
    _accSetFocusToCurrentlyFocusedElement: function () {
      ...
      if (el.length > 0) {
        ...
        if (base.isVisibleInWindowViewport(el)) {
          el.find("input").length ? el.find("input").trigger("focus") : el.trigger("focus");
        }
      }
    },
    ...
    _updateTable: function (collectionOrModel) {
      if (!!this.tableBodyView && !!this.tableBodyView.accFocusedCell) {
        var row    = this.tableBodyView.accFocusedCell.row,
            ...
            tableRow =  this.tableBodyView.children.findByModel(collectionOrModel);
        if (collectionOrModel === this.collection) {
          ...
          if (base.isHybrid()) {
            tableRow._destroyInlineRowActionBar();
            tableRow._showInlineRowActions();
          }
        }
