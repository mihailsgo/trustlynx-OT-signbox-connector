...
define(['csui/lib/jquery',
...
], function ($, _, Backbone, Marionette, base, GlobalMessage, PerfectScrollingBehavior, lang, TabableRegionBehavior,
  ...
  var SelectedColumnItemView = Marionette.View.extend({
    ...
    templateContext: function () {
      var nonRemovable = ['OTName', 'OTMIMEType'],
        ...
        categoryName = columnName && columnName.replace(/\s*:[^:]*/g,''),
        attributeName = columnName && columnName.replace(/^[^:]*:\s*/,''),
        hasAttribute = categoryName !== attributeName;
      return {
        name: columnName,
        ...
        removeColumn: _.str.sformat(lang.removeColumn, columnName),
        removeColumnAria: _.str.sformat(lang.removeColumnMenuAria, columnName)
      };
    },
  ...
  var SelectedColumnsCollectionView = Marionette.CollectionView.extend({
    ...
    onSubItemDrop: function (scope, event) {
      ...
      var sourceKey = me._dragData && me._dragData.sourceKey || me.keyDragSource || null,
        ...
        pasteElementKey,pasteElementRegion,pasteElementIndex;
      event.preventDefault();
      ...
      if (me.keyDragSource) {
       ...
       pasteElementKey =  me._getItemKey(me.$el.find('.active'));
       pasteElementRegion = me.collection.findWhere({ key: pasteElementKey });
       pasteElementIndex = me.collection.indexOf(pasteElementRegion)-1;
       this.focusIndex = pasteElementIndex-1;
      }
    ...
    onChildViewKeydown: function (childView, event) {
      var isMac = base.isMacintosh(),
        ...
        focusables = this.$el.find('*[tabindex]'),dragLineFlag = false,
        addButton;
        ...
        if (focusables.length === 0) {
          addButton = this.$el.parents(".selected-columns-container").find('.add-button');
        ...
        if (keyCode === 38 || keyCode === 40) {
          ...
          $(focusables[this.focusIndex]).closest('.column-item').addClass('active');
          $(focusables[this.focusIndex]).trigger('focus');
          if (this.keyDragSource && dragLineFlag) {
  ...
  var SelectedColumnsView = Marionette.View.extend({
    ...
    templateContext: {
      ...
      ColumnSettingsBackAria: lang.SettingsBackAria,
    ...
    onKeydownBackArrow: function (event) {
      ...
      loaderDisabled = !!this.options.searchResultsView.$el.find('.load-container.binf-hidden').length;
      if (loaderDisabled && (keyCode === 13 || keyCode === 32 || keyCode === 37)) {
    ...
    showSettingsView: function (event) {
      ...
      display_regions = this.options.searchResultsView.headerView.data.display_regions;
      if(!!display_regions){
        this.options.searchResultsView.blockActions();
        data.append('display_regions', display_regions);
        this.options.settings.save(null, {
          ...
          this.options.searchResultsView.collection.settings_changed = true;
          this.options.tableCollection.fetch();
          this.options.settingsView.isChanged = false;
