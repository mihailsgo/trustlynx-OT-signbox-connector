...
], function ($, _, Backbone, Marionette, base, GlobalMessage, PerfectScrollingBehavior, lang, AvailableColumnsTemplate,
  ...
  var AvailableColumnsView = Marionette.View.extend({
    ...
    templateContext: function () {
      return {
        ...
        backToButtonAria: lang.selectcolumnsbackaria,
      };
    },
    ...
    showSelectedColumns: function (event) {      
      ...
      newModelsToBeSaved = !!this.options.selectedColumnsCollection.filter({isNew : true}).length,
      self = this;
      if(newModelsToBeSaved && !!display_regions){
        ...
        this.options.settings.save(null, {
          ...
          self.options.searchResultsView.collection.settings_changed = true;    
          self.options.tableCollection.fetch();
          self.options.searchResultsView.headerView.data = {};
          self.options.searchResultsView.unblockActions();
