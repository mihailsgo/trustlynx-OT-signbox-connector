...
define(['csui/lib/jquery',
  ...
  var SettingsDropDownView = Marionette.CollectionView.extend({
    ...
    onShowSettings: function (view, event) {
      var loaderDisabled = !!this.options.searchResultsView.$el.find('.load-container.binf-hidden').length;
      if(loaderDisabled) {
    ...
    onShowSettingsDropdown: function (view, event) {
      var keyCode = event.keyCode,
        ...
        settingsEle = this.$el.closest(".csui-search-settings");
      if (!loaderDisabled) {
        event.stopPropagation();
        event.preventDefault();
      } else if (keyCode === 27 || (keyCode === 9 && event.target
        && ((event.target.id === "summaryOrDescriptionContainer" && !event.shiftKey)||
        (event.target.id === "selectedColumnsContainer" && event.shiftKey)))) {
        settingsEle.trigger('focus');
        settingsEle.attr('aria-expanded', 'false');
