...
define([
  ...
  _.extend(ConflictResolver.prototype, {
    ...
    checkAddVersion: function(file) {
      if(this.excludeAddVersion) {
        file.excludeAddVersion = this.excludeAddVersion;
        return $.Deferred().resolve().promise();
      }

      var context = this.originatingView && this.originatingView.context,
        collection = file.node.collection ||
                     (this.originatingView && this.originatingView.collection) ||
                     (context && context.hasCollection('children2') &&
                      context.getCollection('children2')),
        nodeModel = null;
      if(collection) {
        nodeModel = _.find(collection.models, function (item) {
          return item.get('id') === file.get('id') ;
        });
      }

      if(nodeModel) {
          file.excludeAddVersion = !(nodeModel.actions.findWhere({signature: 'addversion'}));
          return $.Deferred().resolve().promise();
      }

      var promise = $.Deferred(),
          self = this;
      require(['csui/models/node.actions'
      ], function (NodeActionCollection) {

        var actions = new NodeActionCollection(undefined, {
          connector: self.container.connector,
          nodes: [ file.get('id') ],
          commands: [ 'addversion' ]
        });

        actions.fetch().then(function () {
          var nodeId = file.get('id'),
              node = actions.get(nodeId);
          file.excludeAddVersion = !(node.actions.findWhere({signature: 'addversion'}));
          promise.resolve();
        });
    ...
    validateAddVersion: function(conflictFiles){
      var self = this, promises= [];
      _.each(conflictFiles, function (file) {
               promises.push(self.checkAddVersion(file)) ;
      });
    ...
    showIndividualResolve: function (conflictFiles) {
      var self = this;
      var conflictDialog;
      this.validateAddVersion(conflictFiles).done(function(){
