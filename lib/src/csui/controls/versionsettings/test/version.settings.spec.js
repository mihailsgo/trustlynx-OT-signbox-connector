...
define(['csui/lib/jquery', 'csui/lib/underscore', 'csui/lib/backbone', 'csui/lib/marionette',
  ...
  describe('version settings view', function () {
    ...
    it('By default current folder option is selected, clicking on Current folder and Sub-folders option, it gets selected',
        function (done) {
          expect($("li.csui-folder-permissions-item[title='Current folder']").hasClass(
              'selected')).toBeTruthy();
          $("li.csui-folder-permissions-item[title='Current folder and sub-folders']").trigger(
              'click');
          TestUtils.asyncElement(
              "li.csui-folder-permissions-item[title='Current folder and sub-folders']",
    ...
    it('clicking anywhere on the screen when version settings view is open, closes it',
        function (done) {
          $('.version-mask').trigger('click');
          TestUtils.asyncElement('.binf-widgets',
              ".csui-toolbar-control-settings-dropdown", true).done(
              function ($el) {
                expect($el.is(':visible')).toBeFalsy();
                done();
              });
    ...
    describe('saving states of version control', function () {
      it('turning major/mjnor version in version control on and saving it',
          function (done) {
            $(".csui-toolbar-settings").trigger('click');
            TestUtils.asyncElement('.binf-widgets', ".cs-formfield.cs-booleanfield").done(
                function (ele) {
                  expect(ele.is(":visible")).toBeTruthy();
                  var e = $.Event("mousedown", {pageX: 500});
                  $('.binf-switch-container').trigger(e);
      ...
      it('major/minor version should be turned on',
          function (done) {
            TestUtils.asyncElement('body',
                ".csui-toolbar-settings:visible").done(
                function (el) {
                  expect(el.is(':visible')).toBeTruthy();
                  el.trigger('click');
                  TestUtils.asyncElement('.binf-widgets', ".binf-switch-off").done(
                      function (ele) {
                        expect(ele.is(":visible")).toBeTruthy();
                        done();
                      });
