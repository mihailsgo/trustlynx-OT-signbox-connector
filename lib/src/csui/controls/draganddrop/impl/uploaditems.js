...
define(['csui/lib/jquery', 'csui/lib/underscore',
  ...
  return {
    uploadFiles: function (files, status, options) {
      var fileUploadModel = fileUploadHelper.newUpload(status, options);
      return fileUploadModel.addFilesToUpload(files, options);
    },
    uploadFilesDirectly: function (files, status, options) {
      var fileUploadModel = fileUploadHelper.newUpload(status, options),
        fileUploadCollection = fileUploadModel.addFilesToUploadCollection(files, options);
      return fileUploadModel.addToUpload(fileUploadCollection);
    },

    handleResolveConflictsAndMetadata: function (items, options) {
      ...
      if (this.validateItems(items)) {
        ...
        this.resolveNamingConflicts(options).done(_.bind(function (itemsCollection) {
          var emptyFolders = _.filter(itemsCollection.models, function (item) {
            return item.get('isFolderEmpty');
          });
          if (emptyFolders.length === itemsCollection.length) {
            options.addableType = 0;
          }
          this.handleRequiredMetadata(this.uploadItems, options).done(
    ...
    addFolderToUploadCollection: function (folder, options) {
      this.uploadItems.add({
            ...
            isFolderEmpty: folder.get('isFolderEmpty')
          },
          {
            container: options.container,
            connector: options.container.connector,
    ...
    handleRequiredMetadata: function (itemsCollection, options) {
      var deferred = $.Deferred();
      ...
      require(['csui/widgets/metadata/impl/add.items/metadata.add.item.controller'
        ...
        metadataController.addItemsRequiredMetadata(itemsCollection.models, {
