...
##  autofillChildrenValue : boolean

This Flag used to change the behavior of autofill of depedant TKL attribute when on 1 valid value is available. Set this autofillChildrenValue flag to FALSE to stop the autofill of dependent tkl attribute value. Default is set to TRUE

 ```
 require.config({
    config: {
      'csui/controls/form/fields/tklfield.view': {
        autofillChildrenValue: false
      }
    }
  });
