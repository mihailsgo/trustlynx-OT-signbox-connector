...
define(['module', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/backbone', 'csui/lib/marionette',
...
], function (module, _, $, Backbone, Marionette, FormFieldView, itemTemplate,
  ...
  var TKLFieldView = FormFieldView.extend({
    constructor: function TKLFieldView(options) {
      ...
      if (this.formView) {  // formview is must, otherwise dependency in tkls wont work.
        ...
        this.listenTo(this.model, 'change', function () {
            this.formView.trigger('form:changed');
        });        
      }
      ...
      this.isPerfectScrollbarEnabled = PerfectScrollingBehavior.usePerfectScrollbar();
    },
    ...
    ui: {
      ...
      combobox: '.cs-field-write .combobox'
    },
    ...
    showOptions: function (event) {
      ...
      if (this.isCustomView && this.formView && this.formView.$el.find('.cs-tkl-options-container.binf-show').length) {
      ...
      this.ui.combobox.attr("aria-expanded", true);
    ...
    adjustPosition: function () {
      ...
      setTimeout(_.bind(function() {
        base.adjustDropDownField(inputEle, this.ui.tklOptionsContainer, true, this,
          ...
          this.isPerfectScrollbarEnabled && this.trigger('update:scrollbar');
      }, this), 200);
    },
    ...
    hideOptions: function (event) {
      if (!!event) {
        var isParentEleTKLField = event.target.parentElement && event.target.parentElement.classList.contains(
            'cs-tkl-options-container'),
            skipHiding = isParentEleTKLField &&
                         event.target.classList.contains('csui-normal-scrolling');
        if (this.isPerfectScrollbarEnabled) {
          var tklContainerEle = $('.cs-tkl-options-container:visible'),
              isDropDownOpened = tklContainerEle.length;
          skipHiding = isDropDownOpened && (tklContainerEle.find('.ps--clicking').length);
        }
        if (skipHiding) {
          event.preventDefault();
          event.stopPropagation();
      ...
      if (!(!!event && $(document.activeElement).is($(event.target))) ||
            (this.isCustomView && this.formView &&
             this.formView.$el.find('.cs-tkl-options-container.binf-show').length)) {
          $('.cs-tkl-options-container.binf-show').removeClass('binf-show');
        }
      this.ui.combobox.attr("aria-expanded", false);
      this.options.isDropDownOpen = false;
    ...
    filterRelations: function () {
      if (!this.relationsFiltered && this.mode !== 'writeonly') {
        ...
        var requiredSwitchEnabled = this.formView && this.formView.options &&
                                    this.formView.options.originatingView &&
                                    this.formView.options.originatingView.tabContentHeader &&
                                    this.formView.options.originatingView.tabContentHeader.requiredFieldSwitchModel &&
                                    this.formView.options.originatingView.tabContentHeader.requiredFieldSwitchModel.get(
                                        'data');
        ...
        _.each(this.parentViews, function (parent) {
          ...
          for (index = 0; index < views.length && !validParticipant; index++) {
            if (views[index].$el.is(':visible') || requiredSwitchEnabled) {
              validParticipant = true;
            }
          }
          if (validParticipant) {
    ...
    trySetValue: function (silentFlag) {
      ...
      if (bValid) {
        if ((this.curVal !== this.oldVal) || (this.curVal !== this.getEditValue())) {
        ...
        if (!this.valuesPulled || (this.isServerTypeahead && event && event.type === 'keyup')
          ...
          if (isValidRequest) {
            ...
            if (formData["typeAheadSearch"] &&
                (fromScroll || (!this.pageIndex && (event && event.type === 'keyup')))) {
            ...
            connector.makeAjaxCall(ajaxOptions).done(_.bind(function (attributeKey, resp) {
              if (this.isServerTypeahead) {
    ...
    refresh: function () {
      ...
      if (this.options.enum.length) { 
        ...
        if (!this.tklOptionsTemplate) {
          this.tklOptionsTemplate = _.template('<%_.each(options, function(option) {%>'+
                '<li id="<%_.uniqueId("tklitem")%>" role="option">'+
                  '<a title="<%-option%>" tabindex="-1"><%-option%></a>'+
                '</li>'+
            '<%})%>');
        }
    ...
    onClick: function (event) {
      ...
      if (this.parentViews.length) {
        _.each(this.parentViews, function (view) {
          view.isBooleanField && (self.valuesPulled = false);
        });
      }
    ...
    setSelection: function (el, showApplyAll, callFromInputValidation) {
      ...
      if (!this.isCustomView && this.node && this.node.get('subprocess_id')) {
        data.append("subwork_id", this.node.get('subprocess_id'));
      }
      
      this.connector.extendAjaxOptions(ajaxOptions);
      this.connector.makeAjaxCall(ajaxOptions).done(_.bind(function (resp) {
    ...
    autoPopulateChildrenValues: function (values, children, index, alreadyPopulated) {
      ...
      children.map(function (child) {
        ...
        if (this.isMultiFieldView(child)) {
          ...
          if (index === undefined) {
            child.children.map(function (child) {
              var childData = values[child.name];
              childData = childData !== null && childData !== undefined ? childData.toString() : '' ;
              if (child.getValue().toString() !== childData) {
                self._setTklFieldValue(child.fieldView, childData);
          ...
          if (child.children[index] && (child.children[index].name in values)) {
            var childValue = values[child.children[index].name];
            childValue = childValue !== null && childValue !== undefined ? childValue.toString() : '';
            if (child.children[index].fieldView.getValue().toString() !== childValue ) {
                child.children[index].fieldView.valuesPulled = false;
              self._setTklFieldValue(child.children[index].fieldView, childValue);
          ...
          if (!index) { // if mv parent and index > 0 don't change any dependent fields
            ...
            keyValue = keyValue !== null && keyValue !== undefined ? keyValue.toString() : '';
            if (child.getValue().toString() !== keyValue) {
              self._setTklFieldValue(child, keyValue);
            ...
            if (Array.isArray(child.children) && ((child.children[0] && child.children[0].cid) ===
                                                  (child.parentViews[0] &&
                                                   child.parentViews[0].cid))) {
              child.valuesPulled = false;
            }
          }
        }
    ...
    _setTklFieldValue: function (field, value) {
      if (!config.autofillChildrenValue && !!value) {
        value = '';
      }
