...
define(['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/backbone', 'csui/lib/marionette',
  ...
  var TextAreaFieldView = FormFieldView.extend({
    ...
    constructor: function TextAreaFieldView(options) {
      options || (options = {});
      this.isTextareaAutoHeight = options.isTextareAutoHeight ||  options && options.alpaca && options.alpaca.options && options.alpaca.options.isTextareaAutoHeight;
    ...
    templateHelpers: function () {
      var isReadOnly = this.mode === "readonly",
          ...
          label = "", value = "", isRequired = false, requiredTxt = "", multiFieldLabel = "",
          readModeAria = "", readModeMultiFieldAria = "",
      ...
      return _.extend(FormFieldView.prototype.templateHelpers.apply(this), {
        ...
        data: dataValue,
        isGlobeRequired: this.isMultilingualEnabled
      });
    },
    ...
    onRender: function () {
      ...
      var data = this.model && this.model.get('data');
      this.ui.writeField.val(this.curVal);
      if (_.isObject(data)) {
        data = this.textareaValue;
        this.ui.writeField.val(data);
      }
      if (data && data.trim().length !== 0) {
        this._applyEllipses();
      }
      if (this.isTextareaAutoHeight) {
        this.$el.addClass('csui-autoheight-textareafield');
        this.ui.writeField.attr('rows', 1);
        this.ui.readField.attr('title', "");
      }
    },
    ...
    adjustTextareaHeight: function () {
      var textarea = this.ui.writeField[0],
        scrollbarHeight, textareaHeight;
        if (textarea.value === "") {
          textarea.style.height = "";
          if (this.$el.closest('.binf-tab-content').length) {
          ...
          textarea.style.height = textareaHeight > scrollbarHeight ? textareaHeight : scrollbarHeight + "px";
        }
    },
