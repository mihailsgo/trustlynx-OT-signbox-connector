...
define(['module', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/handlebars',
  ...
  var FormArrayFieldEditable2Behavior = FormFieldEditable2Behavior.extend({
    ...
    constructor: function FormArrayFieldEditable2Behavior(options, view) {
      ...
      'button.csui-icon.apply-all:visible,span.cs-clear-icon:visible,span.cs-browse-icon:visible';
      ...
      propertyId += this.view.formView.cid;
    ...
    onFocusInWrite: function (event) {
      ...
      if(this.view.alpacaField.type === "radio" && inputLength > 1){
        ...
        if (this.view.options && this.view.options.alpacaField.type  === 'otcs_node_picker') {  
          if (this.ui.nodepickerIconsContainer.find(document.activeElement).length === 0) {
      ...
      if (((this.view.alpacaField && this.view.alpacaField.schema.type === "otcs_user_picker") ||
           ...
           (this.view.alpacaField && this.view.alpacaField.schema.type === "otcs_group_picker")) &&
    ...
    _showOrHideInlineActionButtons: function (event) {
      ...
      if (!this.isDropDownOpen()) {
        var csuiActions = containerEle.find(".cs-array .cs-pull-right");
        csuiActions.find(".circle_delete_grey").parent().addClass(
            "binf-hidden");
        csuiActions.removeClass('csui-set-action-bar');
        csuiActions.find(".circle_add_grey").parent().addClass(
            "binf-hidden");
    ...
    onFocusOutWrite: function (event) {
      ...
      if (this.view.options.path && this.view.options.path.split("/").length === 2) { //Multivalue
        ...
        if (this.view.alpacaField && this.view.alpacaField.options && this.view.alpacaField.options.mode !== 'create') {
    ...
    mouseInFieldArea: function (event) {
      ...
      if (currBlock.parent().hasClass("cs-form-multi-action-container")) {
        var activeElementParent = $(document.activeElement).parents('.alpaca-container-item:not(.cs-form-multi-action-container)'),
            allRows = currBlock.closest('.csui-form-set-array-wrapper .cs-form-set').children().not(activeElementParent);
        allRows.find(
            ".circle_add_grey").parent().addClass("binf-hidden");
        allRows.find(
            ".circle_delete_grey").parent().addClass("binf-hidden");
        allRows.find('.cs-pull-right').removeClass("csui-set-action-bar");
      }
    ...
    mouseOutFieldArea: function (event) {
      ...
      var currBlock = this.$el.closest(".cs-pull-left"),
          ...
          currSetBlock = currBlock.parents(".cs-pull-left").length > 0 ?
              currBlock.parents(".cs-pull-left") : currBlock,
          currFormsetContainer = this.$el.parents(".cs-form-set-container"),
          currSetBlockActionContainer = currSetBlock.siblings(".cs-pull-right");
      if ((currFormsetContainer.find(document.activeElement).length === 0 ||
          this.$el.find(document.activeElement).length === 0) &&
          currBlockActionContainer.find(document.activeElement).length === 0 && !currBlockActionContainer.hasClass('csui-set-action-bar')) {
        currBlockActionContainer.removeClass("csui-set-action-bar");
