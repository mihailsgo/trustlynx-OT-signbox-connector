...
define(['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/backbone',
  ...
  var FormFieldView = Marionette.ItemView.extend({
    ...
    constructor: function FormFieldView(options) {
      ...
      var labelEle = this.alpacaField && this.alpacaField.field && this.alpacaField.field.parent().find('label');
      labelEle && labelEle.length && labelEle.on('click', _.bind(this.onClickLabel, this));
    ...
    _render: function () {
      if (((!!this.alpacaField && this.alpacaField.options.mode !== "create") ||
           (this.model.get("options") && this.model.get("options").mode !== "create")) &&
          !this.silentRender) {
        this.render();
    ...
    onClickLabel : function(e) {
      ...
      !readonly && this.ui.readField.trigger(e.type);
    },
    ...
    setValue: function (value, validate, silent) {
      if (_.isFunction(this.preSetValue)) {
        value = this.preSetValue(value);
      }
      var bIsValid = true;
      var formValue = this.ui.writeField && value !== this.ui.writeField.val() ?
      ...
      if (bIsValid || !this.$el.is(":visible")) {
        this.$el.removeClass('cs-formfield-invalid');
        if (this.curVal === undefined || this.curVal === null) {
          silent = silent ? silent : value === "";
        }
        ...
        this.$el.find("input") && this.$el.find("input").select();
    ...
    _getPlaceHolder: function () {
      ...
      if (options && options.placeholder) {
        ...
        case 'integer':
          ...
          break;
        case 'text':
        case 'otcs_multilingual_textarea':
        case 'textarea':
        ...
        case 'otcs_member_picker':
          ...
          break;
        case 'otcs_group_picker':
          label = lang.alpacaPlaceholderOTGroupPicker;
          break;
        case 'otcs_user':
          label = this.model.get("schema").type === "otcs_user_picker" ?
                  lang.alpacaPlaceholderOTUserPicker :
                  (this.model.get("schema").type === "otcs_group_picker" ?
                   lang.alpacaPlaceholderOTGroupPicker : lang.alpacaPlaceholderOTUserGroupPicker);
          break;
