...
define(['module',
  'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/handlebars',
  'csui/lib/marionette', 'csui/controls/form/pub.sub', "csui/utils/log",
  'i18n!csui/controls/form/impl/nls/lang',
  'csui/utils/base'
], function (module, _, $, Handlebars, Marionette, PubSub, Log, lang, base) {
  ...
  var FormFieldEditable2Behavior = Marionette.Behavior.extend({
    ...
    constructor: function FormFieldEditable2Behavior(options, view) {
      ...
      this.tabContentSelectors = 'input:visible, .cs-field-write button:visible:not(:disabled),' +
                                 'textarea:visible, .icon-date_picker:visible, .csui-multilingual-icon:visible';
      ...
      if (!!this.view.options.alpacaField) {
        ...
        if (idToListen) {
          ...
          idToListen += this.view.formView.cid;
      ...
      this.view.listenTo(this.view.formView, 'update:form', _.bind(function () {
        this.trySetValue();
      }, this));
    ...
    events: {
      ...
      'focusout @ui.globeIcon': 'onFocusOutWrite',
      'focusout @ui.writeArea': 'onFocusOutWriteArea',
      'focusin @ui.writeField': 'onFocusInWrite',
      'focusout @ui.browseIcon': 'onFocusOutWrite',
      'focusin @ui.browseIcon': 'onFocusInWrite',
      'focusout @ui.clearIcon': 'onFocusOutWrite',
      'focusin @ui.clearIcon': 'onFocusInWrite',
      ...
      if (!!PubSub.tklHelpers && PubSub.tklHelpers.viewsInWriteMode.length > 0) {
        ...
        for (var i = PubSub.tklHelpers.viewsInWriteMode.length - 1; i >= 0 && !!PubSub.tklHelpers;
          ...
          if (!!targetView) {
            ...
            targetView.alpacaField.parent.getFieldEl().find('.csui-bulk-edit-cancel').trigger(
                'click') :
      ...
      if (event.keyCode === 27) { //escape
        if (this.view.getStatesBehavior().isStateWrite()) { // catch only in write state
          ...
          if (!this.isDropDownOpen()) {
            cancelEvent = !this.view.getStatesBehavior().isWriteOnly();
    ...
    _updateTabableElements: function (event) {
      ...
      if (this.view.alpacaField && this.view.alpacaField.type === "otcs_node_picker") {
        this.tabableElements = _.filter(this.tabableElements, function (item) {
          return !_.contains(item.classList, 'cs-nodepicker-input');
        });
      }
    },
    ...
    onFocusInWrite: function (event) {
      if(!(this.view && this.view.multiLingualForm && this.view.multiLingualForm.$popover && this.view.multiLingualForm.$popover.is(':visible'))) {
    ...
    onFocusOutWrite: function (event) {
      log.debug('onFocusOutWrite') && console.log(log.last);
      var bIsValid;
      ...
      if (!!this.view.selectField) {
        ...
        log.debug('onFocusOutWrite value {0}', event.target.value) && console.log(log.last);
        if (event.target.value !== '') {
          if (this.view.getItemPicked) {
            log.debug('onFocusOutWrite picked item {0}', this.view.getItemPicked()) &&
            console.log(log.last);
            this.view.alpacaField && this.view.alpacaField.refreshValidationState(false);
            if (!this.view.getItemPicked()) {
              this.$el.addClass('cs-formfield-invalid');
              var formValue= this.ui.writeField && this.ui.writeField.val();
              if (this.$el.hasClass('cs-formfield-invalid') && this.preVal !== formValue) {
                this.$el.find('input.typeahead').select();
                this.preVal= formValue;
              }
        ...
        if (this.view.mode !== 'writeonly') {
          this.view.cancelClicked = this.view.alpacaField ?
                                    this.isCancelClicked(this.view.alpacaField.parent) :
                                    this.view.cancelClicked;
          ...
          if (this.isRelatedFieldInFocus(event.relatedTarget) &&
            ...
            if ((!!this.view.getOldValue() &&
                 this.view.getOldValue().toString()) !==
                (this.view.getEditValue() && this.view.getEditValue().toString())) {
              this.changeChildrenValues(this.view.childTKLViews);
            } else {
    ...
    setViewStateWriteAndEnterEditMode: function (childFocus) {
      ...
      if ((this.view.isTextAreaField || this.view.isMultilingualField) &&
          (this.view.metadataLanguages && this.view.metadataLanguages.enabled)) {
    ...
    trySetValueAndLeaveEditMode: function (validate, focus) {
      var oldValue = this.view.getOldValue() !== null && this.view.getOldValue() !== undefined ?
                     this.view.getOldValue().toString() : '';
      this.changeChildrenValues(this.view.childTKLViews, this.view,
          oldValue !== this.view.getEditValue());
    ...
    _trySetValue: function () {
      ...
      if (this.view.alpacaField && this.view.alpacaField.type === "otcs_user") {
        bIsValid = this.view.getItemPicked && this.view.getItemPicked();
      }
      if (isReadyToSaveView && ((editVal !== curVal) || writeonly || hasClassInvalid ||
          (!writeonly && isRequired && validate && emptyField)) || this.view.mlDataChanged) {

        if (this.view.mlDataChanged) {
          var updatedMLData = this.view.formView.node &&
                              this.view.formView.node.get('description_multilingual');
          this.view.setValue(editVal, true, {silent:true});
          this.view.model.trigger('change', this.view.model, updatedMLData);
        } else {
    ...
    _resetOldValue: function () {
      if (!this.view.resetOldValueAfterCancel()) {
        ...
        if (!this.view.getItemPicked || this.view.getItemPicked() !== false) {
          curVal = curVal instanceof Object ? curVal.display_name : curVal;
          this.ui.writeField.val(curVal);
        }
        if (this.view.setItemPicked !== undefined) {
          this.view.setItemPicked(true);
        }
      }
