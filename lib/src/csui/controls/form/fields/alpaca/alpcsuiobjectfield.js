...
define(['module', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/jsonpath',
  'csui/lib/alpaca/js/alpaca.lite', 'i18n!csui/controls/form/impl/nls/lang', 'csui/utils/base',
  'i18n'
  ...
  Alpaca.Fields.CsuiObjectField = Alpaca.Fields.ObjectField.extend({
    ...
    _onFieldChanged: function (event) {
      ...
      var data = {
        ...
        fieldView: event.targetfieldview ?
                         event.targetfieldview : event.fieldView
      };
    ...
    restructureSetContainer: function (setEl, self, addItemIndex) {
      ...
      function removeWraps(isWriteOnlyMode, allChild) {
      ...
      $(setEl).children(":not('.binf-popover'):not('.csui-set-scroll-container-parent')").each(
          function (index, rowEl) {
            ...
            var lockedColumns               = [],
                ...
                childwithHiddenCol          = allChild.filter(function () {
                  if ($(this.children).hasClass('binf-hidden')) {
                    return true;
                  }
                }),
                ...
                popoutIcon                  = $('<span/>', {
                  ...
                  'data-toggle': 'popover',
                  'role': 'link',
                  'title': lang.titleColoutButton,
                  'aria-label': lang.titleColoutButton,
            ...
            if ( numberOfChildren && (numberOfChildren < 3)) {
              $(rowEl).closest(".alpaca-field-array").addClass('cs-single-field');
            }
            if (numberOfChildren && (numberOfChildren === $(rowEl).find('.csui-field-checkbox').length)) {
            ...
            if (!!allChild && allChild.length > 0) {
              if (!!$(rowEl).closest('.cs-formview-wrapper').length) {
                ...
                actualContainerWidth = Math.floor($(rowEl).closest('.cs-formview-wrapper').width() -
                                                  extraWidth + setbacks);
              } else if(!!$(rowEl).closest('.alpaca-container .cs-form-set').length){
                actualContainerWidth = Math.floor($(rowEl).parents('.alpaca-container-item').width());
              } else {
              ...
              if (childwithHiddenCol.length > 0) {
                hadHiddenColumns = true;
              }
              ...
              if (showScrollbarForElements) {
                hadHiddenColumns && removeWraps.call(this, isWriteOnlyMode, allChild);
                if (allChild.parent().find('.csui-show-colout').length === 0 &&
                  ...
                  var ind = _.findIndex(allChildwithoutCalout, function (el) {
                    return !$(el.children).hasClass('binf-hidden');
                  });
                  lockedColumns = isWriteOnlyMode ?
                                  allChildwithoutCalout.splice(0, (ind + 1)) :
                                  allChildwithCalout.splice(0, (ind + 2));
                ...
                removeWraps.call(this, isWriteOnlyMode, allChild);
                return;
              }
            }
            if (i18n && i18n.settings.rtl) {
