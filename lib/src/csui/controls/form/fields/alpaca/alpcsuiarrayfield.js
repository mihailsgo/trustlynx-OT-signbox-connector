...
define(['module', 'csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/jsonpath',
  'csui/lib/backbone', 'csui/lib/marionette', 'csui/lib/alpaca/js/alpaca.lite',
  ...
  Alpaca.Fields.CsuiArrayField = Alpaca.Fields.ArrayField.extend({
    ...
    addItem: function (itemIndex, itemSchema, itemOptions, itemData, callback) {
      ...
      this.base(itemIndex, itemSchema, itemOptions, itemData, _.bind(function () {
        if (this.options.mode === "create") {
          ...
          if (!this.options.isSetType) {
            this.field.find('.cs-form-multi-action-container .csui-icon.circle_add_grey').closest(
                'button').addClass('binf-hidden');
            this.field.find(
                '.cs-form-multi-action-container .csui-icon.circle_delete_grey').closest(
                'button').addClass('binf-hidden');
          }
        }
        callback();
    ...
    removeItem: function (itemIndex, callback) {
      ...
      this.base(itemIndex, _.bind(function () {
        ...
        if (currentElements.length === minItems) {
          if (this.options.isSetType && minItems === 0 || this.options.fields.item.type === "otcs_node_picker") {
            this._raiseValueChanged(this.getValue(), this.name, this.path,
                this.getValue(), this.name, this.path);
          }
        }
    ...
    postRender: function (callback) {
      ...
      var labelEle = this.field && this.field.parent().find('label');
      labelEle && labelEle.on('click', _.bind(this.onClickLabel, this));
    },

    onClickLabel : function(e) {
      !this.isWriteMode && this.children && this.children[0] && this.children[0]. fieldView &&
      this.children[0].fieldView.ui.readField.trigger(e.type);
    },
    ...
    _registerActionEvents: function() {
      if (this.options.mode === 'create') {
        var setContainerParent = this.getFieldEl().find(".cs-form-set-container").parents(
            ".cs-form-set-container");
        if (this.options.isSetType && setContainerParent.length === 0) {
    ...
    _registerBulkSubmitAction: function () {
      ...
      multiFieldBlock.off('keydown').off("focusout").on('focusout', function (event) {
       ...
       var focusoutInterval = that.schema.items.type === 'otcs_node_picker' ? 201 : 
       (that.schema.items.properties && that.schema.items.properties.classNumber && 
        that.schema.items.properties.classNumber.type)  === 'otcs_node_picker' ? 201 : 50;
       ...
       setTimeout(function () {
         if (multiFieldBlock.find(":focus").length === 0 &&
           ...
           if (that.getFieldEl().find('.cs-pull-left .cs-field-write:not(.csui-unedited)').is(
             ...
             if (!that.cancelClicked && !!(that.children && that.children.length > 0)) {
        ...
        var userfieldFocusFix = function (event) {
          if (that.schema.items.type === 'otcs_user_picker' ||
              ...
              that.schema.items.type === 'otcs_group_picker') {
          ...
          if (event.keyCode === 27) { // Escape: reset the block
            ...
            if (!that.options.isSetType) {
              ...
              if (childView.alpacaField.type === "select") {
                ...
                if (childView.getStatesBehavior().isStateRead() && childView.allowEditOnEnter()) {
                  ...
                  if (event.data(".csui-user-picker-no-results") && (that.schema.items.type ===
                    'otcs_member_picker' ||
                    that.schema.items.type ===
                    'otcs_user_picker' ||
                    that.schema.items.type ===
                  ...
                  else if (that.children.length > 0 && that.children[0].type !== 'tkl') {
                    var eventTargetElement;
                    if (that.schema && that.schema.items && that.schema.items.type === 'otcs_node_picker') {
                      eventTargetElement = $(event.target).parents('.cs-field-write-inner').find('input')[0];
                    } else {
                      eventTargetElement = event.target;
                    }
                    fieldView = that.children[that._getFieldIndex(eventTargetElement)].fieldView;
            ...
            if (that.isWriteMode || that.options.mode === 'create') {
              ...
              if (that.options.isSetType) {
                ...
                that.children[0].fieldView && that.children[0].fieldView.getStatesBehavior().isStateWrite() &&
      ...
      if (toWriteMode) { // change to write mode
        if (this.field.find(".csui-bulk-edit-cancel:visible").length === 0) {
          ...
          if (this.schema.items.type === 'otcs_user_picker' ||
              ...
              this.schema.items.type === 'otcs_group_picker') {
    ...
    _showItemsInWriteMode: function (itemIndex, setFlag) {
      ...
      if (!!setFlag && itemIndex !== undefined && itemIndex !== null) {
        ...
        if (this.schema && this.schema.items && this.schema.items.type === 'otcs_node_picker') {
          $(arrWriteObjElements.get(itemIndex)).find('.csui-icon:visible').trigger('focus');
        } else {
      ...
      $.each(elements, function (index, child) {
        if (!!child.fieldView) {
          if (child.options.customField || child.schema.type === "otcs_user_picker" ||
              ...
              child.schema.type === "otcs_group_picker" ||
            ...
            if (!!child.fieldView) {
              if (!$.isEmptyObject(child.fieldView._behaviors) &&
                ...
                if (child.fieldView.alpacaField.type === "select") {
                  ...
                  isFieldValid = child.fieldView.trySetValue(true);
                  ...
                  if (child.fieldView.getEditValue() !== child.fieldView.getValue() || 
                      child.fieldView.getEditValue() !== child.fieldView.getOldValue()) {
                ...
                if (isFieldValid || !child.fieldView.$el.is(":visible")) {
    ...
    _getFieldIndex: function (el) {
      ...
      if (index === -1) {
        var clearedNodepicker = this.containerItemEl.find('.cs-field-write').not('.csui-nodepicker-hasvalue');
        if (clearedNodepicker.find('input.cs-nodepicker-input').length > 0) {
          index = this.containerItemEl.find('.cs-field-write').index(clearedNodepicker);
        } else {
    ...
    updateToolbars: function () {
      ...
      if (base.isTouchBrowser()) {
         this.field.find('.cs-pull-left').trigger('click', undefined); // do nothing
      }
