...
define(['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/backbone', 'csui/lib/marionette',
  ...
  var SelectFieldView = Marionette.CompositeView.extend({
    ...
    constructor: function SelectFieldView(options) {
      ...
      this.deferLoad = Array.isArray(this.options.alpaca.schema.enumExtended);
    },
    ...
    _dropdownScroll: function (event) {
      ...
      if (desiredElement.length > 0) {
        ...
        !this.deferLoad && this.ui.dropdown.perfectScrollbar("update");
      }
    },
    ...
    templateHelpers: function () {
      var options, data,
          ...
          isRequired      = false,
          requiredTxt     = "",
          schema = this.options.model.get('schema');
      ...
      if(this.deferLoad && !hasData && this.model.get('id')) {
        selectedText = this.model.get('name');
        hasData = true;
      }
    ...
    _createSelectOption: function (text) {
      ...
      anchorElm.addEventListener("click", function (event) {
        ...
        self.model.set({ id: data, name: data });
      });
      return listElm;
    },
    ...
    onShowDropDown: function () {
      if (this.deferLoad) {
        var options = this.ui.dropdown[0].querySelectorAll('li');
        var isDeferLoadDone = options.length >= this.options.alpaca.schema.enumExtended.length + this.options.alpaca.schema.enum.length;
        if (!isDeferLoadDone) {
          for (var i = 0; i < this.options.alpaca.schema.enumExtended.length; i++) {
            var enumValue = this.options.alpaca.schema.enumExtended[i];
            var option = this._createSelectOption(enumValue);
            this.ui.dropdown[0].appendChild(option);
          }
        }
      }
      var scrollableCols = this.$el.closest('.csui-scrollable-writemode'),
      customSearchForm = this.$el.closest('.csui-custom-search-formitems');
      if (!!scrollableCols && !scrollableCols.hasClass('csui-dropdown-open')) {
        scrollableCols.addClass('csui-dropdown-open');
      }
      if (base.isTouchBrowser() && !!customSearchForm && !customSearchForm.hasClass('csui-dropdown-open')) {
        customSearchForm.addClass('csui-dropdown-open').css("overflow", "hidden");
      }
      this.ui.dropdown.removeClass('dropup-menu');
      if(this.deferLoad) {
        this.ui.dropdown.css({"overflow-y": "auto", "max-height": "auto"});
      } else {
    ...
    onHideDropDown: function () {
      ...
      if (!!customSearchForm && customSearchForm.hasClass('csui-dropdown-open')) {
        customSearchForm.removeClass('csui-dropdown-open');
      }
    ...
    hideDropdown: function (view) {
      ...
      if (view.isDropdownOpen()) {
        ...
        if (!!view.getEditableBehavior().showActions) {
          ...
          customSearchForm = view.$el.closest('.csui-custom-search-formitems');
          if (!!customSearchForm && customSearchForm.hasClass('csui-dropdown-open')) {
            customSearchForm.removeClass('csui-dropdown-open');
          }
          if (!!scrollableCols && scrollableCols.hasClass('csui-dropdown-open')) {
    ...
    trySetValue: function (validate) {
      this._setSelection(this.getValue());
      var isEditedValue = (this.getValue().id !== this.curVal.id),
          bValid        = isEditedValue;
      validate = validate ? validate : bValid;
      if (this.alpacaField) {
        bValid = this.alpacaField.setValueAndValidate(this.model.id, validate);
      }
      if (isEditedValue) {
        if (bValid) {
