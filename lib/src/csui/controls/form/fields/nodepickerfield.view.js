...
define([
  ...
  var NodePickerFieldView = FormFieldView.extend({
    ...
    ui: {
      ...
      nodepickerIconsContainer: '.csui-nodepicker-icon-container',
      browseIcon: '.cs-browse-icon',
      clearIcon: '.cs-clear-icon',
    ...
    events: {
      ...
      'click @ui.browseIcon': 'onClickWriteField',
      'keydown @ui.browseIcon': 'onClickWriteField',
      'click @ui.clearIcon': 'onClickClearIcon',
      'keydown @ui.clearIcon': 'onClickClearIcon',
    ...
    templateHelpers: function () {
      ...
      return _.extend(FormFieldView.prototype.templateHelpers.apply(this), {
        ...
        clearIconAriaLabel: lang.clearIconAriaLabel,
        clearIconTooltip: lang.clearIconTooltip,
        browseIconAriaLabel: lang.browseIconAriaLabel,
        browseIconTooltip: lang.browseIconTooltip
      });
    },
    ...
    onRender: function () {
      ...
      if (this._parent && this._parent.currentView && !this._parent.currentView.isCustomView) {
        setTimeout(_.bind(function () {
          var event = $.Event('tab:content:field:changed');
          this.$el.trigger(event);
          this.ui.browseIcon && this.ui.browseIcon.trigger("focus");
        }, this), 100);
      }
      this.$el.on('focusout', _.bind(function (event) {
        var self = this;
        setTimeout(function(){
          if (!self.$el[0].contains(document.activeElement)) {
    ...
    onClickClearIcon: function (event) {
      if (event.type === "keydown" && (event.keyCode === 13 || event.keyCode === 32) || event.type === "click") { // enter or space, click
      this.ui.writeArea.removeClass('csui-nodepicker-hasvalue');      
      this.editVal = "";
      this.setValue("", true);
      this.node.unset('id');
      this.model.set('data',{});
    ...
    _getNodePicker: function () {
      ...
      require(['csui/dialogs/node.picker/node.picker'], function (NodePicker, UserModelFactory) {
        ...
        var parameters           = alpOptions.type_control.parameters,
            dialogTitle          = parameters.dialogTitle || label,
            getUnSelectableNodes = function () {
              var unSelectableNodes = self.alpacaField && self.alpacaField.parent &&
                                      self.alpacaField.parent.data ?
                                      self.alpacaField.parent.data : undefined;
              unSelectableNodes = _.isArray(unSelectableNodes) && unSelectableNodes.length ?
                                  unSelectableNodes : [self.node];
              return unSelectableNodes;
            },
            nodePickerOptions    = {
              connector: self.connector,
              dialogTitle: _.str.sformat(lang.nodePickerDialogTitle, dialogTitle),
              ...
              unselectableNodes: getUnSelectableNodes(),
              resolveShortcuts: true,
              resultOriginalNode: true
            };
            _.extend(nodePickerOptions, parameters);
    ...
    setStateWrite: function (validate, focus) {
      ...
      if (this.getStatesBehavior().isWriteOnly() || this.getStatesBehavior().isStateWrite()) {
        ...
        if(!!focus) {
          if ($(this.ui.clearIcon).is(":visible")) {
            this.ui.clearIcon.trigger('focus');
          } else {
            this.ui.browseIcon.trigger('focus');
          }
    ...
    _prepareFieldText: function () {
      var ancestors = this.ancestors.pluck('name'), path;
      if (this.$el.width() > 0 && ancestors.length > 0) {
        ...
        path = truncatedAncestors.slice(-1)[0];
      }
      else {
        path = this.ancestors.pluck('name').slice(-1)[0];
      } 
      return path;
    },
    ...
    _renderNode: function () {
      ...
      this.ui.writeArea.addClass('csui-nodepicker-hasvalue');        
      var self = this;
      if (this.isValueChanged) {
        setTimeout(function () {
          if(self.options.alpacaField.isRequired()) {
            self.ui.browseIcon.trigger('focus');
          }
          else {
            self.ui.clearIcon.trigger('focus');
          }
