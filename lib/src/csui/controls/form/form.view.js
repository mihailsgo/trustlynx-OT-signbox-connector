...
define(['csui/lib/underscore',
  ...
  "csui/controls/form/fields/alpaca/alpcsuimultilingual.textareafield",
  ...
  var FormView = Marionette.ItemView.extend({
    ...
    render: function () {
      ...
      this._renderContent();
      if (this.options.context && this.options.context._applicationScope 
        && this.options.context._applicationScope.get('id') === "search") {
    ...
    _modifyModel: function () {
      ...
      var updateAlpFields = function (formObj, isField) {
        ...
        for (var idx in formObj) {
          if (formObj.hasOwnProperty(idx)) {
            ...
            if (_.isObject(formObj[idx])) {
              ...
              if (!!isField && !formObj[idx]['fieldKey']) {
                (formObj[idx]['fieldKey'] = idx);
              }

              if (objType === 'otcs_user_picker' || objType === 'otcs_member_picker' || objType ===
                  'otcs_group_picker') {
              ...
              updateAlpFields(formObj[idx], false);
            } else {
              objType = formObj[idx];
              if (idx === 'type') {
                if (objType === 'otcs_user_picker' || objType === 'otcs_member_picker' ||
                    objType === 'otcs_group_picker') {
      ...
      updateAlpFields(this.alpaca.options.fields, true);
    ...
    _splitFields: function () {
      ...
      var sortedData = _.sortBy(oOptions.fields, 'order'),
          rgKeysTemp = _.clone(memo.rgKeys),
          rgValsTemp = _.clone(memo.rgVals);
      !!_.findKey(oOptions.fields, 'order') && sortedData.forEach(function (item, idx) {
        var oldIdx = _.indexOf(rgKeysTemp, item.fieldKey);
        memo['rgKeys'][idx] = rgKeysTemp[oldIdx];
        memo['rgVals'][idx] = rgValsTemp[oldIdx];
      });
      return rgSplit;
    },
    ...
    validate: function (cb) {
      ...
      this.form.validate(bValidateChildren);
      var bRet = this.form.isValid(bValidateChildren);
      this.form.refreshValidationState(bValidateChildren, cb);
      return bRet;
    },
    ...
    hideNotRequired: function (bHide) {
      ...
      var targetToHide = applyToWholeForm ? this.$el : optionalFields,
          ...
          if(method ==='addClass'){
            targetToHide =  _.filter(targetToHide, function (el) {
              el = $(el).hasClass('binf-hidden') ? $(el).addClass('hidden-field') : el;
              return el;
            });
          } else {
            targetToHide =  _.filter(targetToHide, function (el) {
              el = !$(el).hasClass('hidden-field') ? el : undefined;
              return el;
            });
          }
          if(bHide !== undefined){
