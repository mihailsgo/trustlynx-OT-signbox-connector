...
define([
  ...
  'css!csui/controls/tableactionbar/impl/tableactionbar',
    ...
    FilteredToolItemsCollection, ToolbarStateBehavior, FlyoutToolItemView, CommandHelper,
    commands, DropdownMenuBehavior, highContrast, template, lazyLoadingTemplate,
    PerfectScrollingBehavior, iconRegistry, ToolItemModel) {
  'use strict';
  ...
  var TableActionBarView = Marionette.CompositeView.extend({
        ...
        constructor: function TableActionBarView(options) {
          ...
          this.renderLazyActions = options.hasOwnProperty('lazyActions') ?
        ...
        _layoutButtons: function () {
          ...
          if (itemViews.length > 0) {
            ...
            if (!!node && !lazyActionsRetrieved && node.nonPromotedActionCommands &&
                this.enabledNonPomotedCommands && this.renderLazyActions &&
              ...
              if (this.$childViewContainer.find('.binf-dropdown').length) {
                this.$childViewContainer.find('.binf-dropdown ul').first().append(lazyLoadingTemplate);
              } else {
        ...
        _wrapToolItemView: function (toolItemView, maxItemsShown) {
          if (!((toolItemView instanceof ToolItemView) ||
                (toolItemView instanceof FlyoutToolItemView))) {
            return;
          }
	      ...
	      this.actionbarOptions.prevToolItemView = toolItemView;
            ...
            if (isSeparator) {
              if (this.actionbarOptions.toolItemCounter >= maxItemsShown) {
                ...
                toolItemView.$el.addClass('binf-hidden');
              }
              ...
              if (this.actionbarOptions.toolItemCounter > maxItemsShown) {
                ...
                this.$el.find('.binf-dropdown-submenu').off('binf.dropdown.submenu.after.show').on(
                    'binf.dropdown.submenu.after.show', _.bind(function (event) {
                      var $dropdownMenu = this.$el.find('.binf-dropdown-submenu .binf-dropdown-menu');
                      if ($dropdownMenu.length) {
                        var $dropdownsubmenu     = this.$el.find(event.target),
                            node                 = this.model,
                            lazyActionsRetrieved = !!node.get('csuiLazyActionsRetrieved'),
                            isLocallyCreatedNode = !!node.isLocallyCreated;
                        if (!this.model.get('csuiLazyActionsRetrieved')) {
                          ...
                          if (!!node && !lazyActionsRetrieved && node.nonPromotedActionCommands &&
                              node.nonPromotedActionCommands.length && !isLocallyCreatedNode &&
                              this.enabledNonPomotedCommands && this.renderLazyActions) {
                            $dropdownMenu.first().append(lazyLoadingTemplate);
                            this._renderLazyActions(maxItemsShown).done(function () {
                              base.alignDropDownSubMenus({
                                targetEl: $dropdownsubmenu,
                                dropdownMenu: $dropdownsubmenu.children().last()
                              });
                            });
                          }
                        }
        ...
        _renderLazyActions: function (maxItemsShown) {
          var self               = this,
              node               = this.model,
              index              = 0,
              existingchildViews = _.sortBy(self.children.toArray()),
              derferred          = $.Deferred();
          !!node && node.setEnabledLazyActionCommands(true).done(_.bind(function () {
            ...
            var newCollection = new FilteredToolItemsCollection(
                self.options.collection.unfilteredModels, {
                  ...
                  addGroupSeparators: self.options.addGroupSeparators,
                  suppressGroupSeparators: self.options.suppressGroupSeparators
                });
              ...
              blockingEle.animate("width: 0", 300, function () {
                ...
                _.each(newCollection.models, function (model) {
                  ...
                  if (existingChildView && existingChildView.model.get('name') === itemName) { //re-render flyout toolitem view,
                    ...
                    if (self.actionbarOptions && !model.isSeparator()) {
                      if (self.options.addGroupSeparators &&
                          self.actionbarOptions.prevToolItemView &&
                          model.get('group') !==
                          self.actionbarOptions.prevToolItemView.model.get('group')) {
                        self._wrapToolItemView(
                            self.addChild(ToolItemModel.createSeparator(), ToolItemView),
                            maxItemsShown);
                      }
          ...
          dropdownToggler.on('binf.dropdown.after.show', function () {
            ...
            if ($dropdown.is('.binf-dropdown-align-left-top, .binf-dropdown-align-right-top')) { //dropup, added by base.autoAlignDropDowns
              css.maxHeight = Math.floor(window.innerHeight -
                              ...
                              3;
              if (!!that.model.get('csuiDelayedActionsRetrieved') && that.renderLazyActions &&
        ...
        _makeDropDown: function () {
          var cssClass = "binf-dropdown-toggle";
          var e = '<a role="button" href="#" tabindex="-1" class="' + cssClass +
