...
define([
  ...
  'css!csui/controls/table/inlineforms/inlineform/impl/inlineform'
    ...
    NodeCollection,
    CommandHelper,
    TabKeyBehavior, jsonPath, NodeCreateFormCollection) {
  ...
  var InlineFormView = Marionette.ItemView.extend({
        ...
        _templateHelpers: function () {
          ...
          var data = {
            ...
            CancelButtonLabel: !_.isUndefined(this.options.cancelTitle) ? this.options.cancelTitle :
                               lang.CancelButtonLabel,
          ...
          if (this.model.get('id') === undefined) {
            ...
            addItemMetadataCmd.execute(status, options)
              ...
              self.options.originatingView.$el.find(
                  '.csui-nodetable .csui-new-item a.csui-table-cell-name-value').first().trigger(
                  'focus');
            })
                .fail(function (err) {
                  self.cancel();
