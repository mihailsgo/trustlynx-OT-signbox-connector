...
define([
  ...
  '../../../../../utils/testutils/async.test.utils.js'
    ...
    TableView, tableColumns,TestUtils) {
  'use strict';
  ...
  describe('NodeStateCellView', function () {
    ...
    xit('focuses the first icon when entering the table cell', function (done) {
        TestUtils.asyncElement(this.tableView.$el, "tbody tr:first-child td:nth-child(2)").done(
          function (el) {
            el = el[0];
            el.tabIndex = -1;
            el.focus();
            ...
            pressKey('Enter', el);
            var reservationIcon = getReservationIcon(el);
            expect(reservationIcon.contains(document.activeElement)).toBeTruthy();
            done();
          });
    });
    xit('focuses the table cell when leaving the icons', function () {
