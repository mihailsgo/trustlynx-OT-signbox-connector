...
define([
  ...
  var ParentCellView = TextCellView.extend({
    ...
    constructor: function ParentCellView(options) {
      ...
      var parent = options.parent || this.model.get('parent_id_expand') || this.model.get('parent_id') ||
                   this.model.get('location_id_expand') || this.model.get('location_id');
      if (this.model && this.model.get("type") === 153) {
        parent = this.model.get('workflow_id_expand') || this.model.get('workflow_id');
      }
      this.parentModel = new NodeModel(parent, {connector: options.connector || this.model.connector});
  ...
  cellViewRegistry.registerByColumnKey('location_name', ParentCellView);
