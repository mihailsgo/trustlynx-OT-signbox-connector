...
define(['module',
  ...
  'csui/utils/focusable', // focusable
  "csui/controls/tile/behaviors/perfect.scrolling.behavior", // PerfectScrollingBehavior
  'csui/behaviors/keyboard.navigation/tabable.region.behavior', // TabableRegionBehavior
  "csui/controls/table/cells/cell.factory", // cellViewFactory
  "csui/models/nodechildrencolumn", // NodeChildrenColumnModel
  "csui/controls/progressblocker/blocker", // BlockingView
  'csui/utils/contexts/factories/connector', // ConnectorFactory
  'csui/behaviors/default.action/impl/defaultaction', // DefaultActionController
  'csui/controls/checkbox/checkbox.view', // CheckboxView
  'csui/controls/disclosure/disclosure.view', // DisclosureView
  'csui/controls/table/cells/select/select.view', // SelectCellView
  'csui/controls/table/cells/searchbox/searchbox.view', //SearchBoxView
  'csui/controls/table/cells/searchbox/typeahead.searchbox.view', // TypeaheadSearchBoxView
  'csui/controls/table/inlineforms/inlineform.factory', // inlineFormViewFactory
  'csui/controls/table/rows/metadata/metadatarow.view', // MetadataRowView
  'csui/controls/table/rows/error/errorrow.view', // ErrorRowView
  'csui/controls/table/impl/table.header.view', // TableHeaderView
  'csui/controls/table/impl/table.body.view', // TableBodyView
  'csui/utils/accessibility', // Accessibility
  'csui/controls/mixins/view.events.propagation/view.events.propagation.mixin', // ViewEventsPropagationMixin
  'csui/controls/mixins/view.state/node.view.state.mixin', // NodeViewStateMixin
  'csui/controls/toolbar/toolitems.filtered.model', // FilteredToolItemsCollection
  'csui/controls/toolbar/toolbar.command.controller', // ToolbarCommandController
  'csui/models/mixins/v2.fields/v2.fields.mixin', // FieldsV2Mixin
  'i18n!csui/controls/table/cells/toggledetails/impl/nls/lang', // toggleDetailsLang
  ...
  var TableView = Marionette.View.extend({
    ...
    constructor: function TableView(options) {
      options || (options = {});
      _.defaults(config, {
        minNumberOfToolbarItems: options.minNumberOfToolbarItems
      });
      _.defaults(config, {
        minNumberOfToolbarItems: 2
      });

      var focusView = options.focusView ? options.focusView : 'tableBody';
      this.accFocusedState = {focusView: focusView, headerColumn: 0, body: {column: 0, row: 0}};
      ...
      this.listenTo(this.collection, "reset", this._handleModelsReset)
          ...
          .listenTo(this.columns, 'update reset remain', this._handleColumnsReset);
      ...
      this.listenTo(this, 'show:descriptions', this.showDetailRowDescriptions);
    ...
    render: function () {
      ...
      if (this.table) {
        ...
        if (this._toggleExpandAllRegion) {
      ...
      if (this._extraWidthForToolbarItems === undefined) {
        ...
        if (this.options.inlineBar && this.options.inlineBar.viewClass) {
          ...
          if (inlineActionBarViewClass.estimateWidthForToolbarItems) {
            ...
            var minNumberOfToolbarItems = config.minNumberOfToolbarItems;
            var fontSize = 13; // todo: get the constant from configuration or constructor options
    ...
    _renderTable: function () {
      ...
      if (this._isSingleSelectionEnabled()) {
    ...
    _ensureFreshColumns: function () {
      if (this._tableColumnsResetTime > this._columnsComputeTime ||
    ...
    _getColumns: function () {
      ...
      var cols = [];
      if (this.options.tableColumns) { // client side columns
        ...
        this.columns.each(function (nodeColumnModel) {
          ...
          if (key === 'order') {
            nodeColumnModel.set('definitions_order', 1, {silent: true});
            order = 1;
          }
          ...
          if (nodeColumnModel.get("type") && order) {
            ...
            if (tableColumnToMergeWithServerColumn) {
              ...
              if (!keyRegistedInCellView) {  // key not registered
                ...
                log.debug('server column \'' + key + '\' has no corresponding column in client column list and no registered cell view') && console.log(log.last);
              }
            }
          }
        }, this);
        ...
        tableColumns.each(function (tableColumn) {
          ...
          if (columnModelsByKey[column_key]) {
            ...
            log.warn('Client side column with key \'' + column_key + '\' is not part of server columns') && console.log(log.last);
          }
        }, this);
        ...
        cols = _.map(sortedColumns, function (definition) {
          ...
          var column = {
            ...
            titleAttr: definition.get("titleAttr"),
            ...
            customHeader: definition.get('customHeader'),
        ...
        log.warn('No client side columns defined in this.options.tableColumns');
      }
    ...
    onKeyInView: function (event) {
      if (event.keyCode === 9) {
        if (this.activeInlineForm) {
          ...
          if ($(document.activeElement).parents('.csui-inline-editform').length===0) {
            var inlineInput = this.$el.find('.csui-inline-editform input')[0];
    ...
    _dataTablesOrderNeutral: function () {
      var orderBy = this.collection && this.collection.orderBy;
      if (this.table && orderBy && orderBy.length > 0) {
        orderBy = orderBy.split(' ');
        var columnName = orderBy[0],
            columnIndex = this._getColumnIndexFromColumnName(columnName);
        if (columnIndex === -1) {
          columnIndex = this._getColumnIndexFromSortKey(columnName);
        }
        if (columnIndex === -1) {
          this.table.orderNeutral();
        }
      }
    },
    ...
    _handleModelsUpdate: function (collection, options) {
      ...
      if (models.length > 0 && this.table) {
        ...
        this._drawTable();
        var descriptionAvailable = _.find(models, function (model) {
          if(model.get('description') && model.get('description').length > 0){
            return true;
          }
        });
    ...
    _handleModelRemove: function (model, collection, options) {
      if (!this.isRendered) {
        return;
      }
    ...
    updateRow: function (model) {
      ...
      if (changedKeys.length === 1 && changedKeys[0] === 'favorite') {
        return;
      }
      ...
      if (this.collection.length === 0) {
        ...
        var classNameToBeAdd = "csui-table-empty csui-table-empty-default" +
                               (!!this.canAddItemsToContainer && !!this.addItemsPermissions ?
                                " csui-can-add-items" : ""),
            emptyEl = $("<div class='" + classNameToBeAdd + "'><p class='csui-no-result-message'>" + 
                        this.emptyTableText + "</p></div>");
        emptyEl.attr('title',this.emptyTableText);
    ...
    setCustomLabels: function (otherLabels) {
      ...
      if (this.table) {
        ...
        if (differs && this.collection.length === 0) {
          this._drawTable();
        }
      }
    },
    ...
    _handleResponseSortBy: function () {
      ...
      if (sorting && sorting.sort && sorting.sort.length > 0 && sorting.sort[0].value) {
        ...
        var sortBy = (!!sortingColumnKey && sortingColumnKey.lastIndexOf('_formatted') === -1) ?
                     sortingColumnKey : this._getSortableColumnKey(sortKey);
        columnIndex = this._getColumnIndexFromColumnName(sortBy);
        if (columnIndex === -1) {
          columnIndex = this._getColumnIndexFromSortKey(sortBy);
        }
        if (this.table && columnIndex !== -1) {
          this.table.order([parseInt(columnIndex), sortDirection]);
        }
        var orderDiff = this.collection.setOrder(sortBy + ' ' + sortDirection, false);
        this.table && ensureTableHeaderSortArrow.call(this, this.table.order(), sortBy);
        if (sortingColumnKey && this.enableViewState && orderDiff) {
          var viewStateOrderBy = sortBy + ' ' + sortDirection;
          if (this.getViewStateOrderBy() !== viewStateOrderBy) {
            this.setViewStateOrderBy([viewStateOrderBy], {default: true});
        }
      }
      }
    },
    ...
    _adjustColumns: function () {
      ...
      if (this.collection.node && this.collection.node.get('type') === 136) {
        var orderColumn = _.findWhere(this.displayedColumns, {name: 'order'});
        orderColumn && (orderColumn.CellView.hasFixedWidth = true);
      }
        ...
        if (this.activeInlineForm) {
          ...
          if (firstRowModelId === inlineFormModelId) {
            if (this.collection.length > 1) {
              var lastEle = $(this.$el.find(
                  'table>tbody>tr:not(.csui-table-row-shows-inlineform) .csui-table-cell-_select')[0]).closest(
                  'tr');
              columnCells = lastEle.children();
            } else {
              columnCells = headerCells;
              this.$el.find('.csui-inlineform-parent').addClass('csui-inline-form-full-width');
            }
          } else {
    ...
    _getColumnIndexFromColumnName: function (columnName) {
      var columnIndex = -1;
    ...
    _getColumnIndexFromSortKey: function(sortKey) {
      var columnIndex = -1;
      this.displayedColumns && this.displayedColumns.some(function (cellValue, index) {
        if (cellValue.attributes['sort_key'] == sortKey) {
          columnIndex = index;
          return true;
        }
      }, this);
    ...
    getViewStateFilterAsObject: function (filterString) {
      var facetFilter = {};
      try {
        facetFilter = JSON.parse(filterString);
      } catch (e) {
      }
      return facetFilter;
    },
  ...
  function createTable() {
    ...
    var creationOptions = _.extend({
      ...
      drawCallback: function (oSettings) {
        ...
        if (accessibleTable) {
          $table.parent().addClass('csui-accessibility-table');
          $table.find('thead > tr')
              .children()
              .each(function () {
                var header = $(this);
                if (header.find('.csui-table-column-search').length) {
                  var wrapper = $('<div>', {class: 'csui-accessibility-table-th'});
        ...
        $table.find('thead th').each(function (index) {
          if (self.displayedColumns.length > index) {
            ...
            if(column.titleAttr) {
              var titleAttr = column.titleAttr;
              if(column.orderable) {
                titleAttr = $(this).attr('data-csui-column-next-sort-dir') === "desc" ? 
                            _.str.sformat(lang.columnSortDescendingTooltip, column.titleAttr) :
                            _.str.sformat(lang.columnSortAscendingTooltip, column.titleAttr);
              }
          ...
          if (subElement.attr('role') === 'button') {
            ...
            if (!currentAriaLabel) {
              ...
              var visibleColumnName = columnFound.titleAttr || columnFound.title;
      ...
      headerCallback: function (nHead, aData, iStart, iEnd, aiDisplay) {
          ...
          if (self.displayedColumns.length > index) {
            ...
            if (_.find(excludeTableHeaderCellsWithClass, function (classToCheck) {
              ...
              if (thEl.attr('aria-label') === undefined) {
                thEl.attr('aria-label', column.title? column.title : lang[column.name]);
              }
              ...
              if (headerEmpty) {
                ...
                if (column.customHeader){
                  column.customHeader.render();
                  contentEl.append(column.customHeader.$el);
                  column.customHeader.trigger('dom:added');
                }
        ...
        ensureTableHeaderSortArrow.call(self, this.api().order());
            ...
            if (displayInlineForm) {
              if (column.name === 'type') {
                ...
                if ((column.isNaming || column.name === 'name') && !tdForInlineForm) {
                  ...
                  if (column.name !== '_select' && column.name !== 'order') {  // skip select column and order column
              ...
              self.listenTo(cellView, 'clicked:checkbox', function (event) {
                if (aData.get('selectable') !== false) {
                  ...
                  if (event.checked === 'false') {
                    if (self._isSingleSelectionEnabled()) {
                        self._allSelectedNodes.reset();
                    }
                    tt.fnSelect(nRow);
                  } else {
                    tt.fnDeselect(nRow);
    ...
    var tableAria = this.options.tableAria;
    tableAria && tableEl.attr('aria-label', tableAria);
    if (!accessibleTable) {
    ...
    if (this._isSingleSelectionEnabled()) {
      selectTableRows.call(this, this._allSelectedNodes);
    }
    ...
    tableEl.on('order.dt', function () {
      if (self.displayedColumns) {
        ...
        _.each(self.table.order(), function (orderinfo) {
          ...
          var sortColumnKey = self.displayedColumns[sortColumnIndex].attributes.column_key;
          if (sortColumnKey) {
            if (orderinfo[1]) {
  ...
  function ensureExpandAllDetailRowsColumn(nHead) {
    ...
    if (this.options.haveDetailsRowExpandCollapseColumn &&
        this.options.haveToggleAllDetailsRows && this.collection.length) {
      var expandAllHeaderCell = $("th.csui-table-cell-_toggledetails", nHead);
      if (expandAllHeaderCell.length > 0 && !this._toggleExpandAllRegion) {

        var toggleExpandAllView = new DisclosureView({
          titleDisclosed: toggleDetailsLang.expandAllTooltip,
          titleExpanded: toggleDetailsLang.collapseAllTooltip,
          ariaLabelDisclosed: toggleDetailsLang.expandAllAria,
          ariaLabelExpanded: toggleDetailsLang.collapseAllAria,
            });
        this.listenTo(toggleExpandAllView.model, 'change:expanded', toggleDetailsRow);
        this._toggleExpandAllRegion = new Marionette.Region({el: expandAllHeaderCell});
        this._toggleExpandAllRegion.show(toggleExpandAllView);
      }
    }
  }
  ...
  function ensureTableHeaderSortArrow(order, sortKey) {
        ...
        if (self.displayedColumns.length > index) {
          ...
          if (searchOn > -1 || self.displayedColumns[index].filterable === true) {
            ...
            searchbox.off('opened').on('opened', function () {
              ...
              $(thead).removeAttr('title');
              $(thead).attr('aria-label', columnTitle);
            ...
            searchbox.off('closed').on('closed', function () {
              ...
              if ($(thead).hasClass('sorting_asc')) {
                $(thead).attr('title', lang.NodeTableSortDescendingTooltip);
                $(thead).attr('aria-label', _.str.sformat(lang.NodeTableSortedAscendingColumnAria, columnTitle));
              } else if ($(thead).hasClass('sorting_desc')) {
                $(thead).attr('title', lang.NodeTableSortAscendingTooltip);
                $(thead).attr('aria-label', _.str.sformat(lang.NodeTableSortedDescendingColumnAria, columnTitle));
              } else {
                $(thead).attr('title', lang.NodeTableSortAscendingTooltip);
              }
  ...
  function selectTableRows(nodes) {
    nodes && nodes.forEach(function(node) {
      selectRowsByNodeIds.call(this, node.get('id'));
    }.bind(this));
  }
  ...
  function freeObjects() {
    destroyProcessingPanel.call(this);
    cleanUpTableEventListeners.call(this);
    this._destroyTableHeaderAndBodyViews();
  }

  function destroyTable() {

    freeObjects.call(this);
  ...
  function setOrder(aoData, oSettings) {
    ...
    _.each(oSettings.oInstance.api().table().order(), function (orderinfo) {
      var sortColumnIndex = orderinfo[0];
      var sortColumnKey = columns[sortColumnIndex].attributes.column_key;
      if (sortColumnKey) {
        if (orderinfo[1]) {
          orderBy.push(sortColumnKey + " " + orderinfo[1]);
        } else {
    ...
    if (this.enableViewState) {
      ...
      orderBy.length > 0 && this.setViewStateOrderBy(orderBy, {default: makeItDefault});
    }
