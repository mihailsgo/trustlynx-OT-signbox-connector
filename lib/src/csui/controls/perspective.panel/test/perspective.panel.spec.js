...
define([
  ...
  describe('PerspectivePanelView', function () {
    ...
    it('switching to the default container page makes 6 server calls', function (done) {
      checkBlockingView();
      checkEventFlow();
      checkAnimationEffect(done);
      checkAJAXCallCount(6);
      nextNode.set({id: 2000});
    });

    it('reloading the default container page makes 7 server calls', function (done) {
      reloadPerspective();
      checkBlockingView();
      checkEventFlow();
      checkAJAXCallCount(7);
