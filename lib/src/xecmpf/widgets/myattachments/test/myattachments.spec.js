...
define(['csui/lib/marionette', 'csui/lib/jquery', 'csui/utils/connector',
...
], function (Marionette, $, Connector, MyAttachmentsView, PageContext, NodeModelFactory, MockData,
             GlobalMessage) {
    'use strict';
    describe('Test MyAttachmentsView', function () {
        ...
        it('displays 9 business attachment', function (done) {
            setTimeout(function () {
                expect(tableView.$el.find("tr.odd").length + tableView.$el.find("tr.even").length).toEqual(9);
                done();
            }, 300);
        });
        ...
        describe('sort business attachments - ', function () {
            it('click on sort of name', function (done) {
                var sortArrow = tableView.$el.find('thead tr th[data-csui-attribute=name][title="Name - Click to sort ascending"]')
        ...
        afterAll(function () {
            ...
            $('body').empty();
            MockData["testMyAttachments"].disable();
        });
    });
