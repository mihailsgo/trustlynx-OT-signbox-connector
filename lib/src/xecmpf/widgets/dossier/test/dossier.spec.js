    ...
    describe('Dossier Widget', function () {
        ...
        describe('List view of the widget', function () {
            it('can be instantiated and rendered', function (done) {
                ...
                dossierView.completeCollection.fetch(dossierView.options);
        ...
        describe('Dossier View Tile Header', function () {
            it('verifies the tile header of the widget', function (done) {
                var tileHeader = dossierView.$el.find('.tile-header .dossier-properties');
        ...
        describe('Grouped by Classification', function () {
            it('is grouped by Classification', function (done) {
                var groupBy = $(dossierView.$el.find('.tile-header')[0]).find('.tile-controls').find('.dossier-dropdown-wrapper').children()[0].innerHTML;
                expect(groupBy.trim()).toEqual('Group by');

                var grpByClassification = $(dossierView.$el.find('.tile-header')[0]).find('.tile-controls').find('.dossier-dropdown-wrapper').find('.dossier-dropdown').find('.xecmpf-dropdown').find('.binf-dropdown-toggle').children()[0].innerHTML;
                expect(grpByClassification.trim()).toEqual('Classification');
