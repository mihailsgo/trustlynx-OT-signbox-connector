...
define([
  ...
  'nuc/models/node/node.model', 'nuc/models/node.children2/node.children2',
  'nuc/utils/connector', 'nuc/models/utils/v1tov2', './node.children2.mock.js'
  ...
  describe('NodeChildrenCollection', function () {
    ...
    describe('when cloned', function () {
      ...
      it('does clone the autoreset status', function () {
        var original = new NodeChildrenCollection({id: 2000}, {
          node: container,
          expand: {properties: []}
        });
        original.autoreset = true;
        var clone = original.clone();
        expect(clone.autoreset).toBeTruthy();
      });

      it('does clone the useSpecialPaging status', function () {
        var original = new NodeChildrenCollection({id: 2000}, {
          node: container,
          expand: {properties: []}
        });
        original.useSpecialPaging = true;
        var clone = original.clone();
        expect(clone.useSpecialPaging).toBeTruthy();
      });
