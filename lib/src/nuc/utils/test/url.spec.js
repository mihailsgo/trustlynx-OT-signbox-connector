...
define(['nuc/utils/url'], function (Url) {
  'use strict';
  ...
  describe('Url', function () {
    ...
    describe('isAbsolute', function () {
      it('works with default path', function () {
        expect(this.url.isAbsolute()).toBeTruthy();
      });

      it('works with a local file', function () {
        var url = new Url('file://irgendwas/irgendwo');
        expect(url.isAbsolute()).toBeTruthy();
      });

      it('works with local path', function () {
        var url = new Url('/lala');
        expect(url.isAbsolute()).toBeFalsy();
      });
    ...
    describe('getAbsolute', function () {
      it('works with default Url', function () {
        expect(this.url.getAbsolute()).toEqual('//server/otcs/cs/api/v1');
      });

      it('works with empty string', function () {
        var url = new Url('');
        expect(url.getAbsolute()).not.toEqual('null'); // would be the case in firefox if delivered as file://
      });
    });
