...
define([
  'nuc/lib/underscore', 'nuc/utils/basicauthenticator', 'nuc/utils/connector',
  'nuc/models/node/node.model', './connector.mock.data.js'
  ...
  describe('Connector', function () {
    ...
    describe('makeAjaxCall', function () {
      ...
      it('makeAjaxCall adds the properties passed in the options.data object to the url', function (done) {
        var options = {
          type: 'GET',
          url: '//server/otcs/cs/api/v2/nodes/2345',
          data: 'testName=testValue'
        };
        expect(options.url).toEqual('//server/otcs/cs/api/v2/nodes/2345');

        connector
          .makeAjaxCall(options)
          .done(function (response) {
            expect(options.url).toEqual('//server/otcs/cs/api/v2/nodes/2345?testName=testValue');
            done();
          });
      });
    });
