 ...
 * 0.1.10+ (Oct 31, 2017 commit)
 * Guy Bedford 2014
 * MIT
 */

define(['require', 'nuc/lib/normalize'], function(req, normalize) {
  var cssAPI = {};

  var isWindows = !!process.platform.match(/^win/);

  function compress(css, fileName) {
    if (config.optimizeCss == 'none' || config.compressCSS === false) {
      return css;
    }
    ...
    if (typeof process !== "undefined" && process.versions && !!process.versions.node && require.nodeRequire) {
      ...
      try {
        ...
        console.log('Compressing ' + fileName + ' failed due to a CSS syntax error.');
        return css;
      }
      console.log('Compressed ' + fileName + ' to ' + Math.round(css.length / csslen * 100) + '%.');
      return css;
    }
  ...
  cssAPI.onLayerEnd = function(write, data) {
    ...
    if (config.separateCSS) {
      ...
      process.nextTick(function() {
        ...
        // Normalize the URLs once more, because the separate CSS file can be placed
        // next to the Require.js output module; not to the outDir, which the first
        // normalization applied to single files used as a base directory.
        css = normalize(css, siteRoot, outPath);
        saveFile(outPath, compress(css, outPath));
      });
