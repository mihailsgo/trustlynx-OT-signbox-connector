...
define(['csui/lib/underscore', 'csui/lib/jquery',
  ...
  describe('RelatedWorkspacesExpandedViewTest', function () {
    ...
    function testsetup() {
        ...
        node = pageContext.getModel(NodeModelFactory, { attributes: { id: 2513449, "type": 848 } });
        pageContext.options.factories.node = node;
        ...
        if (expandedView instanceof RelatedWorkspacesTableView) {
          ...
          expandedView.paginationRegion.once("show",function(){
            var prerequisites = {
                "add-menu": function() { return expandedView.$el.find('.csui-addToolbar .csui-icon-v2__csui_action_add32').length; },
    ...
    describe('CommonCheck', function () {
      ...
      it('can be created', function (done) {
        ...
        var ariaLabel = expandedView.$('table').attr('aria-label');
        expect(ariaLabel).toBe(expandedView.options.tableAria);
        expect(ariaLabel).toBe("Content of " + expandedView.options.title);
        done();
      });

      it('add menu available', function (done) {
        addCommandElement = expandedView.$el.find('.csui-addToolbar .csui-icon-v2__csui_action_add32');
        ...
        addCommandElement = expandedView.$el.find('.csui-addToolbar [data-csui-command="addrelation"] .csui-icon-v2__csui_action_add32');
    ...
    describe("default action", function () {
      ...
      it("is called at onClick", function (done) {
          var defaultAction       = expandedView.tableView.$(".csui-table-cell-name-value"),
              defaultActionCalled = false;

          expandedView.tableView.listenTo(expandedView.tableView, 'execute:defaultAction', function () {
            defaultActionCalled = true;
          });
          expect(defaultActionCalled).toBeFalsy();
          $(defaultAction[0]).trigger('click');
          expect(defaultActionCalled).toBeTruthy();

          done();
      });

      it("is not available with no 'SeeContent' permission", function (done) {
        var defaultAction       = expandedView.tableView.$(".csui-table-cell-name-value"),
            defaultActionCalled = false;
        ...
        expect(defaultActionCalled).toBeFalsy();
        $(defaultAction[29]).trigger('click');
        expect(defaultActionCalled).toBeFalsy();

        expect($(defaultAction)[29].getAttribute("class")).toEqual("csui-table-cell-name-value csui-table-cell-no-default-action");
