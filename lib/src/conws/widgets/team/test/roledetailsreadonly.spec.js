...
define([
  ...
  'csui/utils/log'
], function ($, Marionette, PageContext, TeamCollectionFactory, ModalDialogView, RoleDetailsView,
  ...
  describe('RoleDetails readonly', function () {
    ...
    beforeEach(function () {
      ...
      $("body").append('<div id="test_team_view" style="width: 1264px; height: 632px;"></div>');
      region = new Marionette.Region({el:"#test_team_view"});
      TestManager.reset();
      TestManager.init(id);
    ...
    describe('view renders', function () {
      ...
      it('a team with just one role and no member.', function (done) {
        ...
        TestUtil.run(done,function (done) {
          ...
          expect(roleName.text()).toEqual('Staff Team lead',"roleName.text");
      ...
      afterEach(function () {
        ...
        function prepareTabbables(done, number) {
          ...
          TestUtil.run(done,function(done) {
            ...
            if (tabbables) {
              log.info("prepare tabbables (A), skip {0} {1}",number,count) && console.log(log.last);
              return;
            }

            count = count + 1;
            log.info("prepare tabbables (A), try {0} {1}",number,count) && console.log(log.last);
          ...
          TestUtil.run(done,function(done) {
            ...
            var found;
            TestUtil.waitFor(done, function( ) {
              found = view.$(':not(.ps-scrollbar-x):not(.ps-scrollbar-y):tabbable');
              if (found.length>=3) {
                tabbables = found;
                return true;
              }
        ...
        TestUtil.run(done,function (done) {
          ...
          var elem = view.$(tabbables.filter(".conws-role-details-member"));
          expect(elem.length).toEqual(1,"elem.hasClass conws-role-details-member");
          expect(elem.find('.member-name').text()).toEqual('Famous Actors',"elem with class member-name text");
          elem = view.$(tabbables.filter('.conws-roles-edit-button'));
          expect(elem.length).toEqual(1,"elem.hasClass conws-roles-edit-button");
          expect(elem.text()).toEqual('Close',"elem.text");
          elem = view.$(tabbables.filter(".conws-role-details-member")).trigger('focus');
          expect(elem.find('.member-name').text()).toEqual('Famous Actors',"elem with class member-name text");
        });
      ...
      it('(B) with TAB and more than 15 participants', function (done) {
        ...
        function prepareTabbables(done, number) {
          ...
          TestUtil.run(done,function(done) {
            ...
            if (tabbables) {
              log.info("prepare tabbables (B), skip {0} {1}",number,count) && console.log(log.last);
              return;
            }

            count = count + 1;
            log.info("prepare tabbables (B), try {0} {1}",number,count) && console.log(log.last);
        ...
        TestUtil.run(done,function (done) {
          ...
          var elem = view.$(tabbables.filter(".icon-search"));
          expect(elem.length).toEqual(1,"elem.hasClass icon-search");
          expect(elem.attr('title')).toEqual('Participants',"elem.attr title");
          elem = view.$(tabbables.filter(".conws-role-details-member"));
          expect(elem.length).toEqual(1,"elem.hasClass conws-role-details-member");
      ...
      it('(C) with TAB and activated filter', function (done) {
        ...
        function prepareTabbables(done, number) {
          ...
          TestUtil.run(done,function(done) {
            ...
            if (tabbables) {
              log.info("prepare tabbables (C), skip {0} {1}",number,count) && console.log(log.last);
              return;
            }

            count = count + 1;
            log.info("prepare tabbables (C), try {0} {1}",number,count) && console.log(log.last);
        ...
        TestUtil.run(done,function (done) {
          ...
          expect(elem.length).toEqual(1,"elem.hasClass conws-roles-edit-button");
          expect(elem.text()).toEqual('Close',"elem.text");
          view.$(tabbables.filter(".icon-search")).trigger('click');
        });
        TestUtil.waitFor(done,function () {
          tabbables = view.$(':not(.ps-scrollbar-x):not(.ps-scrollbar-y):tabbable');
          return (tabbables.length>=5);
        }, 'Failed to enable filter input element!', 500);
        ...
        TestUtil.waitFor(done,function () {
          tabbables = view.$(':not(.ps-scrollbar-x):not(.ps-scrollbar-y):tabbable');
          return (tabbables.length>=6);
        }, 'Failed to enable filter clear element!', 500);

        TestUtil.run(done,function (done) {
          tabbables = view.$(':not(.ps-scrollbar-x):not(.ps-scrollbar-y):tabbable');
          expect(tabbables.length).toEqual(6,"tabbables.length");
          var elem = view.$(tabbables.filter(".search"));
          expect(elem.length).toEqual(1,"elem.hasClass search");
          expect(elem.attr('title')).toEqual('Search participants',"elem.attr title");
          elem = view.$(tabbables.filter(".sbclearer"));
          expect(elem.length).toEqual(1,"elem.hasClass sbclearer");
          expect(elem.attr('title')).toEqual('Clear filter',"elem.attr title");
          elem = view.$(tabbables.filter(".icon-search"));
          expect(elem.length).toEqual(1,"elem.hasClass icon-search");
          expect(elem.attr('title')).toEqual('Participants',"elem.attr title");
          elem = view.$(tabbables.filter(".conws-role-details-member"));
          expect(elem.length).toEqual(1,"elem.hasClass conws-role-details-member");
      ...
      it('(D) with DOWN / UP in participants list', function (done) {
        ...
        function prepareTabbables(done, number) {
          ...
          TestUtil.run(done,function(done) {
            ...
            if (tabbables) {
              log.info("prepare tabbables (D), skip {0} {1}",number,count) && console.log(log.last);
              return;
            }

            count = count + 1;
            log.info("prepare tabbables (D), try {0} {1}",number,count) && console.log(log.last);
