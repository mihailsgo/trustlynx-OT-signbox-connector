...
define([
  ...
  xdescribe('Team Tables Cells', function () {
    var contentRegion;
    var view;
    ...
    describe('Participants team tables cells and columns', function () {
      ...
      describe('Table row cells 1', function () {
        ...
        describe('Name cell view', function () {
          ...
          it('renders correctly depending on the provided properties', function (done) {
            ...
            TestUtil.run(done,function () {
              ...
              var cbronson = view.$(rows[2]).find('td[data-csui-attribute="conws_participantname"]' +
                                                  ' .csui-name');
              expect(cbronson.length).toEqual(1);
              expect(cbronson.attr('title')).toEqual('Bud Spencer');

              var lfunes = view.$(rows[4]).find('td[data-csui-attribute="conws_participantname"]' +
                                                ' .csui-name');
              expect(lfunes.length).toEqual(1);
          ...
          it('renders correctly including roles indicator', function (done) {
            ...
            TestUtil.run(done,function () {
              ...
              expect(rows.length).toEqual(4);
               var bspencerRole = view.$(rows[0]).find('td.team-table-cell-rolename');
               expect(bspencerRole.length).toEqual(1);
               expect(bspencerRole.find(
                '.role-name-box > .role-name-name-cell > span').length).toEqual(1);
               expect($(bspencerRole.find('.role-name-box > .role-name-name-cell > span')[0]).text()).toEqual(
                   'Famous Actors');
               var bspencerParticipants = view.$(rows[0]).find('td.team-table-cell-participants');  
               expect($(bspencerParticipants.find('span')).text()).toEqual('Bud Spencer +1');
              var lcaprioRole = view.$(rows[1]).find('td.team-table-cell-rolename');
              expect(lcaprioRole.length).toEqual(1);
              expect(lcaprioRole.find(
                '.role-name-box > .role-name-name-cell > span').length).toEqual(1);
              expect($(lcaprioRole.find('.role-name-box > .role-name-name-cell > span')[0]).text()).toEqual(
                  'Looser');
              var lcaprioParticipants = view.$(rows[1]).find('td.team-table-cell-participants');  
              expect($(lcaprioParticipants.find('span')).text()).toEqual('Leonardo Di Caprio');
              var bwillisRole = view.$(rows[2]).find('td.team-table-cell-rolename');
              expect(bwillisRole.length).toEqual(1);
              expect(bwillisRole.find(
                  '.role-name-box > .role-name-name-cell > span').length).toEqual(1);
              expect($(bwillisRole.find('.role-name-box > .role-name-name-cell > span')[0]).text()).toEqual(
                    'Staff');
              var bwillisParticipants = view.$(rows[2]).find('td.team-table-cell-participants');  
              expect($(bwillisParticipants.find('span')).text()).toEqual('Bruce Willis +2');
             bwillisRole = view.$(rows[3]).find('td.team-table-cell-rolename');
             expect(bwillisRole.length).toEqual(1);
             expect(bwillisRole.find(
                 '.role-name-box > .role-name-name-cell > span').length).toEqual(2);
             expect($(bwillisRole.find('.role-name-box > .role-name-name-cell > span')[0]).text()).toEqual(
                   'Super Heroes');
              expect($(bwillisRole.find('.role-name-box > .role-name-name-cell > span')[1]).text()).toEqual(
                    'Team lead');
             bwillisParticipants = view.$(rows[3]).find('td.team-table-cell-participants');  
             expect($(bwillisParticipants.find('span')).text()).toEqual('Bruce Willis');
        ...
        describe('Login cell view', function () {
          ...
          it('renders correctly for users and groups', function (done) {
            ...
            TestUtil.run(done,function () {
              ...
              var superHeroes = view.$(rows[2]).find('td[data-csui-attribute="conws_participantlogin"]');
        ...
        describe('Email cell view', function () {
          ...
          xit('renders correctly with or without email', function (done) {
            TestManager.createRoles([
              {id: 101, name: 'Staff', leader: true}
            ]);
            TestManager.createMembers([
        ...
        describe('Department cell view', function () {
          ...
          it('renders correctly for users and groups', function (done) {
            ...
            TestUtil.run(done,function () {
              ...
              var superHeroes = view.$(rows[2]).find('td.team-table-cell-department');
              expect(superHeroes.length).toEqual(1);
              expect(superHeroes.find('span').text()).toEqual('');
            });
          });
