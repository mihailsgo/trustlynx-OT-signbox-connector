  ...
  describe('Team Tables Toolbars', function () {
    ...
    describe('Participants team tables toolbar and commands', function () {
      ...
      describe('Toolbar', function () {
        ...
        it('is rendered correctly when the participants list is empty and a editable role is available.', function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            expect(command.find('>svg').hasClass('csui-icon-v2__csui_action_add32')).toBeTruthy("csui-icon-v2__csui_action_add32");
          });
        });
        ...
        it('is rendered correctly when a single participant is selected', function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            var buttons = view.$('#participantsview tbody > tr > td.csui-table-cell-_select').find('button');
            view.$(buttons[0]).trigger('click');
            var toolbar = view.$('#participantstoolbar');
            expect(toolbar.length).toEqual(1,"toolbar.length");
            ...
            expect(command.find('>svg').hasClass('csui-icon-v2__csui_action_download32')).toBeTruthy("csui-icon-v2__csui_action_download32");
      ...
      describe('Commands', function () {
        ...
        it('RemoveParticipant', function (done) {
          ...
          TestUtil.waitFor(done,function () {
            var confirmed = ($.find('.csui-alert .binf-modal-dialog').length === 0);
          ...
          TestUtil.run(done,function (done) {
            ...
            TestUtil.waitFor(done,function () {
              return $.find('.csui-alert .binf-modal-dialog .binf-modal-content').length > 0;
            }, 5000);
    ...
    describe('Roles team tables toolbar and commands', function () {
      ...
      describe('Toolbar', function () {
          ...
          function createAndShowTeamTablesRoles(){
        ...
        it('is rendered correctly when the roles list is empty', function (done) {
          TestUtil.awaitPromise(done,createAndShowTeamTablesRoles(),"roles table shown");
          TestUtil.run(done,function () {
        ...
        it('is rendered correctly when a single role is selected', function (done) {
          ...
          TestManager.addMembers('Staff', ['bspencer']);
          TestUtil.awaitPromise(done,createAndShowTeamTablesRoles(),"roles table shown");
          view.tabRolesRegion.currentView.$el.trigger('click');
          TestUtil.run(done,function () {
            var rows = view.$('#rolesview tbody > tr > td.csui-table-cell-_select');
            var buttons = view.$('#rolesview tbody > tr > td.csui-table-cell-_select').find('button');
            view.$(buttons[0]).trigger('click');
            var toolbar = view.$('#rolestoolbar');
            expect(toolbar.length).toEqual(1,"toolbar.length");
            var commands = toolbar.find('li:not(.binf-divider)');
            ...
            expect(command.find('>svg').hasClass('csui-icon-v2__csui_action_download32')).toBeTruthy();
