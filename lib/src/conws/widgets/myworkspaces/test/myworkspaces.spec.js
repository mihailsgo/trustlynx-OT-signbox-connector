...
define(['csui/lib/jquery', 'csui/lib/underscore',
  ...
  var selectorViewItems                    = '.binf-list-group-item',
      ...
      selectorDefaultContainerIcon         = '.csui-icon-group',
      selectorUIIcon                       = '.csui-icon',
      selectorTitle                        = '.tile-title',
      selectorListItemTitle                = '.list-item-title',
      selectorTitleIcon                    = ".tile-type-image",
      selectorDefaultTitleIcon             = ".csui-icon-v2-view.tile-type-icon.tile-type-icon-img",
      ...
      selectorNoDefaultAction              ='.csui-table-cell-no-default-action',
  ...
  describe('MyWorkspacesView', function () {
    ...
    function prepareCollapsedView(done,totalCount) {
      ...
      $.mockjax.clear();
      DataManager.test(totalCount, title, workspaceTypeId, defaultAction, false);
      view.render();
      $("body #my_wksp_view").empty().append(view.el);
    ...
    afterEach(function(){
      ...
      $("body #my_wksp_view_1").remove();
    });

    beforeEach(function () {
      width = $("body").css('width');
      $("body").css('width', '1260px');
      $("body").append('<div id="my_wksp_view" style="width: 420px; height:420px; padding: 20px;"></div>');
    ...
    describe("in collapsed state", function () {
      ...
      describe("provide header with", function () {
        ...
        var viewIcon, viewDefault, defaultAction = true;
        ...
        beforeEach(function (done) {
          ...
          $.mockjax.clear();
          DataManager.test(1, title, workspaceTypeId, defaultAction, false);
          DataManager.test(1, title, 3, defaultAction, false);
          viewIcon.render();
          viewDefault.render();
      ...
      describe("show", function () {
        ...
        it("custom empty view for 0 workspaces", function (done) {
          ...
          $.mockjax.clear();
          DataManager.test(0, title, workspaceTypeId, defaultAction, false);
          view.render();
          view.completeCollection.connector = context.options.factories.connector;
          waitAsync(done,[view.completeCollection.fetch()]);
          TestUtil.run(done,function () {
      ...
      describe("ignore workspaces order", function () {
        ...
        var viewAsc, viewDesc, defaultAction = true;
        ...
        beforeEach(function (done) {
          ...
          $.mockjax.clear();
          DataManager.test(5, title, workspaceTypeId, defaultAction, false);
          viewAsc.render();
          viewDesc.render();
      ...
      describe("default action", function () {
        ...
        defaultAction = true;
        beforeEach(function (done) {
          view = prepareCollapsedView(done,1, defaultAction);
        });
    ...
    describe("in expanded state", function () {
      ...
      function prepareExpandedView(done,totalCount, defaultAction, addCustomColumns, addIntCustomColumn) {
        var view, tableView;
        ...
        if (addIntCustomColumn) {
          DataManager.testIntCustomColumn(totalCount, title, workspaceTypeId, defaultAction);
        } else {
          DataManager.test(totalCount, title, workspaceTypeId, defaultAction, addCustomColumns || false);
        }
      ...
      describe("show", function () {
        ...
        it("0 workspace", function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            var ariaLabel = tableView.$('table').attr('aria-label');
            expect(ariaLabel).toBe(tableView.options.tableAria);
            expect(ariaLabel).toBe("Content of " + title);
          });
        });
        ...
        it("name, type, size, modify date and favorite column", function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            expect($(tableHead).find('[data-csui-attribute]').length).toEqual(5);
            tableView.$el.remove();
          });
        });

        it("all custom columns", function (done) {
          tableView = prepareExpandedView(done,1, defaultAction, true).tableView;
          ...
          TestUtil.run(done,function () {
            ...
            function hasValue(tableElement,rowNr,fieldid) {
              ...
              if (bodyRows.length>rowNr) {
                ...
                if (field.length>0) {
                  return fieldid+" has value "+field.text().trim();
                }
              }
              var detailsRows = $(tableElement).find('tbody tr.csui-details-row');
        ...
        it("icons for workspaces", function (done) {
          ...
          TestUtil.run(done,function () {
            expect(tableView.$(selectorDefaultContainerIcon).length).toEqual(1);
          });
        });
      });
      ...
      describe("support workspaces order", function () {
        ...
        var viewAsc, viewDesc, tableView, defaultAction = true;
        ...
        beforeEach(function (done) {
          tableView = prepareExpandedView(done,5, defaultAction, true).tableView;
          ...
          $.mockjax.clear();
          DataManager.test(5, title, workspaceTypeId, defaultAction, true);
          DataManager.test(5, title, workspaceTypeId, defaultAction, false);
        });
        afterEach(function(){
          tableView.$el.remove();
        ...
        it("default - by name asc", function () {
          expect(tableView.$(selectorDefaultContainerIcon).length).toEqual(5);
        ...
        function test_sorting(done,key) {
          var tableHead     = tableView.$(expandedExpandedTableHead),
              tableHeadName = $(tableHead).find("[data-csui-attribute=" + key + "]");
          if (key !== 'name') {
            tableHeadName.trigger('click');
        ...
        it("server side via click on '123_2'", function (done) {
          test_sorting(done,'123_2');
        });
        it("server side via click on '124_1'", function (done) {
      ...
      describe("support paging", function () {
        ...
        afterEach(function(){
          tableView.$el.remove();
          tableView.paginationView && tableView.paginationView.$el && tableView.paginationView.$el.remove();
        });
        ...
        it("second page", function (done) {
          ...
          expect($(tableView.$(selectorExpandedTableItemContainer)[0]).find(
              selectorExpandedViewCellName)[0].text.trim()).toEqual(title +
                                                             firstId);
      ...
      describe("default action", function () {
        ...
        beforeEach(function (done) {
          var ret = prepareExpandedView(done,1, defaultAction);
          tableView = ret.tableView;
          tableView.$el.appendTo("body");
          view = ret.view;
        });
        ...
        it("is called at onClick", function () {
          ...
          expect(defaultActionCalled).toBeFalsy();
          defaultAction.trigger('click');
          expect(defaultActionCalled).toBeTruthy();
        });

      });

      describe("no default action - no 'SeeContent' permission", function () {
        ...
        it("no click event available", function () {
          var noDeaultAction = tableView.$(selectorNoDefaultAction);
          expect(noDeaultAction.length).toEqual(2);
          var element = tableView.$(selectorExpandedViewCellName);
          expect(element[0].ariaLabel).toEqual("Insurance1. Can not be opened.")
        });
      ...
      describe("filtering", function () {
        ...
        var tableView, defaultAction = true;
        ...
        beforeEach(function (done) {
          tableView = prepareExpandedView(done,100, defaultAction, true).tableView;
          tableView.$el.appendTo("body");
        });
        afterEach(function(){
          tableView.$el.remove();
        ...
        it("by name fetches proper workspaces", function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            expect($(tableView.$(selectorExpandedTableItemContainer)[0]).find(
                selectorExpandedViewCellName)[0].title.trim()).toEqual(title + 'filter' + firstId);
          });
        });
        ...
        it("by name and 123_1 fetches proper workspaces", function (done) {
          ...
          expect($(tableView.$(selectorExpandedTableItemContainer)[0]).find(
              selectorExpandedViewCellName)[0].title.trim()).toEqual(title + firstId);
          setFilterName(done,tableView);
          ...
          TestUtil.run(done,function (done) {
            ...
            DataManager.testAddFilterExpanded123_1(100, title, workspaceTypeId, defaultAction);
        ...
        it("returns proper error message", function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            $(".binf-modal-dialog .binf-modal-content button.csui-cancel").trigger("click");
          });
        });
      });
      ...
      describe("filtering only allowed", function () {
        ...
        beforeEach(function (done) {
          tableView = prepareExpandedView(done,100, defaultAction, false, true).tableView;
          tableView.$el.appendTo("body");
        });
        afterEach(function(){
          tableView.$el.remove();
