  ...
  TestUtil, lang, MockData) {
    ...
    function FormOptions() {
      ...
      $.extend(true,this.model.attributes,{
        data: {
          customColumnNameA: "{wnf_att_w81_2}" // Contract.Name
        },
        schema: {
          properties: {
            customColumnNameA: {
      ...
      $.extend(true,this.model.attributes,{
        data: {
          customColumnNameB: "{wnf_att_w81_3}" // Contract.Country
        },
        schema: {
          properties: {
            customColumnNameB: {
      ...
      $.extend(true,this.model.attributes,{
        data: {
          customColumnNameD: "{wnf_att_w82_1}" // Contact.ID
        },
        schema: {
          properties: {
            customColumnNameD: {
              description: "Custom column displayed in this widget",
              title: "Custom column D",
              type: "string"
            }
          }
        },
    ...
    function waitForElementReady(elem,msg,timeout) {
      var error = new Error();
      ...
      TestUtil.waitFor(function(){
        ...
        return false;
      },msg||elem,timeout).then(function(){
        deferred.resolve(el);
      },function(){
      ...
      describe('ShowInCreateMode', function () {
        ...
        it('displays placeholder in empty field C', function (done) {
          TestUtil.waitFor(function(){
            ...
            expect(text).toEqual(lang.alpacaPlaceholderOTCustomColumnPicker,"placeholder text");
            done();
          },done.fail);
        });

        describe('First: Custom column picker for empty field C', function() {

          var picker;

          it('opens after click in icons', function (done) {
            el = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-write .csui-nodepicker-icon-container");
            el.trigger("mousedown");
            sel = ".binf-modal-dialog .binf-modal-header .csui-title:contains('Select Custom column C')";
  
            var el1 = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-write .csui-nodepicker-icon-container .cs-clear-icon");
            el1.trigger("mouseover");
  
            el1.trigger("click");
            var el2 = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-write .csui-nodepicker-icon-container .cs-browse-icon");
            el2.trigger("mouseover");
            el2.trigger("click");
  
            waitForElementReady(sel, "node picker title",2000).then(function ($el) {
        ...
        describe('Second: Custom column picker for editable field A', function() {

          var picker;

          it('opens after click in icons', function (done) {
            el = $("[data-alpaca-container-item-name=customColumnNameA] .cs-field-write .csui-nodepicker-icon-container");
            el.trigger("mousedown");
            sel = ".binf-modal-dialog .binf-modal-header .csui-title:contains('Select Custom column A')";
  
            var el1 = $("[data-alpaca-container-item-name=customColumnNameA] .cs-field-write .csui-nodepicker-icon-container .cs-clear-icon");
            el1.trigger("mouseover");
  
            el1.trigger("click");
            var el2 = $("[data-alpaca-container-item-name=customColumnNameA] .cs-field-write .csui-nodepicker-icon-container .cs-browse-icon");
            el2.trigger("mouseover");
            el2.trigger("click");
          ...
          it('has not disabled name column', function (done) {
            el = $(".binf-modal-body .csui-select-lists .csui-np-content .cs-left-item-208963", picker);
            expect(el.hasClass("csui-disabled")).toEqual(false, "field is disabled");
          ...
          it('selects "Area" column', function(done){
            ...
            $(".icon.icon-checkbox", el).trigger("click");
        ...
        describe('Third: Custom column picker for empty field C', function() {
          ...
          it('starts at parent of previously selected custom column', function(done){
            el = $(".binf-modal-header .csui-start-locations .binf-dropdown-toggle .cs-label",picker);
            text = el.text();
            expect(text).toEqual("Facets","correct locations toggle");
            el = $(".binf-modal-body .cs-breadcrumb .binf-breadcrumb",picker);
          ...
          it('shows correct columns', function(done){
            el = $(".binf-modal-body .csui-select-lists .csui-np-content .csui-list-group-item .csui-list-item-title",picker);
            text = el.text().trim().replace(/\s*\n\s*/g,"|");
            expect(text).toEqual("Contact Columns|Contract Columns|Others|People Columns|System Default Columns|System Default Facets|Workspace Columns","correct list");
            done();
          });
          it('closes after click on cancel button', function(done){
            el = $(".binf-modal-footer .csui-footer-buttons button:contains('Cancel')",picker);
        ...
        describe('Fourth: Custom column picker for editable field D', function() {

          var picker;

          it('opens after click in icons', function (done) {
            el = $("[data-alpaca-container-item-name=customColumnNameD] .cs-field-write .csui-nodepicker-icon-container");
            el.trigger("mousedown");
            sel = ".binf-modal-dialog .binf-modal-header .csui-title:contains('Select Custom column D')";
  
            var el1 = $("[data-alpaca-container-item-name=customColumnNameD] .cs-field-write .csui-nodepicker-icon-container .cs-clear-icon");
            el1.trigger("mouseover");
  
            el1.trigger("click");
            var el2 = $("[data-alpaca-container-item-name=customColumnNameD] .cs-field-write .csui-nodepicker-icon-container .cs-browse-icon");
            el2.trigger("mouseover");
            el2.trigger("click");
          ...
          it('starts at custom column parent', function(done){
            ...
            el = $(".binf-modal-body .cs-breadcrumb .binf-breadcrumb",picker);
            text = el.text().trim().replace(/\n/g," > ").replace(/\s+/g," ");
            expect(text).toEqual("Facets > Contact Columns","correct breadcrumb");
            el = $(".binf-modal-body .csui-select-lists .csui-np-header .folder-name",picker);
            text = el.text();
            expect(text).toEqual("Contact Columns","correct folder");
            done();
          });
      ...
      describe('ShowInUpdateMode', function () {
        ...
        it('displays path in readonly field B', function (done) {
          TestUtil.waitFor(function () {
            ...
            el = $("[data-alpaca-container-item-name=customColumnNameB] .cs-field-read a");
            expect(TestUtil.isElementVisible(el[0])).toEqual(true, "readonly path visibility");
            el = $("[data-alpaca-container-item-name=customColumnNameB] .cs-field-read a span");
            expect(el.attr('title')).toEqual("Facets > Contract Columns > Country");
        ...
        it('displays placeholder in empty field C', function (done) {
          TestUtil.waitFor(function () {
            text = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-read span").text().trim();
            return text === "Select column";
          }, "item path to appear").then(function () {
            el = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-write input");
            expect(TestUtil.isElementVisible(el[0])).toEqual(false, "input field visibility");
            el = $("[data-alpaca-container-item-name=customColumnNameC] .cs-field-read span");
            expect(TestUtil.isElementVisible(el[0])).toEqual(true, "readonly path visibility");
            done();
          }, done.fail);
        });
        ...
        describe('Node picker for editable field A', function() {
          ...
          it('opens after click in field browse icon', function (done) {
            el = $("[data-alpaca-container-item-name=customColumnNameA] .cs-field-write .csui-nodepicker-icon-container");
            el.trigger("mousedown");
            sel = ".binf-modal-dialog .binf-modal-header .csui-title:contains('Select Custom column A')";
